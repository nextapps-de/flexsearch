...
- Using `index.where()` to find documents is very slow when using a tagged field
...
- Verify if you can activate _contextual index_ by setting the ___depth___ to a minimum meaningful value
- Pass a ___limit___ when searching (lower is better)
- Pass a minimum ___threshold___ when searching (higher is better)
