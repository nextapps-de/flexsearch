...
function ya(a,c,b){const e=[],d=C();let f;var g=a.length;let k;for(let h=g-1;0<=h;h--)if(k=(g=a[h])&&g.length)for(let l=0;l<k;l++)if(f=g[l],!d[f])if(d[f]=1,b)b--;else if(e.push(f),e.length===c)return e;return e}function za(a,c){const b=C(),e=[];for(let d=0,f;d<c.length;d++){f=c[d];for(let g=0;g<f.length;g++)b[f[g]]=1}for(let d=0,f;d<a.length;d++)f=a[d],b[f]&&(e.push(f),b[f]=0);return e};function W(a,c,b){if(!a.length)return a;if(1===a.length)return a=a[0],a=b||a.length>c?c?a.slice(b,b+c):a.slice(b):a;let e=[];for(let d=0,f,g;d<a.length;d++)if((f=a[d])&&(g=f.length)){if(b){if(b>=g){b-=g;continue}b<g&&(f=c?f.slice(b,b+c):f.slice(b),g=f.length,b=0)}g>c&&(f=f.slice(0,c),g=c);if(!e.length&&g>=c)return f;e.push(f);c-=g;if(!c)break}return e=1<e.length?[].concat.apply([],e):e[0]};function Aa(a,c,b){var e=b[0];if(e.then)return Promise.all(b).then(function(m){return a[c].apply(a,m)});if(e[0]&&e[0].index)return a[c].apply(a,e);e=[];let d=[],f=0,g=0,k,h,l;for(let m=0,n;m<b.length;m++)if(n=b[m]){let r;if(n.constructor===X)r=n.result;else if(n.constructor===Array)r=n;else if(f=n.limit||0,g=n.offset||0,l=n.suggest,h=n.resolve,k=n.enrich&&h,n.index)n.resolve=!1,r=n.index.search(n).result,n.resolve=h;else if(n.and)r=a.and(n.and);else if(n.or)r=a.or(n.or);else if(n.xor)r=a.xor(n.xor);
else if(n.not)r=a.not(n.not);else continue;if(r.then)d.push(r);else if(r.length)e[m]=r;else if(!l&&("and"===c||"xor"===c)){e=[];break}}return{O:e,P:d,limit:f,offset:g,enrich:k,resolve:h,suggest:l}};X.prototype.or=function(){const {O:a,P:c,limit:b,offset:e,enrich:d,resolve:f}=Aa(this,"or",arguments);return Ba.call(this,a,c,b,e,d,f)};function Ba(a,c,b,e,d,f){if(c.length){const g=this;return Promise.all(c).then(function(k){a=[];for(let h=0,l;h<k.length;h++)(l=k[h]).length&&(a[h]=l);return Ba.call(g,a,[],b,e,d,f)})}a.length&&(this.result.length&&a.push(this.result),2>a.length?this.result=a[0]:(this.result=ya(a,b,e),e=0));return f?this.resolve(b,e,d):this};X.prototype.and=function(){let a=this.result.length,c,b,e,d;if(!a){const f=arguments[0];f&&(a=!!f.suggest,d=f.resolve,c=f.limit,b=f.offset,e=f.enrich&&d)}if(a){const {O:f,P:g,limit:k,offset:h,enrich:l,resolve:m,suggest:n}=Aa(this,"and",arguments);return Ca.call(this,f,g,k,h,l,m,n)}return d?this.resolve(c,b,e):this};
function Ca(a,c,b,e,d,f,g){if(c.length){const h=this;return Promise.all(c).then(function(l){a=[];for(let m=0,n;m<l.length;m++)(n=l[m]).length&&(a[m]=n);return Ca.call(h,a,[],b,e,d,f,g)})}if(a.length)if(this.result.length&&a.unshift(this.result),2>a.length)this.result=a[0];else{if(c=aa(a)){this.result=xa(a,c,b,e,g);var k;f?d?k=this.result:k=this.result:k=this;return k}this.result=[]}else g||(this.result=a);return f?this.resolve(b,e,d):this};X.prototype.xor=function(){const {O:a,P:c,limit:b,offset:e,enrich:d,resolve:f,suggest:g}=Aa(this,"xor",arguments);return Da.call(this,a,c,b,e,d,f,g)};
function Da(a,c,b,e,d,f,g){if(c.length){const h=this;return Promise.all(c).then(function(l){a=[];for(let m=0,n;m<l.length;m++)(n=l[m]).length&&(a[m]=n);return Da.call(h,a,[],b,e,d,f,g)})}if(a.length)if(this.result.length&&a.unshift(this.result),2>a.length)this.result=a[0];else{this.result=Ea.call(this,a,b,e,f,this.h);if(f){var k;d?k=this.result:k=this.result;c=k}else c=this;return c}else g||(this.result=a);return f?this.resolve(b,e,d):this}
function Ea(a,c,b,e,d){const f=[],g=C();let k=0;for(let h=0,l;h<a.length;h++)if(l=a[h]){k<l.length&&(k=l.length);for(let m=0,n;m<l.length;m++)if(n=l[m])for(let r=0,p;r<n.length;r++)p=n[r],g[p]=g[p]?2:1}for(let h=0,l,m=0;h<k;h++)for(let n=0,r;n<a.length;n++)if(r=a[n])if(l=r[h])for(let p=0,q;p<l.length;p++)if(q=l[p],1===g[q])if(b)b--;else if(e){if(f.push(q),f.length===c)return f}else{const t=h+(n?d:0);f[t]||(f[t]=[]);f[t].push(q);if(++m===c)return f}return f};X.prototype.not=function(){const {O:a,P:c,limit:b,offset:e,enrich:d,resolve:f,suggest:g}=Aa(this,"not",arguments);return Fa.call(this,a,c,b,e,d,f,g)};
function Fa(a,c,b,e,d,f,g){if(c.length){const h=this;return Promise.all(c).then(function(l){a=[];for(let m=0,n;m<l.length;m++)(n=l[m]).length&&(a[m]=n);return Fa.call(h,a,[],b,e,d,f,g)})}if(a.length&&this.result.length)this.result=Ga.call(this,a,b,e,f);else if(f)return this.resolve(b,e,d);if(f){var k;d?k=this.result:k=this.result;c=k}else c=this;return c}
...
X.prototype.offset=function(a){if(this.result.length){const c=[];for(let b=0,e;b<this.result.length;b++)if(e=this.result[b])e.length<=a?a-=e.length:(c[b]=e.slice(a),a=0);this.result=c}return this};X.prototype.boost=function(a){this.h+=a;return this};X.prototype.resolve=function(a,c){const b=this.result,e=this.index;this.result=this.index=null;return b.length?("object"===typeof a&&(c=a.offset,a=a.limit),W.call(e,b,a||100,c)):b};C();V.prototype.search=function(a,c,b,e){b||(!c&&H(a)?(b=a,a=""):H(c)&&(b=c,c=0));let d=[];var f=[];let g;let k,h,l;let m=0,n=!0,r;if(b){b.constructor===Array&&(b={index:b});a=b.query||a;var p=b.pluck;var q=b.merge;h=p||b.field||(h=b.index)&&(h.index?null:h);l=this.tag&&b.tag;k=b.suggest;n=!0;r=(g=this.store&&b.enrich&&n)&&b.highlight;c=b.limit||c;var t=b.offset||0;c||(c=100);if(l){l.constructor!==Array&&(l=[l]);var u=[];for(let y=0,v;y<l.length;y++)if(v=l[y],v.field&&v.tag){var w=v.tag;if(w.constructor===
Array)for(var z=0;z<w.length;z++)u.push(v.field,w[z]);else u.push(v.field,w)}else{w=Object.keys(v);for(let E=0,F,A;E<w.length;E++)if(F=w[E],A=v[F],A.constructor===Array)for(z=0;z<A.length;z++)u.push(F,A[z]);else u.push(F,A)}l=u;if(!a){f=[];if(u.length)for(p=0;p<u.length;p+=2)q=Ha.call(this,u[p],u[p+1],c,t),d.push({field:u[p],tag:u[p+1],result:q});return f.length?Promise.all(f).then(function(y){for(let v=0;v<y.length;v++)d[v].result=y[v];return d}):d}}h&&h.constructor!==Array&&(h=[h])}h||(h=this.field);
u=!e&&(this.worker||this.db)&&[];for(let y=0,v,E,F;y<h.length;y++){E=h[y];let A;D(E)||(A=E,E=A.field,a=A.query||a,c=A.limit||c,t=A.offset||t,k=A.suggest||k,g=this.store&&(A.enrich||g));if(e)v=e[y];else if(w=A||b,z=this.index.get(E),l&&(w.enrich=!1),u){u[y]=z.search(a,c,w);w&&g&&(w.enrich=g);continue}else v=z.search(a,c,w),w&&g&&(w.enrich=g);F=v&&(n?v.length:v.result.length);if(l&&F){w=[];z=0;for(let J=0,G,Xa;J<l.length;J+=2){G=this.tag.get(l[J]);if(!G)if(k)continue;else return n?d:new X(d);if(Xa=
(G=G&&G.get(l[J+1]))&&G.length)z++,w.push(G);else if(!k)return n?d:new X(d)}if(z){v=za(v,w);F=v.length;if(!F&&!k)return n?v:new X(v);z--}}if(F)f[m]=E,d.push(v),m++;else if(1===h.length)return n?d:new X(d)}if(u){const y=this;return Promise.all(u).then(function(v){return v.length?y.search(a,c,b,v):v})}if(!m)return n?d:new X(d);if(p&&(!g||!this.store))return d[0];u=[];for(t=0;t<f.length;t++){e=d[t];if(p)return n?e:new X(e);d[t]={field:f[t],result:e}}return q?Ia(d,c):r?Ja(d,a,this.index,this.field,this.C,
r):d};
function Ja(a,c,b,e,d,f){let g,k,h;for(let m=0,n,r,p,q;m<a.length;m++){let t=a[m].result;n=a[m].field;p=b.get(n);r=p.encoder;h=p.tokenize;q=d[e.indexOf(n)];r!==g&&(g=r,k=g.encode(c));for(let u=0;u<t.length;u++){let w="";var l=I(t[u].doc,q);let z=g.encode(l);l=l.split(g.split);for(let y=0,v,E;y<z.length;y++){v=z[y];E=l[y];let F;for(let A=0,J;A<k.length;A++)if(J=k[A],"strict"===h){if(v===J){w+=(w?" ":"")+f.replace("$1",E);F=!0;break}}else{const G=v.indexOf(J);if(-1<G){w+=(w?" ":"")+E.substring(0,G)+
