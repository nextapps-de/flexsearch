...
l[w];let z;E(C)||(z=C,C=z.field,a=z.query||a,b=z.limit||b,n=z.offset||n,k=z.suggest||k,f=this.store&&(z.enrich||f));if(e)q=e[w];else if(u=z||c,y=this.index.get(C),m&&(u.enrich=!1),v){v[w]=y.search(a,b,u);u&&f&&(u.enrich=f);continue}else q=y.search(a,b,u),u&&f&&(u.enrich=f);A=q&&q.length;if(m&&A){u=[];y=0;for(let H=0,F,L;H<m.length;H+=2){F=this.tag.get(m[H]);if(!F)if(k)continue;else return d;if(L=(F=F&&F.get(m[H+1]))&&F.length)y++,u.push(F);else if(!k)return d}if(y){q=oa(q,u);A=q.length;if(!A&&!k)return d;
...
new Map;this.B=[];this.R=[];for(let d=0,g,f;d<c.length;d++){g=c[d];f=g.field||g;if(!f)throw Error("The tag field from the document descriptor is undefined.");g.custom?this.B[d]=g.custom:(this.B[d]=S(f,this.H),g.filter&&("string"===typeof this.B[d]&&(this.B[d]=new String(this.B[d])),this.B[d].F=g.filter));this.R[d]=f;this.tag.set(f,new Map)}}}
function S(a,b){const c=a.split(":");let e=0;for(let d=0;d<c.length;d++)a=c[d],"]"===a[a.length-1]&&(a=a.substring(0,a.length-2))&&(b[e]=!0),a&&(c[e++]=a);e<c.length&&(c.length=e);return 1<e?c:c[0]}x=Q.prototype;x.append=function(a,b){return this.add(a,b,!0)};x.update=function(a,b){return this.remove(a).add(a,b)};
...
function Ha(a){let b=0;if(a.constructor===Array)for(let c=0,e;c<a.length;c++)(e=a[c])&&(b+=e.length);else for(const c of a){const e=c[0],d=Ha(c[1]);d?b+=d:a.delete(e)}return b}x.cleanup=function(){if(!this.fastupdate)return this;Ha(this.map);this.depth&&Ha(this.ctx);return this};x.searchCache=ta;
