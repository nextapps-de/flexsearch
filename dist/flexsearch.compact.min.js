 ...
 */
(function(self){'use strict';var B;function I(a,c,b){const e=typeof b,d=typeof a;if(e!=="undefined"){if(d!=="undefined"){if(b){if(d==="function"&&e===d)return function(k){return a(b(k))};c=a.constructor;if(c===b.constructor){if(c===Array)return b.concat(a);if(c===Map){var f=new Map(b);for(var g of a)f.set(g[0],g[1]);return f}if(c===Set){g=new Set(b);for(f of a.values())g.add(f);return g}}}return a}return b}return d==="undefined"?c:a}function K(a,c){return typeof a==="undefined"?c:a}function N(){return Object.create(null)}
...
H);else continue;(H-=n.length)||(H=-1);E[q]=H}else{y[q]=1;continue}if(F+n.length+1<=h)n+=" ",z[q]=n+z[q];else if(m)O=n.length+1-(h-F),O>=0&&O<n.length&&(n=n.substring(O)+" ",z[q]=n+z[q]),y[q]=1;else{y[q]=1;continue}}else{n=r[G].match;u&&(E[q]=u);p&&(D[q]=p);q&&F++;let ua;G?!q&&x&&(F+=x):(t[q]=1,y[q]=1);G>=v.length-1?ua=1:G<r.length-1&&r[G+1].match?ua=1:x&&(F+=x);F-=f.length-2;if(!q||F+n.length<=h)z[q]=n;else{O=R=W=t[q]=0;break}ua&&(t[q+1]=1,y[q+1]=1)}F+=n.length;O=C[G]=1}if(O)J===L?L++:J++;else{J===
