...
new Set);this.S=b.resolution||1;this.rtl=e.rtl||a.rtl||!1;this.cache=(d=a.cache||null)&&new J(d)}v=M.prototype;v.clear=function(){this.map.clear();this.D.clear();this.h.clear();this.cache&&this.cache.clear();return this};v.append=function(a,c){return this.add(a,c,!0)};v.contain=function(a){return this.h.has(a)};v.update=function(a,c){if(this.async){const b=this,e=this.remove(a);return e.then?e.then(()=>b.add(a,c)):this.add(a,c)}return this.remove(a).add(a,c)};
function U(a){let c=0;if(a.constructor===Array)for(let b=0,e;b<a.length;b++)(e=a[b])&&(c+=e.length);else for(const b of a){const e=b[0],d=U(b[1]);d?c+=d:a.delete(e)}return c}v.cleanup=function(){if(!this.fastupdate)return this;U(this.map);this.depth&&U(this.D);return this};v.searchCache=ia;
...
f.I;if(g&&!g(c))continue;f.constructor===String&&(f=""+f);f=F(c,f)}if(d&&f){z(f)&&(f=[f]);for(let k=0,m,l;k<f.length;k++)m=f[k],h[m]||(h[m]=1,(g=d.get(m))?l=g:d.set(m,l=[]),b&&l.includes(a)||(l.push(a),this.fastupdate&&((g=this.h.get(a))?g.push(l):this.h.set(a,[l]))))}}if(this.store&&(!b||!this.store.has(a))){let h;if(this.B){h=y();for(let k=0,m;k<this.B.length;k++){m=this.B[k];if((b=m.I)&&!b(c))continue;let l;if("function"===typeof m){l=m(c);if(!l)continue;m=[m.U]}else if(z(m)||m.constructor===String){h[m]=
c[m];continue}X(c,h,m,0,m[0],l)}}this.store.set(a,h||c)}}return this};function X(a,c,b,e,d,f){a=a[d];if(e===b.length-1)c[d]=f||a;else if(a)if(a.constructor===Array)for(c=c[d]=Array(a.length),d=0;d<a.length;d++)X(a,c,b,e,d);else c=c[d]||(c[d]=y()),d=b[++e],X(a,c,b,e,d)}
...
else n.push(q.field,u)}else{u=Object.keys(q);for(let C=0,D,A;C<u.length;C++)if(D=u[C],A=q[D],A.constructor===Array)for(B=0;B<A.length;B++)n.push(D,A[B]);else n.push(D,A)}l=n;if(!a){e=[];if(n.length)for(f=0;f<n.length;f+=2)t=xa.call(this,n[f],n[f+1],c,p,g),d.push({field:n[f],tag:n[f+1],result:t});return e.length?Promise.all(e).then(function(w){for(let q=0;q<w.length;q++)d[q].result=w[q];return d}):d}}z(m)&&(m=[m])}m||(m=this.field);p=!e&&(this.worker||this.async)&&[];for(let w=0,q,C,D;w<m.length;w++){C=
m[w];let A;z(C)||(A=C,C=A.field,a=A.query||a,c=A.limit||c,k=A.suggest||k);if(e)q=e[w];else if(n=A||b,u=this.index.get(C),l&&(n.enrich=!1),p){p[w]=u.searchAsync(a,c,n);n&&g&&(n.enrich=g);continue}else q=u.search(a,c,n),n&&g&&(n.enrich=g);D=q&&q.length;if(l&&D){n=[];u=0;for(let P=0,G,Ba;P<l.length;P+=2){G=this.tag.get(l[P]);if(!G)if(k)continue;else return d;if(Ba=(G=G&&G.get(l[P+1]))&&G.length)u++,n.push(G);else if(!k)return d}if(u){q=va(q,n);D=q.length;if(!D&&!k)return d;u--}}if(D)f[r]=C,d.push(q),
r++;else if(1===m.length)return d}if(p){const w=this;return Promise.all(p).then(function(q){return q.length?w.search(a,c,b,q):q})}if(!r)return d;if(t&&(!g||!this.store))return d[0];p=[];for(let w=0,q;w<f.length;w++){q=d[w];g&&q.length&&!q[0].doc&&q.length&&(q=ya.call(this,q));if(t)return q;d[w]={field:f[w],result:q}}return h?za(d,c):d};
function za(a,c){const b=[],e=y();for(let d=0,f,g;d<a.length;d++){f=a[d];g=f.result;for(let h=0,k,m,l;h<g.length;h++)if(m=g[h],k=m.id,l=e[k])l.push(f.field);else{if(b.length===c)return b;m.field=e[k]=[f.field];b.push(m)}}return b}function xa(a,c,b,e,d){a=this.tag.get(a);if(!a)return[];if((c=(a=a&&a.get(c))&&a.length-e)&&0<c){if(c>b||e)a=a.slice(e,e+b);d&&(a=ya.call(this,a));return a}}
...
v.import=function(a,c){if(c)switch(z(c)&&(c=JSON.parse(c)),a){case "tag":this.G=c;break;case "reg":this.fastupdate=!1;this.h=c;for(let e=0,d;e<this.field.length;e++)d=this.index[this.field[e]],d.h=c,d.fastupdate=!1;break;case "store":this.store=c;break;default:a=a.split(".");const b=a[0];a=a[1];b&&a&&this.index[b].import(a,c)}};ta(V.prototype);const Aa={Index:M,Charset:qa,Encoder:I,Document:V,Worker:null,Resolver:null,IndexedDB:null,Language:{}},Z=self;let Ca;(Ca=Z.define)&&Ca.amd?Ca([],function(){return Aa}):"object"===typeof Z.exports?Z.exports=Aa:Z.FlexSearch=Aa;}(this));
