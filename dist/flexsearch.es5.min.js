...
b+d+e+"}":b+d+e};Ya(V.prototype);function ob(a){var b,c,d,e,h,f,g,k;return H(function(l){a=a.data;b=self._index;c=a.args;d=a.task;switch(d){case "init":e=a.options||{};(h=e.config)&&(e=(await import(h))["default"]);(f=a.factory)?(Function("return "+f)()(self),self._index=new self.FlexSearch.Index(e),delete self.FlexSearch):self._index=new V(e);postMessage({id:a.id});break;default:g=a.id,k=b[d].apply(b,c),postMessage("search"===d?{id:g,msg:k}:{id:g})}l.h=0})};var pb=0;
function qb(a){function b(f){function g(k){k=k.data||k;var l=k.id,m=l&&e.h[l];m&&(m(k.msg),delete e.h[l])}this.worker=f;this.h=L();if(this.worker){d?this.worker.on("message",g):this.worker.onmessage=g;if(a.config)return new Promise(function(k){e.h[++pb]=function(){k(e)};e.worker.postMessage({id:pb,task:"init",factory:c,options:a})});this.worker.postMessage({task:"init",factory:c,options:a});return this}}a=void 0===a?{}:a;if(!this)return new qb(a);var c="undefined"!==typeof self?self._factory:"undefined"!==
typeof window?window._factory:null;c&&(c=c.toString());var d="undefined"===typeof window,e=this,h=rb(c,d,a.worker);return h.then?h.then(function(f){return b.call(e,f)}):b.call(this,h)}sb("add");sb("append");sb("search");sb("update");sb("remove");
...
0,k=x(h.index.entries()),l=k.next();!l.done;l=k.next()){l=l.value;var m=l[0];l[1].then&&h.index.set(m,f[g++])}return h})}}else a.db&&this.mount(a.db)}u=Z.prototype;
...
f.enrich(k):k}return g})};u.tag=function(a,b,c,d){b=void 0===b?0:b;c=void 0===c?0:c;d=void 0===d?!1:d;a=this.db.transaction("tag","readonly").objectStore("tag").get(a);var e=this;return Db(a).then(function(h){if(!h||!h.length||c>=h.length)return[];if(!b&&!c)return h;h=h.slice(c,c+b);return d?e.enrich(h):h})};
