...
u.keys=U.prototype.keys=function $a(){var b,c=this,d,e,h;return ra($a,function(f){switch(f.h){case 1:b=0;case 2:if(!(b<c.h.length)){f.h=0;break}d=x(c.h[b].keys());e=d.next();case 5:if(e.done){b++;f.h=2;break}h=e.value;return G(f,h,6);case 6:e=d.next(),f.h=5}})};
...
t&&t.length;if(m&&z){A=[];B=0;if(this.db&&d){if(!F)for(D=l.length;D<d.length;D++){var L=d[D];if(L&&L.length)B++,A.push(L);else if(!p)return e}}else{D=0;for(var Nb=L=void 0;D<m.length;D+=2){L=this.tag.get(m[D]);if(!L)if(p)continue;else return e;if(Nb=(L=L&&L.get(m[D+1]))&&L.length)B++,A.push(L);else if(!p)return e}}if(B){t=fb(t,A);z=t.length;if(!z&&!p)return e;B--}}if(z)h[f]=y,e.push(t),f++;else if(1===l.length)return e}if(v){if(this.db&&m&&m.length&&!F)for(n=0;n<m.length;n+=2){h=this.index.get(m[n]);
if(!h)if(p)continue;else return e;v.push(h.db.tag(m[n+1],b,r,!1))}var Ob=this;return Promise.all(v).then(function(P){return P.length?Ob.search(a,b,c,P):P})}if(!f)return e;if(g&&(!n||!this.store))return e[0];v=[];r=0;for(p=void 0;r<h.length;r++){p=e[r];n&&p.length&&!p[0].doc&&(this.db?v.push(p=this.index.get(this.field[0]).db.enrich(p)):p.length&&(p=hb.call(this,p)));if(g)return p;e[r]={field:h[r],result:p}}if(n&&this.db&&v.length){var Ga=this;return Promise.all(v).then(function(P){for(var Q=0;Q<P.length;Q++)e[Q].result=
P[Q];return k?ib(e,b):q?jb(e,a,Ga.index,Ga.field,Ga.I,q):e})}return k?ib(e,b):q?jb(e,a,this.index,this.field,this.I,q):e};
...
u.get=function(a){return this.db?this.index.get(this.field[0]).db.enrich(a).then(function(b){return b[0]&&b[0].doc}):this.store.get(a)};u.set=function(a,b){this.store.set(a,b);return this};u.searchCache=mb;
...
(b||this.commit_task.push({del:a}),this.da&&zb(this));return this};
...
this.not(l.not);else continue;d[k]=m;m.then&&c.push(m)}if(c.length)return Promise.all(c).then(function(){a.result.length&&(d=[a.result].concat(d));a.result=Db(d,e,h,f,!g,a.J);return g?a.result:a});d.length&&(this.result.length&&(d=[this.result].concat(d)),this.result=Db(d,e,h,f,!g,a.J));return g?this.result:this};
...
else continue;d[g]=l;l.then&&c.push(l)}if(c.length)return Promise.all(c).then(function(){a.result=Eb.call(a,d,e,h,f);return f?a.result:a});d.length&&(this.result=Eb.call(this,d,e,h,f));return f?this.result:this};
...
t?(f=Z(this,k,t,0,0,!1,!1),f=Hb(f,d,g,this.aa),g&&!1===f&&d.length||(t=k)):(f=Z(this,k,"",0,0,!1,!1),f=Hb(f,d,g,this.resolution));if(f)return f;if(g&&y===n-1){f=d.length;if(!f){if(t){t="";y=-1;continue}return d}if(1===f)return h?X(d[0],b,e):new Y(d[0])}}d=db(d,this.resolution,b,e,g,l,h);return h?d:new Y(d)};
function Gb(a,b,c,d,e,h,f){a=Z(this,a,b,c,d,e,h,f);return this.db?a.then(function(g){return e?g:g&&g.length?e?X(g,c,d):new Y(g):e?[]:new Y([])}):a&&a.length?e?X(a,c,d):new Y(a):e?[]:new Y([])}function Hb(a,b,c,d){var e=[];if(a){d=Math.min(a.length,d);for(var h=0,f=void 0;h<d;h++)(f=a[h])&&f&&(e[h]=f);if(e.length){b.push(e);return}}return!c&&e}
function Z(a,b,c,d,e,h,f,g){var k;c&&(k=a.bidirectional&&b>c);if(a.db)return c?a.db.get(k?c:b,k?b:c,d,e,h,f,g):a.db.get(b,"",d,e,h,f,g);a=c?(a=a.ctx.get(k?b:c))&&a.get(k?c:b):a.map.get(b);return a};O.prototype.remove=function(a,b){var c=this.reg.size&&(this.fastupdate?this.reg.get(a):this.reg.has(a));if(c){if(this.fastupdate)for(var d=0,e;d<c.length;d++){if(e=c[d])if(2>e.length)e.pop();else{var h=e.indexOf(a);h===c.length-1?e.pop():e.splice(h,1)}}else Ib(this.map,a),this.depth&&Ib(this.ctx,a);b||this.reg.delete(a)}this.db&&(this.commit_task.push({del:a}),this.da&&zb(this));this.cache&&this.cache.remove(a);return this};
...
u.commit=function(a,b){this.commit_timer&&(clearTimeout(this.commit_timer),this.commit_timer=null);return this.db.commit(this,a,b)};u.destroy=function(){this.commit_timer&&(clearTimeout(this.commit_timer),this.commit_timer=null);return this.db.destroy()};function zb(a){a.commit_timer||(a.commit_timer=setTimeout(function(){a.commit_timer=null;a.db.commit(a,void 0,void 0)},0))}
...
function Jb(a){var b=0;if(a.constructor===Array)for(var c=0,d=void 0;c<a.length;c++)(d=a[c])&&(b+=d.length);else for(c=x(a),d=c.next();!d.done;d=c.next()){var e=d.value;d=e[0];(e=Jb(e[1]))?b+=e:a.delete(d)}return b}u.cleanup=function(){if(!this.fastupdate)return this;Jb(this.map);this.depth&&Jb(this.ctx);return this};u.searchCache=mb;
u.export=function(a,b,c,d){d=void 0===d?0:d;switch(d){case 0:var e="reg";var h=Ta(this.reg);break;case 1:e="cfg";h={};break;case 2:e="map";h=Pa(this.map,this.reg.size);break;case 3:e="ctx";h=Ra(this.ctx,this.reg.size);break;default:return}return Va.call(this,a,b,e,h,c,d)};
u.import=function(a,b){if(b)switch(K(b)&&(b=JSON.parse(b)),a=a.split("."),"json"===a[a.length-1]&&a.pop(),a=1<a.length?a[1]:a[0],a){case "reg":this.fastupdate=!1;this.reg=Ua(b,this.reg);break;case "map":this.map=Qa(b,this.map);break;case "ctx":this.ctx=Sa(b,this.ctx)}};
...
b+d+e+"}":b+d+e};Na(O.prototype);var Kb="undefined"!==typeof window&&(window.indexedDB||window.mozIndexedDB||window.webkitIndexedDB||window.msIndexedDB),Lb=["map","ctx","tag","reg","cfg"];
function Mb(a,b){b=void 0===b?{}:b;if(!this)return new Mb(a,b);"object"===typeof a&&(b=a=a.name);a||console.info("Default storage space was used, because a name was not passed.");this.id="flexsearch"+(a?":"+a.toLowerCase().replace(/[^a-z0-9_\-]/g,""):"");this.field=b.field?b.field.toLowerCase().replace(/[^a-z0-9_\-]/g,""):"";this.support_tag_search=!1;this.db=null;this.h={}}u=Mb.prototype;u.mount=function(a){if(!a.encoder)return a.mount(this);a.db=this;return this.open()};
...
u.close=function(){this.db.close();this.db=null};u.destroy=function(){return Kb.deleteDatabase(this.id+(this.field?":"+this.field:""))};u.clear=function(){for(var a=this.db.transaction(Lb,"readwrite"),b=0;b<Lb.length;b++)a.objectStore(Lb[b]).clear();return Pb(a)};
...
f.enrich(k):k}return g})};u.tag=function(a,b,c,d){b=void 0===b?0:b;c=void 0===c?0:c;d=void 0===d?!1:d;a=this.db.transaction("tag","readonly").objectStore("tag").get(a);var e=this;return Pb(a).then(function(h){if(!h||!h.length||c>=h.length)return[];if(!b&&!c)return h;h=h.slice(c,c+b);return d?e.enrich(h):h})};
u.enrich=function(a){"object"!==typeof a&&(a=[a]);for(var b=this.db.transaction("reg","readonly").objectStore("reg"),c=[],d=0;d<a.length;d++)c[d]=Pb(b.get(a[d]));return Promise.all(c).then(function(e){for(var h=0;h<e.length;h++)e[h]={id:a[h],doc:e[h]?JSON.parse(e[h]):null};return e})};u.has=function(a){a=this.db.transaction("reg","readonly").objectStore("reg").getKey(a);return Pb(a)};u.search=null;u.info=function(){};
...
u.remove=function(a){"object"!==typeof a&&(a=[a]);return Promise.all([this.transaction("map","readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&Qb(c,a)}}),this.transaction("ctx","readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&Qb(c,a)}}),this.transaction("tag","readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&Qb(c,a,!0)}}),this.transaction("reg","readwrite",function(b){for(var c=0;c<a.length;c++)b.delete(a[c])})])};
function Pb(a){return new Promise(function(b,c){a.onsuccess=function(){b(this.result)};a.oncomplete=function(){b(this.result)};a.onerror=c;a=null})};var Rb={Index:O,Charset:vb,Encoder:N,Document:V,Worker:R,Resolver:Y,IndexedDB:Mb,Language:{}},Sb=self,Tb;(Tb=Sb.define)&&Tb.amd?Tb([],function(){return Rb}):"object"===typeof Sb.exports?Sb.exports=Rb:Sb.FlexSearch=Rb;}(this||self));
