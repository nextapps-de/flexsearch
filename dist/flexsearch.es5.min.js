...
5;break}return E(h,g,7);case 7:g=h.D;h.h=5;break;case 9:f=h.D,Ga.import(d[0],f);case 5:postMessage("search"===b?{id:c,msg:g}:{id:c}),h.h=0}})};function Ja(a){Ma.call(a,"add");Ma.call(a,"append");Ma.call(a,"search");Ma.call(a,"update");Ma.call(a,"remove")}var Na,Oa,Pa;function Qa(){Na=Pa=0}
...
e:new X(e)}}if(H){v=lb(v,D,k);y=v.length;if(!y&&!q)return k?v:new X(v);H--}}if(y)g[h]=z,e.push(v),h++;else if(1===n.length)return k?e:new X(e)}if(t){if(this.db&&p&&p.length&&!Ka)for(f=0;f<p.length;f+=2){g=this.index.get(p[f]);if(!g)if(q)continue;else return k?e:new X(e);t.push(g.db.tag(p[f+1],b,w,!1))}var Yb=this;return Promise.all(t).then(function(P){return P.length?Yb.search(a,b,c,P):P})}if(!h)return k?e:new X(e);if(l&&(!f||!this.store))return e[0];t=[];for(w=0;w<g.length;w++){q=e[w];f&&q.length&&
"undefined"===typeof q[0].doc&&(this.db?t.push(q=this.index.get(this.field[0]).db.enrich(q)):q=W.call(this,q));if(l)return k?q:new X(q);e[w]={field:g[w],result:q}}if(f&&this.db&&t.length){var La=this;return Promise.all(t).then(function(P){for(var Q=0;Q<P.length;Q++)e[Q].result=P[Q];return m?vb(e,b):r?wb(e,a,La.index,La.field,La.J,r):e})}return m?vb(e,b):r?wb(e,a,this.index,this.field,this.J,r):e};
...
function Qb(a){var b=0;if(a.constructor===Array)for(var c=0,d=void 0;c<a.length;c++)(d=a[c])&&(b+=d.length);else for(c=x(a.entries()),d=c.next();!d.done;d=c.next()){var e=d.value;d=e[0];(e=Qb(e[1]))?b+=e:a.delete(d)}return b}u.cleanup=function(){if(!this.fastupdate)return this;Qb(this.map);this.depth&&Qb(this.ctx);return this};u.searchCache=zb;
...
b+c+d+"}":b+c+d};Ja(O.prototype);var Rb="undefined"!==typeof window&&(window.indexedDB||window.mozIndexedDB||window.webkitIndexedDB||window.msIndexedDB),Sb=["map","ctx","tag","reg","cfg"],Tb=J();
...
u.open=function(){if(this.db)return this.db;var a=this;navigator.storage&&navigator.storage.persist();Tb[a.id]||(Tb[a.id]=[]);Tb[a.id].push(a.field);var b=Rb.open(a.id,1);b.onupgradeneeded=function(){for(var c=a.db=this.result,d=0,e;d<Sb.length;d++){e=Sb[d];for(var g=0,f;g<Tb[a.id].length;g++)f=Tb[a.id][g],c.objectStoreNames.contains(e+("reg"!==e?f?":"+f:"":""))||c.createObjectStore(e+("reg"!==e?f?":"+f:"":""))}};return a.db=Z(b,function(c){a.db=c;a.db.onversionchange=function(){a.close()}})};
u.close=function(){this.db&&this.db.close();this.db=null};u.destroy=function(){var a=Rb.deleteDatabase(this.id);return Z(a)};u.clear=function(){for(var a=[],b=0,c;b<Sb.length;b++){c=Sb[b];for(var d=0,e;d<Tb[this.id].length;d++)e=Tb[this.id][d],a.push(c+("reg"!==c?e?":"+e:"":""))}b=this.db.transaction(a,"readwrite");for(c=0;c<a.length;c++)b.objectStore(a[c]).clear();return Z(b)};
...
function Vb(a,b,c){for(var d=a.value,e,g=0,f=0,h;f<d.length;f++){if(h=c?d:d[f]){for(var k=0,l;k<b.length;k++)if(l=b[k],l=h.indexOf(l),0<=l)if(e=1,1<h.length)h.splice(l,1);else{d[f]=[];break}g+=h.length}if(c)break}g?e&&a.update(d):a.delete();a.continue()}
u.remove=function(a){"object"!==typeof a&&(a=[a]);return Promise.all([this.transaction("map","readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&Vb(c,a)}}),this.transaction("ctx","readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&Vb(c,a)}}),this.transaction("tag","readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&Vb(c,a,!0)}}),this.transaction("reg","readwrite",function(b){for(var c=0;c<a.length;c++)b.delete(a[c])})])};
function Z(a,b){return new Promise(function(c,d){a.onsuccess=a.oncomplete=function(){b&&b(this.result);b=null;c(this.result)};a.onerror=a.onblocked=d;a=null})};var Wb={Index:O,Charset:Gb,Encoder:Fa,Document:V,Worker:Sa,Resolver:X,IndexedDB:Ub,Language:{}},Zb="undefined"!==typeof self?self:"undefined"!==typeof global?global:self,$b;($b=Zb.define)&&$b.amd?$b([],function(){return Wb}):"object"===typeof Zb.exports?Zb.exports=Wb:Zb.FlexSearch=Wb;}(this||self));
