...
O[P];return e}):e}}L(l)&&(l=[l])}l||(l=this.field);r=!d&&(this.worker||this.async)&&[];x=0;for(v=u=t=void 0;x<l.length;x++)if(u=l[x],!this.db||!this.tag||this.O[x]){t=void 0;L(u)||(t=u,u=t.field,a=t.query||a,b=t.limit||b,p=t.suggest||p);if(d)t=d[x];else{v=t||c;t=this.index.get(u);if(m){if(this.db){v.tag=m;var E=t.db.sa;v.field=l}E||(v.enrich=!1)}if(r){r[x]=t.searchAsync(a,b,v);v&&n&&(v.enrich=n);continue}else t=t.search(a,b,v),v&&n&&(v.enrich=n)}v=t&&t.length;if(m&&v){A=[];C=0;if(this.db&&d){if(!E)for(D=
