...
function wb(a,b,c,d,e,g){for(var f,h,k,l=0,m,n,p;l<a.length;l++)for(m=a[l].result,n=a[l].field,k=c.get(n),p=k.encoder,k=k.tokenize,n=e[d.indexOf(n)],p!==f&&(f=p,h=f.encode(b)),p=0;p<m.length;p++){var q="",r=ya(m[p].doc,n),w=f.encode(r);r=r.split(f.split);for(var t=0,A,z;t<w.length;t++){A=w[t];z=r[t];for(var v=void 0,y=0,D;y<h.length;y++)if(D=h[y],"strict"===k){if(A===D){q+=(q?" ":"")+g.replace("$1",z);v=!0;break}}else{var G=A.indexOf(D);if(-1<G){q+=(q?" ":"")+z.substring(0,G)+g.replace("$1",z.substring(G,
