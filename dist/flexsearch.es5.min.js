...
1],b,x,q))}else k=ub.call(this,t[g],t[g+1],b,x,q);e.push({field:t[g],tag:t[g+1],result:k})}return h.length?Promise.all(h).then(function(P){for(var Q=0;Q<P.length;Q++)e[Q].result=P[Q];return e}):e}}m&&m.constructor!==Array&&(m=[m])}m||(m=this.field);t=!d&&(this.worker||this.db)&&[];A=0;for(y=z=v=void 0;A<m.length;A++)if(z=m[A],!this.db||!this.tag||this.J[A]){v=void 0;K(z)||(v=z,z=v.field,a=v.query||a,b=v.limit||b,x=v.offset||x,p=v.suggest||p,q=this.store&&(v.enrich||q));if(d)v=d[A];else{y=v||c;v=this.index.get(z);
if(n){if(this.db){y.tag=n;var Ka=v.db.support_tag_search;y.field=m}Ka||(y.enrich=!1)}if(t){t[A]=v.search(a,b,y);y&&q&&(y.enrich=q);continue}else v=v.search(a,b,y),y&&q&&(y.enrich=q)}y=v&&(h?v.length:v.result.length);if(n&&y){D=[];G=0;if(this.db&&d){if(!Ka)for(I=m.length;I<d.length;I++){var L=d[I];if(L&&L.length)G++,D.push(L);else if(!p)return h?e:new Y(e)}}else{I=0;for(var Wb=L=void 0;I<n.length;I+=2){L=this.tag.get(n[I]);if(!L)if(p)continue;else return h?e:new Y(e);if(Wb=(L=L&&L.get(n[I+1]))&&L.length)G++,
D.push(L);else if(!p)return h?e:new Y(e)}}if(G){v=lb(v,D,h);y=v.length;if(!y&&!p)return h?v:new Y(v);G--}}if(y)g[f]=z,e.push(v),f++;else if(1===m.length)return h?e:new Y(e)}if(t){if(this.db&&n&&n.length&&!Ka)for(q=0;q<n.length;q+=2){g=this.index.get(n[q]);if(!g)if(p)continue;else return h?e:new Y(e);t.push(g.db.tag(n[q+1],b,x,!1))}var Xb=this;return Promise.all(t).then(function(P){return P.length?Xb.search(a,b,c,P):P})}if(!f)return h?e:new Y(e);if(k&&(!q||!this.store))return e[0];t=[];x=0;for(p=void 0;x<
g.length;x++){p=e[x];q&&p.length&&!p[0].doc&&(this.db?t.push(p=this.index.get(this.field[0]).db.enrich(p)):p=X.call(this,p));if(k)return h?p:new Y(p);e[x]={field:g[x],result:p}}if(q&&this.db&&t.length){var La=this;return Promise.all(t).then(function(P){for(var Q=0;Q<P.length;Q++)e[Q].result=P[Q];return l?vb(e,b):r?wb(e,a,La.index,La.field,La.J,r):e})}return l?vb(e,b):r?wb(e,a,this.index,this.field,this.J,r):e};
