...
p;var Ka=v.db.support_tag_search;y.field=n}Ka||(y.enrich=!1)}if(t){t[A]=v.search(a,b,y);y&&f&&(y.enrich=f);continue}else v=v.search(a,b,y),y&&f&&(y.enrich=f)}y=v&&(k?v.length:v.result.length);if(p&&y){D=[];G=0;if(this.db&&d){if(!Ka)for(I=n.length;I<d.length;I++){var M=d[I];if(M&&M.length)G++,D.push(M);else if(!q)return k?e:new X(e)}}else{I=0;for(var Xb=M=void 0;I<p.length;I+=2){M=this.tag.get(p[I]);if(!M)if(q)continue;else return k?e:new X(e);if(Xb=(M=M&&M.get(p[I+1]))&&M.length)G++,D.push(M);else if(!q)return k?
e:new X(e)}}if(G){v=mb(v,D,k);y=v.length;if(!y&&!q)return k?v:new X(v);G--}}if(y)g[h]=z,e.push(v),h++;else if(1===n.length)return k?e:new X(e)}if(t){if(this.db&&p&&p.length&&!Ka)for(f=0;f<p.length;f+=2){g=this.index.get(p[f]);if(!g)if(q)continue;else return k?e:new X(e);t.push(g.db.tag(p[f+1],b,w,!1))}var Yb=this;return Promise.all(t).then(function(P){return P.length?Yb.search(a,b,c,P):P})}if(!h)return k?e:new X(e);if(l&&(!f||!this.store))return e[0];t=[];for(w=0;w<g.length;w++){q=e[w];f&&q.length&&
...
b+c+d+"}":b+c+d};Ma(O.prototype);var Ub="undefined"!==typeof window&&(window.indexedDB||window.mozIndexedDB||window.webkitIndexedDB||window.msIndexedDB),Vb=["map","ctx","tag","reg","cfg"],Z=J();
...
u.open=function(){var a=this;navigator.storage&&navigator.storage.persist();return this.db||(this.db=new Promise(function(b,c){Z[a.id]||(Z[a.id]=[]);Z[a.id].push(a.field);var d=Ub.open(a.id,1);d.onupgradeneeded=function(){for(var e=a.db=this.result,g=0,f;g<Vb.length;g++){f=Vb[g];for(var h=0,k;h<Z[a.id].length;h++)k=Z[a.id][h],e.objectStoreNames.contains(f+("reg"!==f?k?":"+k:"":""))||e.createObjectStore(f+("reg"!==f?k?":"+k:"":""))}};d.onblocked=function(e){console.error("blocked",e);c()};d.onerror=
function(e){console.error(this.error,e);c()};d.onsuccess=function(){a.db=this.result;a.db.onversionchange=function(){a.close()};b(a)}}))};u.close=function(){this.db&&this.db.close();this.db=null};u.destroy=function(){var a=Ub.deleteDatabase(this.id);return Zb(a)};
u.clear=function(){for(var a=[],b=0,c;b<Vb.length;b++){c=Vb[b];for(var d=0,e;d<Z[this.id].length;d++)e=Z[this.id][d],a.push(c+("reg"!==c?e?":"+e:"":""))}b=this.db.transaction(a,"readwrite");for(c=0;c<a.length;c++)b.objectStore(a[c]).clear();return Zb(b)};
u.get=function(a,b,c,d,e,g){c=void 0===c?0:c;d=void 0===d?0:d;e=void 0===e?!0:e;g=void 0===g?!1:g;a=this.db.transaction((b?"ctx":"map")+(this.field?":"+this.field:""),"readonly").objectStore((b?"ctx":"map")+(this.field?":"+this.field:"")).get(b?b+":"+a:a);var f=this;return Zb(a).then(function(h){var k=[];if(!h||!h.length)return k;if(e){if(!c&&!d&&1===h.length)return h[0];for(var l=0,m=void 0;l<h.length;l++)if((m=h[l])&&m.length)if(d>=m.length)d-=m.length;else{for(var n=c?d+Math.min(m.length-d,c):
m.length,p=d;p<n;p++)k.push(m[p]);d=0;if(k.length===c)break}return g?f.enrich(k):k}return h})};u.tag=function(a,b,c,d){b=void 0===b?0:b;c=void 0===c?0:c;d=void 0===d?!1:d;a=this.db.transaction("tag"+(this.field?":"+this.field:""),"readonly").objectStore("tag"+(this.field?":"+this.field:"")).get(a);var e=this;return Zb(a).then(function(g){if(!g||!g.length||c>=g.length)return[];if(!b&&!c)return g;g=g.slice(c,c+b);return d?e.enrich(g):g})};
u.enrich=function(a){"object"!==typeof a&&(a=[a]);for(var b=this.db.transaction("reg","readonly").objectStore("reg"),c=[],d=0;d<a.length;d++)c[d]=Zb(b.get(a[d]));return Promise.all(c).then(function(e){for(var g=0;g<e.length;g++)e[g]={id:a[g],doc:e[g]?JSON.parse(e[g]):null};return e})};u.has=function(a){a=this.db.transaction("reg","readonly").objectStore("reg").getKey(a);return Zb(a).then(function(b){return!!b})};u.search=null;u.info=function(){};
u.transaction=function(a,b,c){var d=this,e=a+("reg"!==a?this.field?":"+this.field:"":""),g=this.h[e+":"+b];if(g)return c.call(this,g);var f=this.db.transaction(e,b);this.h[e+":"+b]=g=f.objectStore(e);return new Promise(function(h,k){f.onerror=function(m){f.abort();f=g=null;k(m)};f.oncomplete=function(m){f=g=null;h(m||!0)};var l=c.call(d,g);d.h[e+":"+b]=null;return l})};
...
function $b(a,b,c){for(var d=a.value,e,g,f=0,h=0,k;h<d.length;h++){if(k=c?d:d[h]){for(var l=0,m,n;l<b.length;l++)if(n=b[l],m=k.indexOf(g?parseInt(n,10):n),0>m&&!g&&"string"===typeof n&&!isNaN(n)&&(m=k.indexOf(parseInt(n,10)))&&(g=1),0<=m)if(e=1,1<k.length)k.splice(m,1);else{d[h]=[];break}f+=k.length}if(c)break}f?e&&a.update(d):a.delete();a.continue()}
u.remove=function(a){"object"!==typeof a&&(a=[a]);return Promise.all([this.transaction("map"+(this.field?":"+this.field:""),"readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&$b(c,a)}}),this.transaction("ctx"+(this.field?":"+this.field:""),"readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&$b(c,a)}}),this.transaction("tag"+(this.field?":"+this.field:""),"readwrite",function(b){b.openCursor().onsuccess=function(){var c=this.result;c&&$b(c,
a,!0)}}),this.transaction("reg","readwrite",function(b){for(var c=0;c<a.length;c++)b.delete(a[c])})])};function Zb(a){return new Promise(function(b,c){a.onsuccess=function(){b(this.result)};a.oncomplete=function(){b()};a.onerror=c;a=null})};var ac={Index:O,Charset:Jb,Encoder:Ga,Document:V,Worker:Ta,Resolver:X,IndexedDB:Wb,Language:{}},bc="undefined"!==typeof global?global:self,cc;(cc=bc.define)&&cc.amd?cc([],function(){return ac}):"object"===typeof bc.exports?bc.exports=ac:bc.FlexSearch=ac;}(this||self));
