...
I.prototype.encode=function(a){if(this.cache&&a.length<=this.h)if(this.L){if(this.H.has(a))return this.H.get(a)}else this.L=setTimeout(ka,50,this);this.normalize&&(a="function"===typeof this.normalize?this.normalize(a):ja?a.normalize("NFKD").replace(ja,"").toLowerCase():a.toLowerCase());this.prepare&&(a=this.prepare(a));this.numeric&&3<a.length&&(a=a.replace(ha,"$1 $2").replace(ia,"$1 $2").replace(fa,"$1 "));const b=!(this.dedupe||this.mapper||this.filter||this.matcher||this.stemmer||this.replacer);
...
this.minlength||this.filter&&this.filter.has(g))&&(g="");if(g&&(this.mapper||this.dedupe&&1<g.length)){e="";for(let l=0,m="",n,p;l<g.length;l++)n=g.charAt(l),n===m&&this.dedupe||((p=this.mapper&&this.mapper.get(n))||""===p?p===m&&this.dedupe||!(m=p)||(e+=p):e+=m=n);g=e}this.matcher&&1<g.length&&(this.O||(this.O=new RegExp("("+this.B+")","g")),g=g.replace(this.O,l=>this.matcher.get(l)));if(g&&this.replacer)for(e=0;g&&e<this.replacer.length;e+=2)g=g.replace(this.replacer[e],this.replacer[e+1]);this.cache&&
