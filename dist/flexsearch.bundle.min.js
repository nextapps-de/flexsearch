...
function eb(a){let b=0;if(a.constructor===Array)for(let c=0,d;c<a.length;c++)(d=a[c])&&(b+=d.length);else for(const c of a){const d=c[0],e=eb(c[1]);e?b+=e:a.delete(d)}return b}v.cleanup=function(){if(!this.fastupdate)return this;eb(this.map);this.depth&&eb(this.ctx);return this};v.searchCache=Ja;
...
v.close=function(){this.db.close();this.db=null};v.destroy=function(){return fb.deleteDatabase(this.id+(this.field?":"+this.field:""))};v.clear=function(){const a=this.db.transaction(gb,"readwrite");for(let b=0;b<gb.length;b++)a.objectStore(gb[b]).clear();return Z(a)};
...
function Z(a){return new Promise((b,c)=>{a.onsuccess=function(){b(this.result)};a.oncomplete=function(){b(this.result)};a.onerror=c;a=null})};const kb={Index:L,Charset:Sa,Encoder:K,Document:T,Worker:N,Resolver:X,IndexedDB:ib,Language:{}},lb=self;let mb;(mb=lb.define)&&mb.amd?mb([],function(){return kb}):"object"===typeof lb.exports?lb.exports=kb:lb.FlexSearch=kb;}(this||self));
