...
d.length;M++){let G=d[M];if(G&&G.length)x++,r.push(G);else if(!t)return e}}else for(let M=0,G,Za;M<n.length;M+=2){G=this.tag.get(n[M]);if(!G)if(t)continue;else return e;if(Za=(G=G&&G.get(n[M+1]))&&G.length)x++,r.push(G);else if(!t)return e}if(x){v=Ba(v,r);J=v.length;if(!J&&!t)return e;x--}}if(J)f[m]=I,e.push(v),m++;else if(1===k.length)return e}if(p){if(this.db&&n&&n.length&&!A)for(q=0;q<n.length;q+=2){d=this.index.get(n[q]);if(!d)if(t)continue;else return e;p.push(d.db.tag(n[q+1],b,l,!1))}const w=
...
(db=cb.define)&&db.amd?db([],function(){return bb}):"object"===typeof cb.exports?cb.exports=bb:cb.FlexSearch=bb;}(this));
