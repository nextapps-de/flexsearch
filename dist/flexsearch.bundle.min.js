...
t.open=function(){let a=this;navigator.storage&&navigator.storage.persist();return this.db||(this.db=new Promise(function(c,b){Y[a.id]||(Y[a.id]=[]);Y[a.id].push(a.field);const e=pb.open(a.id,1);e.onupgradeneeded=function(){const d=a.db=this.result;for(let f=0,g;f<qb.length;f++){g=qb[f];for(let k=0,h;k<Y[a.id].length;k++)h=Y[a.id][k],d.objectStoreNames.contains(g+("reg"!==g?h?":"+h:"":""))||d.createObjectStore(g+("reg"!==g?h?":"+h:"":""))}};e.onblocked=function(d){console.error("blocked",d);b()};
e.onerror=function(d){console.error(this.error,d);b()};e.onsuccess=function(){a.db=this.result;a.db.onversionchange=function(){a.close()};c(a)}}))};t.close=function(){this.db&&this.db.close();this.db=null};t.destroy=function(){const a=pb.deleteDatabase(this.id);return Z(a)};
t.clear=function(){const a=[];for(let b=0,e;b<qb.length;b++){e=qb[b];for(let d=0,f;d<Y[this.id].length;d++)f=Y[this.id][d],a.push(e+("reg"!==e?f?":"+f:"":""))}const c=this.db.transaction(a,"readwrite");for(let b=0;b<a.length;b++)c.objectStore(a[b]).clear();return Z(c)};
...
tb(b,a,!0)}}),this.transaction("reg","readwrite",function(c){for(let b=0;b<a.length;b++)c.delete(a[b])})])};function Z(a){return new Promise((c,b)=>{a.onsuccess=function(){c(this.result)};a.oncomplete=function(){c()};a.onerror=b;a=null})};const ub={Index:N,Charset:eb,Encoder:J,Document:U,Worker:P,Resolver:W,IndexedDB:sb,Language:{}},vb="undefined"!==typeof global?global:self;let wb;(wb=vb.define)&&wb.amd?wb([],function(){return ub}):"object"===typeof vb.exports?vb.exports=ub:vb.FlexSearch=ub;}(this||self));
