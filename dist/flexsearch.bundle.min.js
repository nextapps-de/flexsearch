...
(this.db&&(x.tag=p,F=y.db.support_tag_search,x.field=h),F||(x.enrich=!1)),v){v[z]=y.search(a,b,x);x&&u&&(x.enrich=u);continue}else w=y.search(a,b,x),x&&u&&(x.enrich=u);G=w&&(r?w.length:w.result.length);if(p&&G){x=[];y=0;if(this.db&&e){if(!F)for(let H=h.length;H<e.length;H++){let K=e[H];if(K&&K.length)y++,x.push(K);else if(!q)return r?d:new W(d)}}else for(let H=0,K,jb;H<p.length;H+=2){K=this.tag.get(p[H]);if(!K)if(q)continue;else return r?d:new W(d);if(jb=(K=K&&K.get(p[H+1]))&&K.length)y++,x.push(K);
...
function gb(a){let b=0;if(a.constructor===Array)for(let c=0,e;c<a.length;c++)(e=a[c])&&(b+=e.length);else for(const c of a){const e=c[0],d=gb(c[1]);d?b+=d:a.delete(e)}return b}t.cleanup=function(){if(!this.fastupdate)return this;gb(this.map);this.depth&&gb(this.ctx);return this};t.searchCache=Pa;
...
t.open=function(){let a=this;navigator.storage&&navigator.storage.persist();return this.db||new Promise(function(b,c){const e=hb.open(a.id+(a.field?":"+a.field:""),1);e.onupgradeneeded=function(){const d=a.db=this.result;ib.forEach(f=>{d.objectStoreNames.contains(f)||d.createObjectStore(f)})};e.onblocked=function(d){console.error("blocked",d);c()};e.onerror=function(d){console.error(this.error,d);c()};e.onsuccess=function(){a.db=this.result;a.db.onversionchange=function(){a.close()};b(a)}})};
t.close=function(){this.db.close();this.db=null};t.destroy=function(){const a=hb.deleteDatabase(this.id+(this.field?":"+this.field:""));return Z(a)};t.clear=function(){const a=this.db.transaction(ib,"readwrite");for(let b=0;b<ib.length;b++)a.objectStore(ib[b]).clear();return Z(a)};
