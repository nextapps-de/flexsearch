...
m[0];g=m[1];for(const n of g.entries()){g=n[0];h=n[1];k="";for(let q=0,r;q<h.length;q++){r=h[q]||[""];l="";for(let p=0;p<r.length;p++)l+=(l?",":"")+("string"===c?'"'+r[p]+'"':r[p]);l="["+l+"]";k+=(k?",":"")+l}k='new Map([["'+g+'",['+k+"]]])";k='["'+f+'",'+k+"]";e+=(e?",":"")+k}}e="index.ctx=new Map(["+e+"]);";return a?"function inject(index){"+b+d+e+"}":b+d+e};Ba(O.prototype);async function Na(a){a=a.data;var b=self._index;const c=a.args;var d=a.task;switch(d){case "init":d=a.options||{};(b=d.config)&&(d=(await import(b))["default"]);(b=a.factory)?(Function("return "+b)()(self),self._index=new self.FlexSearch.Index(d),delete self.FlexSearch):self._index=new O(d);postMessage({id:a.id});break;default:a=a.id,b=b[d].apply(b,c),postMessage("search"===d?{id:a,msg:b}:{id:a})}};let Oa=0;
function W(a={}){function b(g){function h(k){k=k.data||k;const l=k.id,m=l&&e.h[l];m&&(m(k.msg),delete e.h[l])}this.worker=g;this.h=z();if(this.worker){d?this.worker.on("message",h):this.worker.onmessage=h;if(a.config)return new Promise(function(k){e.h[++Oa]=function(){k(e)};e.worker.postMessage({id:Oa,task:"init",factory:c,options:a})});this.worker.postMessage({task:"init",factory:c,options:a});return this}}if(!this)return new W(a);let c="undefined"!==typeof self?self._factory:"undefined"!==typeof window?
window._factory:null;c&&(c=c.toString());const d="undefined"===typeof window,e=this,f=Pa(c,d,a.worker);f.worker=!0;return f.then?f.then(function(g){return b.call(e,g)}):b.call(this,f)}X("add");X("append");X("search");X("update");X("remove");
...
function Wa(a,b){const c=new Map;let d=b.index||b.field||b;B(d)&&(d=[d]);for(let e=0,f,g;e<d.length;e++){f=d[e];B(f)||(g=f,f=f.field);g=C(g)?Object.assign({},a,g):a;if(this.worker){const h=new W(g);c.set(f,h)}this.worker||c.set(f,new O(g,this.reg));g.custom?this.G[e]=g.custom:(this.G[e]=Va(f,this.K),g.filter&&("string"===typeof this.G[e]&&(this.G[e]=new String(this.G[e])),this.G[e].I=g.filter));this.field[e]=f}if(this.C){a=b.store;B(a)&&(a=[a]);for(let e=0,f,g;e<a.length;e++)f=a[e],g=f.field||f,f.custom?
