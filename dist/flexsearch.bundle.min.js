...
K);else continue;(K-=n.length)||(K=-1);D[w]=K}else{A[w]=1;continue}if(E+n.length+1<=h)n+=" ",B[w]=n+B[w];else if(l)S=n.length+1-(h-E),0<=S&&S<n.length&&(n=n.substring(S)+" ",B[w]=n+B[w]),A[w]=1;else{A[w]=1;continue}}else{n=x[I].match;p&&(D[w]=p);r&&(C[w]=r);w&&E++;let Ma;I?!w&&q&&(E+=q):(z[w]=1,A[w]=1);I>=y.length-1?Ma=1:I<x.length-1&&x[I+1].match?Ma=1:q&&(E+=q);E-=f.length-2;if(!w||E+n.length<=h)B[w]=n;else{S=V=ba=z[w]=0;break}Ma&&(z[w+1]=1,A[w+1]=1)}E+=n.length;S=J[I]=1}if(S)H===M?M++:H++;else{H===
...
v.serialize=function(a=!0){let b="",c="",e="";if(this.reg.size){let f;for(var d of this.reg.keys())f||(f=typeof d),b+=(b?",":"")+("string"===f?'"'+d+'"':d);b="index.reg=new Set(["+b+"]);";c=Ka(this.map,f);c="index.map=new Map(["+c+"]);";for(const g of this.ctx.entries()){d=g[0];let k=Ka(g[1],f);k="new Map(["+k+"])";k='["'+d+'",'+k+"]";e+=(e?",":"")+k}e="index.ctx=new Map(["+e+"]);"}return a?"function inject(index){"+b+c+e+"}":b+c+e};sa(O.prototype);const Bb="undefined"!==typeof window&&(window.indexedDB||window.mozIndexedDB||window.webkitIndexedDB||window.msIndexedDB),Cb=["map","ctx","tag","reg","cfg"],Db=G();
