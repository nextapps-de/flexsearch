...
"module/worker/worker.js"),{type:"module"})};Na.prototype.add=function(a,b,c){ba(a)&&(b=a,a=ea(b,this.key));if(b&&(a||0===a)){if(!c&&this.reg.has(a))return this.update(a,b);for(let h=0,k;h<this.field.length;h++){k=this.G[h];var e=this.index.get(this.field[h]);if("function"===typeof k){var d=k(b);d&&e.add(a,d,!1,!0)}else if(d=k.J,!d||d(b))k.constructor===String?k=[""+k]:M(k)&&(k=[k]),Oa(b,k,this.K,0,e,a,k[0],c)}if(this.tag)for(e=0;e<this.F.length;e++){var f=this.F[e];d=this.tag.get(this.P[e]);let h=I();if("function"===typeof f){if(f=f(b),!f)continue}else{var g=
...
L);else continue;(L-=n.length)||(L=-1);E[x]=L}else{A[x]=1;continue}if(C+n.length+1<=k)n+=" ",B[x]=n+B[x];else if(l)U=n.length+1-(k-C),0<=U&&U<n.length&&(n=n.substring(U)+" ",B[x]=n+B[x]),A[x]=1;else{A[x]=1;continue}}else{n=v[J].match;r&&(E[x]=r);u&&(z[x]=u);x&&C++;let Qa;J?!x&&t&&(C+=t):(G[x]=1,A[x]=1);J>=y.length-1?Qa=1:J<v.length-1&&v[J+1].match?Qa=1:t&&(C+=t);C-=f.length-2;if(!x||C+n.length<=k)B[x]=n;else{U=N=O=G[x]=0;break}Qa&&(G[x+1]=1,A[x+1]=1)}C+=n.length;U=D[J]=1}if(U)K===F?F++:K++;else{K===
...
{field:f[y],result:p}}if(n&&this.db&&v.length){const E=this;return Promise.all(v).then(function(z){for(let C=0;C<z.length;C++)d[C].result=z[C];u&&(d=cb(a,d,E.index,g,u));return h?fb(d):d})}u&&(d=cb(a,d,this.index,g,u));return h?fb(d):d};
function fb(a){const b=[],c=I(),e=I();for(let d=0,f,g,h,k,l,m,q;d<a.length;d++){f=a[d];g=f.field;h=f.result;for(let t=0;t<h.length;t++)if(l=h[t],"object"!==typeof l?l={id:k=l}:k=l.id,(m=c[k])?m.push(g):(l.field=c[k]=[g],b.push(l)),q=l.highlight)m=e[k],m||(e[k]=m={},l.highlight=m),m[g]=q}return b}function eb(a,b,c,e,d){a=this.tag.get(a);if(!a)return[];a=a.get(b);if(!a)return[];b=a.length-e;if(0<b){if(c&&b>c||e)a=a.slice(e,e+c);d&&(a=Ta.call(this,a))}return a}
