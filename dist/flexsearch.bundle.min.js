...
["\u0477","\u0475"],["\u04c2","\u0436"],["\u04d1","\u0430"],["\u04d3","\u0430"],["\u04d7","\u0435"],["\u04db","\u04d9"],["\u04dd","\u0436"],["\u04df","\u0437"],["\u04e3","\u0438"],["\u04e5","\u0438"],["\u04e7","\u043e"],["\u04eb","\u04e9"],["\u04ed","\u044d"],["\u04ef","\u0443"],["\u04f1","\u0443"],["\u04f3","\u0443"],["\u04f5","\u0447"]];const ea=/[^\p{L}\p{N}]+/u,fa=/(\d{3})/g,ha=/(\D)(\d{3})/g,ia=/(\d{3})(\D)/g,ja="".normalize&&/[\u0300-\u036f]/g;function J(a){if(!this||this.constructor!==J)return new J(...arguments);for(let b=0;b<arguments.length;b++)this.assign(arguments[b])}t=J.prototype;
...
function Fa(a,b,c,e,d){const f=[],g=B();let k;var h=a.length;let l;if(e)for(d=h-1;0<=d;d--){if(l=(e=a[d])&&e.length)for(h=0;h<l;h++)if(k=e[h],!g[k])if(g[k]=1,c)c--;else if(f.push(k),f.length===b)return f}else for(let n=h-1,m,q=0;0<=n;n--){m=a[n];for(let p=0;p<m.length;p++)if(l=(e=m[p])&&e.length)for(let r=0;r<l;r++)if(k=e[r],!g[k])if(g[k]=1,c)c--;else{let u=(p+(n<h-1?d||0:0))/(n+1)|0;(f[u]||(f[u]=[])).push(k);if(++q===b)return f}}return f}
...
a.push(d);if(a.length){const d=this;return Promise.all(a).then(function(f){let g=0;for(const k of d.index.entries()){const h=k[0];k[1].then&&d.index.set(h,f[g++])}return d})}}else a.db&&(this.fastupdate=!1,this.mount(a.db))}t=V.prototype;
...
t.set=function(a,b){this.store.set(a,b);return this};t.searchCache=Ua;
...
function mb(a){let b=0;if(a.constructor===Array)for(let c=0,e;c<a.length;c++)(e=a[c])&&(b+=e.length);else for(const c of a){const e=c[0],d=mb(c[1]);d?b+=d:a.delete(e)}return b}t.cleanup=function(){if(!this.fastupdate)return this;mb(this.map);this.depth&&mb(this.ctx);return this};t.searchCache=Ua;
...
function qb(a,b={}){if(!this)return new qb(a,b);"object"===typeof a&&(b=a,a=a.name);a||console.info("Default storage space was used, because a name was not passed.");this.id="flexsearch"+(a?":"+a.toLowerCase().replace(/[^a-z0-9_\-]/g,""):"");this.field=b.field?b.field.toLowerCase().replace(/[^a-z0-9_\-]/g,""):"";this.type=b.type;this.fastupdate=this.support_tag_search=!1;this.db=null;this.h={}}t=qb.prototype;t.mount=function(a){if(!a.encoder)return a.mount(this);a.db=this;return this.open()};
...
t.transaction=function(a,b,c){let e=this.h[a+":"+b];if(e)return c.call(this,e);let d=this.db.transaction(a,b);this.h[a+":"+b]=e=d.objectStore(a);return new Promise((f,g)=>{d.onerror=h=>{d.abort();d=e=null;g(h)};d.oncomplete=h=>{d=e=null;f(h||!0)};const k=c.call(this,e);this.h[a+":"+b]=null;return k})};
...
0;m<u.length;m++)r.push(u[m]);else n[p]=u;m=1}}else n=l,m=1;m&&e.put(n,f+":"+h)})}}}),a.store?await this.transaction("reg","readwrite",function(e){for(const d of a.store){const f=d[0],g=d[1];e.put("object"===typeof g?JSON.stringify(g):1,f)}}):a.bypass||await this.transaction("reg","readwrite",function(e){for(const d of a.reg.keys())e.put(1,d)}),a.tag&&await this.transaction("tag","readwrite",function(e){for(const d of a.tag){const f=d[0],g=d[1];g.length&&(e.get(f).onsuccess=function(){let k=this.result;
