...
[];x=0;if(this.db&&d){if(!F)for(let L=k.length;L<d.length;L++){let G=d[L];if(G&&G.length)x++,v.push(G);else if(!t)return e}}else for(let L=0,G,bb;L<p.length;L+=2){G=this.tag.get(p[L]);if(!G)if(t)continue;else return e;if(bb=(G=G&&G.get(p[L+1]))&&G.length)x++,v.push(G);else if(!t)return e}if(x){w=wa(w,v);J=w.length;if(!J&&!t)return e;x--}}if(J)f[m]=B,e.push(w),m++;else if(1===k.length)return e}if(r){if(this.db&&p&&p.length&&!F)for(q=0;q<p.length;q+=2){d=this.index.get(p[q]);if(!d)if(t)continue;else return e;
...
function Za(a){let b=0;if(a.constructor===Array)for(let c=0,d;c<a.length;c++)(d=a[c])&&(b+=d.length);else for(const c of a){const d=c[0],e=Za(c[1]);e?b+=e:a.delete(d)}return b}u.cleanup=function(){if(!this.fastupdate)return this;Za(this.map);this.depth&&Za(this.ctx);return this};u.searchCache=Da;
