...
["\u0477","\u0475"],["\u04c2","\u0436"],["\u04d1","\u0430"],["\u04d3","\u0430"],["\u04d7","\u0435"],["\u04db","\u04d9"],["\u04dd","\u0436"],["\u04df","\u0437"],["\u04e3","\u0438"],["\u04e5","\u0438"],["\u04e7","\u043e"],["\u04eb","\u04e9"],["\u04ed","\u044d"],["\u04ef","\u0443"],["\u04f1","\u0443"],["\u04f3","\u0443"],["\u04f5","\u0447"]];const da=/[^\p{L}\p{N}]+/u,ea=/(\d{3})/g,fa=/(\D)(\d{3})/g,ha=/(\d{3})(\D)/g,ia="".normalize&&/[\u0300-\u036f]/g;function K(a){if(!this||this.constructor!==K)return new K(...arguments);for(let b=0;b<arguments.length;b++)this.assign(arguments[b])}
K.prototype.assign=function(a){this.normalize=z(a.normalize,!0,this.normalize);let b=a.include,c=b||a.exclude||a.split;if("object"===typeof c){let d=!b,e="";a.include||(e+="\\p{Z}");c.letter&&(e+="\\p{L}");c.number&&(e+="\\p{N}",d=!!b);c.symbol&&(e+="\\p{S}");c.punctuation&&(e+="\\p{P}");c.control&&(e+="\\p{C}");if(c=c.char)e+="object"===typeof c?c.join(""):c;try{this.split=new RegExp("["+(b?"^":"")+e+"]+","u")}catch(f){this.split=/\s+/}this.numeric=d}else{try{this.split=z(c,da,this.split)}catch(d){this.split=
/\s+/}this.numeric=z(this.numeric,!0)}this.prepare=z(a.prepare,null,this.prepare);this.finalize=z(a.finalize,null,this.finalize);ia||(this.mapper=new Map(ca));this.rtl=a.rtl||!1;this.dedupe=z(a.dedupe,!0,this.dedupe);this.filter=z((c=a.filter)&&new Set(c),null,this.filter);this.matcher=z((c=a.matcher)&&new Map(c),null,this.matcher);this.mapper=z((c=a.mapper)&&new Map(c),null,this.mapper);this.stemmer=z((c=a.stemmer)&&new Map(c),null,this.stemmer);this.replacer=z(a.replacer,null,this.replacer);this.minlength=
z(a.minlength,1,this.minlength);this.maxlength=z(a.maxlength,0,this.maxlength);if(this.cache=c=z(a.cache,!0,this.cache))this.Z=null,this.la="number"===typeof c?c:2E5,this.R=new Map,this.W=new Map,this.D=this.h=128;this.H="";this.da=null;this.$="";this.ea=null;if(this.matcher)for(const d of this.matcher.keys())this.H+=(this.H?"|":"")+d;if(this.stemmer)for(const d of this.stemmer.keys())this.$+=(this.$?"|":"")+d;return this};
...
null;c&&(c=c.toString());const d="undefined"===typeof window,e=this,f=ma(c,d,a.F);return f.then?f.then(function(g){return b.call(e,g)}):b.call(this,f)}N("add");N("append");N("search");N("update");N("remove");
function N(a){M.prototype[a]=M.prototype[a+"Async"]=async function(){const b=this,c=[].slice.call(arguments);var d=c[c.length-1];let e;"function"===typeof d&&(e=d,c.splice(c.length-1,1));d=new Promise(function(f){b.h[++la]=f;b.F.postMessage({task:a,id:la,args:c})});return e?(d.then(e),this):d}}
function ma(a,b,c){return b?"undefined"!==typeof module?(0,eval)('new (require("worker_threads")["Worker"])(__dirname + "/node/node.js")'):(0,eval)('import("worker_threads").then(function(worker){ return new worker["Worker"]((1,eval)("import.meta.dirname") + "/node/node.mjs"); })'):a?new window.Worker(URL.createObjectURL(new Blob(["onmessage="+ka.toString()],{type:"text/javascript"}))):new window.Worker(G(c)?c:(0,eval)("import.meta.url").replace("/worker.js","/worker/worker.js").replace("flexsearch.bundle.module.min.js",
...
f.V;if(k&&!k(b))continue;f.constructor===String&&(f=""+f);f=J(b,f)}if(e&&f){G(f)&&(f=[f]);for(let k=0,l,m;k<f.length;k++)if(l=f[k],!h[l]&&(h[l]=1,(g=e.get(l))?m=g:e.set(l,m=[]),!c||!m.includes(a))){if(m.length===2**31-1){g=new P(m);if(this.C)for(let r of this.B.values())r.includes(m)&&(r[r.indexOf(m)]=g);e.set(l,m=g)}m.push(a);this.C&&((g=this.B.get(a))?g.push(m):this.B.set(a,[m]))}}else e||console.warn("Tag '"+g+"' was not found")}if(this.store&&(!c||!this.store.has(a))){let h;if(this.M){h=B();for(let k=
...
"' will be skipped because there is no field '"+q[n]+"'.");continue}t.push(d=d.db.tag(q[n+1],b,l,p))}else d=za.call(this,q[n],q[n+1],b,l,p);e.push({field:q[n],tag:q[n+1],result:d})}return t.length?Promise.all(t).then(function(x){for(let v=0;v<x.length;v++)e[v].result=x[v];return e}):e}}G(k)&&(k=[k])}k||(k=this.A);q=!d&&(this.F||this.db)&&[];let D;for(let x=0,v,A,I;x<k.length;x++){A=k[x];if(this.db&&this.tag&&!this.N[x])continue;let C;G(A)||(C=A,A=C.A,a=C.query||a,b=C.J||b,l=C.offset||l,t=C.ka||t,
p=this.store&&(C.I||p));if(d)v=d[x];else if(u=C||c,w=this.index.get(A),n&&(this.db&&(u.tag=n,D=w.db.Ea,u.A=k),D||(u.I=!1)),q){q[x]=w.search(a,b,u);u&&p&&(u.I=p);continue}else v=w.search(a,b,u),u&&p&&(u.I=p);I=v&&v.length;if(n&&I){u=[];w=0;if(this.db&&d){if(!D)for(let E=k.length;E<d.length;E++){let F=d[E];if(F&&F.length)w++,u.push(F);else if(!t)return e}}else for(let E=0,F,Za;E<n.length;E+=2){F=this.tag.get(n[E]);if(!F)if(console.warn("Tag '"+n[E]+":"+n[E+1]+"' will be skipped because there is no field '"+
n[E]+"'."),t)continue;else return e;if(Za=(F=F&&F.get(n[E+1]))&&F.length)w++,u.push(F);else if(!t)return e}if(w){v=ya(v,u);I=v.length;if(!I&&!t)return e;w--}}if(I)f[m]=A,e.push(v),m++;else if(1===k.length)return e}if(q){if(this.db&&n&&n.length&&!D)for(p=0;p<n.length;p+=2){d=this.index.get(n[p]);if(!d)if(console.warn("Tag '"+n[p]+":"+n[p+1]+"' was not found because there is no field '"+n[p]+"'."),t)continue;else return e;q.push(d.db.tag(n[p+1],b,l,!1))}const x=this;return Promise.all(q).then(function(v){return v.length?
...
d[h]=l;l.then&&c.push(l)}if(c.length)return Promise.all(c).then(function(){a.result=$a.call(a,d,e,f,g);return g?a.result:a});d.length&&(this.result=$a.call(this,d,e,f,g));return g?this.result:this};
...
L.prototype.search=function(a,b,c){c||(!b&&H(a)?(c=a,a=""):H(b)&&(c=b,b=0));let d=[],e;let f,g=0,h,k,l,m;if(c){a=c.query||a;b=c.J||b;g=c.offset||0;var r=c.context;f=c.ka;(h=ab&&!1!==c.resolve)||(ab=0);k=h&&c.I;m=c.ma;l=this.db&&c.tag}else h=this.resolve||ab;a=this.U.encode(a);e=a.length;b||!h||(b=100);if(1===e)return bb.call(this,a[0],"",b,g,h,k,l);r=this.depth&&!1!==r;if(2===e&&r&&!f)return bb.call(this,a[0],a[1],b,g,h,k,l);let n=c=0;if(1<e){const q=B(),u=[];for(let w=0,D;w<e;w++)if((D=a[w])&&!q[D]){if(f||
...
stemmer:new Map([["ational",""],["iveness",""],["fulness",""],["ousness",""],["ization",""],["tional",""],["biliti",""],["icate",""],["ative",""],["alize",""],["iciti",""],["entli",""],["ousli",""],["alism",""],["ation",""],["aliti",""],["iviti",""],["ement",""],["izer",""],["able",""],["alli",""],["ator",""],["logi",""],["ical",""],["ance",""],["ence",""],["ness",""],["ble",""],["ment",""],["eli",""],["bli",""],["ful",""],["ant",""],["ent",""],["ism",""],["ate",""],["iti",""],["ous",""],["ive",""],
["ize",""],["ing",""],["ion",""],["al",""],["ou",""],["er",""],["ic",""],["ly",""]])};"undefined"!==typeof module&&module.exports?module.exports=eb:self.FlexSearch&&(self.FlexSearch.Language.en=eb);}(this||self));
