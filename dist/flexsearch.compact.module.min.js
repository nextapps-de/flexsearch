...
function xa(a,c){c||(c=new Map);for(let b=0,e,d;b<a.length;b++)e=a[b],d=c.get(e[0]),c.set(e[0],ta(e[1],d));return c}function ya(a){let c=[],b=[];for(const e of a.keys())b.push(e),25E4===b.length&&(c.push(b),b=[]);b.length&&c.push(b);return c}function za(a,c){c||(c=new Set);for(let b=0;b<a.length;b++)c.add(a[b]);return c}
...
function Pa(a,c,b,e){const d=[];a=new Set(a.flat().flat());for(let f=0,g,k=0;f<this.result.length;f++)if(g=this.result[f])for(let h=0,l;h<g.length;h++)if(l=g[h],!a.has(l))if(b)b--;else if(e){if(d.push(l),d.length===c)return d}else if(d[f]||(d[f]=[]),d[f].push(l),++k===c)return d;return d};function X(a){if(!this||this.constructor!==X)return new X(a);if(a&&a.index)return a.resolve=!1,this.index=a.index,this.h=a.boost||0,this.result=a.index.search(a).result,this;this.index=null;this.result=a||[];this.h=0}X.prototype.limit=function(a){if(this.result.length){const c=[];for(let b=0,e;b<this.result.length;b++)if(e=this.result[b])if(e.length<=a){if(c[b]=e,a-=e.length,!a)break}else{c[b]=e.slice(0,a);break}this.result=c}return this};
X.prototype.offset=function(a){if(this.result.length){const c=[];for(let b=0,e;b<this.result.length;b++)if(e=this.result[b])e.length<=a?a-=e.length:(c[b]=e.slice(a),a=0);this.result=c}return this};X.prototype.boost=function(a){this.h+=a;return this};X.prototype.resolve=function(a,c,b){const e=this.result,d=this.index;this.result=this.index=null;return e.length?("object"===typeof a&&(b=a.enrich,c=a.offset,a=a.limit),Ha.call(d,e,a||100,c,b)):e};I();function Qa(a,c,b,e,d){let f,g,k;"string"===typeof d?(f=d,d=""):f=d.template;g=f.indexOf("$1");k=f.substring(g+2);g=f.substring(0,g);let h=d&&d.boundary,l=!d||!1!==d.clip,m=d&&d.merge&&k&&g&&new RegExp(k+" "+g,"g");d=d&&d.ellipsis;var p=0;if("object"===typeof d){var r=d.template;p=r.length-2;d=d.pattern}"string"!==typeof d&&(d=!1===d?"":"...");p&&(d=r.replace("$1",d));r=d.length-p;let q,t;"object"===typeof h&&(q=h.before,0===q&&(q=-1),t=h.after,0===t&&(t=-1),h=h.total||9E5);p=new Map;for(let va=0,
V,Oa,ca;va<c.length;va++){let da;if(e)da=c,ca=e;else{var w=c[va];ca=w.field;if(!ca)continue;da=w.result}Oa=b.get(ca);V=Oa.encoder;w=p.get(V);"string"!==typeof w&&(w=V.encode(a),p.set(V,w));for(let la=0;la<da.length;la++){var n=da[la].doc;if(!n)continue;n=Q(n,ca);if(!n)continue;var x=n.trim().split(/\s+/);if(!x.length)continue;n="";var y=[];let ma=[];var E=-1,v=-1,B=0;for(var z=0;z<x.length;z++){var C=x[z],K=V.encode(C);K=1<K.length?K.join(" "):K[0];let u;if(K&&C){var D=C.length,M=(V.split?C.replace(V.split,
...
G);else continue;(G-=n.length)||(G=-1);C[u]=G}else{z[u]=1;continue}if(D+n.length+1<=h)n+=" ",v[u]=n+v[u];else if(l)P=n.length+1-(h-D),0<=P&&P<n.length&&(n=n.substring(P)+" ",v[u]=n+v[u]),z[u]=1;else{z[u]=1;continue}}else{n=y[F].match;q&&(C[u]=q);t&&(K[u]=t);u&&D++;let wa;F?!u&&r&&(D+=r):(B[u]=1,z[u]=1);F>=x.length-1?wa=1:F<y.length-1&&y[F+1].match?wa=1:r&&(D+=r);D-=f.length-2;if(!u||D+n.length<=h)v[u]=n;else{P=S=W=B[u]=0;break}wa&&(B[u+1]=1,z[u+1]=1)}D+=n.length;P=E[F]=1}if(P)M===L?L++:M++;else{M===
...
r?Qa(a,e,this.index,q,r):e:new X(e);d[w]={field:f[w],result:e}}return t?Sa(d):r?Qa(a,d,this.index,q,r):d};function Sa(a){const c=[],b=I();for(let e=0,d,f;e<a.length;e++){d=a[e];f=d.result;for(let g=0,k,h,l;g<f.length;g++)h=f[g],"object"!==typeof h&&(h={id:h}),k=h.id,(l=b[k])?l.push(d.field):(h.field=b[k]=[d.field],c.push(h))}return c}function Ra(a,c,b,e,d){a=this.tag.get(a);if(!a)return[];if((c=(a=a&&a.get(c))&&a.length-e)&&0<c){if(c>b||e)a=a.slice(e,e+b);d&&(a=U.call(this,a));return a}}
...
A.import=function(a,c){if(c)switch("string"===typeof c&&(c=JSON.parse(c)),a=a.split("."),"json"===a[a.length-1]&&a.pop(),3===a.length&&a.shift(),a=1<a.length?a[1]:a[0],a){case "reg":this.fastupdate=!1;this.reg=za(c,this.reg);break;case "map":this.map=ta(c,this.map);break;case "ctx":this.ctx=xa(c,this.ctx)}};
A.serialize=function(a=!0){let c="",b="",e="";if(this.reg.size){let f;for(var d of this.reg.keys())f||(f=typeof d),c+=(c?",":"")+("string"===f?'"'+d+'"':d);c="index.reg=new Set(["+c+"]);";b=Ba(this.map,f);b="index.map=new Map(["+b+"]);";for(const g of this.ctx.entries()){d=g[0];let k=Ba(g[1],f);k="new Map(["+k+"])";k='["'+d+'",'+k+"]";e+=(e?",":"")+k}e="index.ctx=new Map(["+e+"]);"}return a?"function inject(index){"+c+b+e+"}":c+b+e};ka(Z.prototype);I();export default {Index:Z,Charset:ab,Encoder:ja,Document:T,Worker:null,Resolver:null,IndexedDB:null,Language:{}};
