...
[],e=this.split||""===this.split?a.split(this.split):a;for(let f=0,g,k;f<e.length;f++)if((g=k=e[f])&&!(g.length<this.minlength))if(c)b.push(g);else if(!this.filter||("function"===typeof this.filter?this.filter(g):!this.filter.has(g))){if(this.cache&&g.length<=this.L)if(this.H){var d=this.G.get(g);if(d||""===d){d&&b.push(d);continue}}else this.H=setTimeout(K,50,this);this.stemmer&&2<g.length&&(this.N||(this.N=new RegExp("(?!^)("+this.D+")$")),d=g,g=g.replace(this.N,h=>this.stemmer.get(h)),d!==g&&this.filter&&
...
function Fa(a,c,b,e,d){const f=[],g=C();let k=0;for(let h=0,l;h<a.length;h++)if(l=a[h]){k<l.length&&(k=l.length);for(let m=0,n;m<l.length;m++)if(n=l[m])for(let u=0,p;u<n.length;u++)p=n[u],g[p]=g[p]?2:1}for(let h=0,l,m=0;h<k;h++)for(let n=0,u;n<a.length;n++)if(u=a[n])if(l=u[h])for(let p=0,q;p<l.length;p++)if(q=l[p],1===g[q])if(b)b--;else if(e){if(f.push(q),f.length===c)return f}else{const r=h+(n?d:0);f[r]||(f[r]=[]);f[r].push(q);if(++m===c)return f}return f};W.prototype.not=function(){const {O:a,P:c,limit:b,offset:e,enrich:d,resolve:f,suggest:g}=Ba(this,"not",arguments);return Ga.call(this,a,c,b,e,d,f,g)};function Ga(a,c,b,e,d,f,g){if(c.length){const k=this;return Promise.all(c).then(function(h){a=[];for(let l=0,m;l<h.length;l++)(m=h[l]).length&&(a[l]=m);return Ga.call(k,a,[],b,e,d,f,g)})}if(a.length&&this.result.length)this.result=Ha.call(this,a,b,e,f);else if(f)return this.resolve(b,e,d);return f?d?V.call(this.index,this.result):this.result:this}
...
W.prototype.offset=function(a){if(this.result.length){const c=[];for(let b=0,e;b<this.result.length;b++)if(e=this.result[b])e.length<=a?a-=e.length:(c[b]=e.slice(a),a=0);this.result=c}return this};W.prototype.boost=function(a){this.h+=a;return this};W.prototype.resolve=function(a,c,b){const e=this.result,d=this.index;this.result=this.index=null;return e.length?("object"===typeof a&&(b=a.enrich,c=a.offset,a=a.limit),Aa.call(d,e,a||100,c,b)):e};C();U.prototype.search=function(a,c,b,e){b||(!c&&G(a)?(b=a,a=""):G(c)&&(b=c,c=0));let d=[];var f=[];let g;let k,h,l;let m=0,n=!0,u;if(b){b.constructor===Array&&(b={index:b});a=b.query||a;var p=b.pluck;var q=b.merge;h=p||b.field||(h=b.index)&&(h.index?null:h);l=this.tag&&b.tag;k=b.suggest;n=!0;u=(g=this.store&&b.enrich&&n)&&b.highlight;c=b.limit||c;var r=b.offset||0;c||(c=100);if(l){l.constructor!==Array&&(l=[l]);var t=[];for(let y=0,v;y<l.length;y++)if(v=l[y],v.field&&v.tag){var x=v.tag;if(x.constructor===
Array)for(var z=0;z<x.length;z++)t.push(v.field,x[z]);else t.push(v.field,x)}else{x=Object.keys(v);for(let E=0,F,A;E<x.length;E++)if(F=x[E],A=v[F],A.constructor===Array)for(z=0;z<A.length;z++)t.push(F,A[z]);else t.push(F,A)}l=t;if(!a){f=[];if(t.length)for(p=0;p<t.length;p+=2)q=Ia.call(this,t[p],t[p+1],c,r,g),d.push({field:t[p],tag:t[p+1],result:q});return f.length?Promise.all(f).then(function(y){for(let v=0;v<y.length;v++)d[v].result=y[v];return d}):d}}h&&h.constructor!==Array&&(h=[h])}h||(h=this.field);
t=!e&&(this.worker||this.db)&&[];for(let y=0,v,E,F;y<h.length;y++){E=h[y];let A;D(E)||(A=E,E=A.field,a=A.query||a,c=A.limit||c,r=A.offset||r,k=A.suggest||k,g=this.store&&(A.enrich||g));if(e)v=e[y];else if(x=A||b,z=this.index.get(E),l&&(x.enrich=!1),t){t[y]=z.search(a,c,x);x&&g&&(x.enrich=g);continue}else v=z.search(a,c,x),x&&g&&(x.enrich=g);F=v&&(n?v.length:v.result.length);if(l&&F){x=[];z=0;for(let I=0,O,Ua;I<l.length;I+=2){O=this.tag.get(l[I]);if(!O)if(k)continue;else return n?d:new W(d);if(Ua=
(O=O&&O.get(l[I+1]))&&O.length)z++,x.push(O);else if(!k)return n?d:new W(d)}if(z){v=za(v,x);F=v.length;if(!F&&!k)return n?v:new W(v);z--}}if(F)f[m]=E,d.push(v),m++;else if(1===h.length)return n?d:new W(d)}if(t){const y=this;return Promise.all(t).then(function(v){return v.length?y.search(a,c,b,v):v})}if(!m)return n?d:new W(d);if(p&&(!g||!this.store))return d[0];t=[];for(r=0;r<f.length;r++){e=d[r];g&&e.length&&"undefined"===typeof e[0].doc&&(e=V.call(this,e));if(p)return n?e:new W(e);d[r]={field:f[r],
result:e}}return q?Ja(d,c):u?Ka(d,a,this.index,this.field,this.C,u):d};
function Ka(a,c,b,e,d,f){let g,k,h;for(let l=0,m,n,u,p;l<a.length;l++){let q=a[l].result;m=a[l].field;u=b.get(m);n=u.encoder;h=u.tokenize;p=d[e.indexOf(m)];n!==g&&(g=n,k=g.encode(c));for(let r=0;r<q.length;r++){let t="",x=H(q[r].doc,p).split(/\s+/);for(let z=0,y,v;z<x.length;z++){y=x[z];v=g.encode(y).join(" ");let E;if(v&&y)for(let F=0,A;F<k.length;F++)if(A=k[F],"strict"===h){if(v===A){t+=(t?" ":"")+f.replace("$1",y);E=!0;break}}else{const I=v.indexOf(A);if(-1<I){t+=(t?" ":"")+y.substring(0,I)+f.replace("$1",
y.substring(I,A.length))+y.substring(I+A.length);E=!0;break}}E||(t+=(t?" ":"")+x[z])}q[r].highlight=t}}return a}function Ja(a,c){const b=[],e=C();for(let d=0,f,g;d<a.length;d++){f=a[d];g=f.result;for(let k=0,h,l,m;k<g.length;k++)if(l=g[k],"object"!==typeof l&&(l={id:l}),h=l.id,m=e[h])m.push(f.field);else{if(b.length===c)return b;l.field=e[h]=[f.field];b.push(l)}}return b}
