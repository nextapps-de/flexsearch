...
" ":"")+E.substring(0,G)+f.replace("$1",E.substring(G,J.length))+E.substring(G+J.length);F=!0;break}}F||(w+=(w?" ":"")+l[y])}n[u].highlight=w}}return a}function Ja(a,c){const b=[],e=C();for(let d=0,f,g;d<a.length;d++){f=a[d];g=f.result;for(let h=0,k,l,m;h<g.length;h++)if(l=g[h],"object"!==typeof l&&(l={id:l}),k=l.id,m=e[k])m.push(f.field);else{if(b.length===c)return b;l.field=e[k]=[f.field];b.push(l)}}return b}
