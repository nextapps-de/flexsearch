...
u=!e&&(this.worker||this.db)&&[];for(let y=0,v,E,F;y<h.length;y++){E=h[y];let A;D(E)||(A=E,E=A.field,a=A.query||a,c=A.limit||c,t=A.offset||t,k=A.suggest||k,g=this.store&&(A.enrich||g));if(e)v=e[y];else if(w=A||b,z=this.index.get(E),l&&(w.enrich=!1),u){u[y]=z.search(a,c,w);w&&g&&(w.enrich=g);continue}else v=z.search(a,c,w),w&&g&&(w.enrich=g);F=v&&(n?v.length:v.result.length);if(l&&F){w=[];z=0;for(let J=0,G,Ua;J<l.length;J+=2){G=this.tag.get(l[J]);if(!G)if(k)continue;else return n?d:new X(d);if(Ua=
