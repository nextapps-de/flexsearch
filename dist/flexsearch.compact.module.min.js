...
this.K=c.resolution||1;this.rtl=d.rtl||a.rtl||!1}w=N.prototype;w.clear=function(){this.map.clear();this.B.clear();this.h.clear();return this};w.append=function(a,b){return this.add(a,b,!0)};w.contain=function(a){return this.h.has(a)};w.update=function(a,b){if(this.async){const c=this,d=this.remove(a);return d.then?d.then(()=>c.add(a,b)):this.add(a,b)}return this.remove(a).add(a,b)};
function V(a){let b=0;if(a.constructor===Array)for(let c=0,d;c<a.length;c++)(d=a[c])&&(b+=d.length);else for(const c of a){const d=c[0],e=V(c[1]);e?b+=e:a.delete(d)}return b}w.cleanup=function(){if(!this.fastupdate)return this;V(this.map);this.depth&&V(this.B);return this};
...
u)}else{u=Object.keys(q);for(let C=0,D,z;C<u.length;C++)if(D=u[C],z=q[D],z.constructor===Array)for(B=0;B<z.length;B++)m.push(D,z[B]);else m.push(D,z)}l=m;if(!a){d=[];if(m.length)for(f=0;f<m.length;f+=2)t=ta.call(this,m[f],m[f+1],b,p,g),e.push({field:m[f],tag:m[f+1],result:t});return d.length?Promise.all(d).then(function(v){for(let q=0;q<v.length;q++)e[q].result=v[q];return e}):e}}A(n)&&(n=[n])}n||(n=this.field);p=!d&&(this.worker||this.async)&&[];for(let v=0,q,C,D;v<n.length;v++){C=n[v];let z;A(C)||
(z=C,C=z.field,a=z.query||a,b=z.limit||b,k=z.suggest||k);if(d)q=d[v];else if(m=z||c,u=this.index.get(C),l&&(m.enrich=!1),p){p[v]=u.searchAsync(a,b,m);m&&g&&(m.enrich=g);continue}else q=u.search(a,b,m),m&&g&&(m.enrich=g);D=q&&q.length;if(l&&D){m=[];u=0;for(let O=0,G,va;O<l.length;O+=2){G=this.tag.get(l[O]);if(!G)if(k)continue;else return e;if(va=(G=G&&G.get(l[O+1]))&&G.length)u++,m.push(G);else if(!k)return e}if(u){q=ra(q,m);D=q.length;if(!D&&!k)return e;u--}}if(D)f[r]=C,e.push(q),r++;else if(1===
n.length)return e}if(p){const v=this;return Promise.all(p).then(function(q){return q.length?v.search(a,b,c,q):q})}if(!r)return e;if(t&&(!g||!this.store))return e[0];p=[];for(let v=0,q;v<f.length;v++){q=e[v];g&&q.length&&!q[0].doc&&q.length&&(q=ua.call(this,q));if(t)return q;e[v]={field:f[v],result:q}}return h?wa(e,b):e};
