...
["\u0477","\u0475"],["\u04c2","\u0436"],["\u04d1","\u0430"],["\u04d3","\u0430"],["\u04d7","\u0435"],["\u04db","\u04d9"],["\u04dd","\u0436"],["\u04df","\u0437"],["\u04e3","\u0438"],["\u04e5","\u0438"],["\u04e7","\u043e"],["\u04eb","\u04e9"],["\u04ed","\u044d"],["\u04ef","\u0443"],["\u04f1","\u0443"],["\u04f3","\u0443"],["\u04f5","\u0447"]];const ca=/[^\p{L}\p{N}]+/u,da=/(\d{3})/g,ea=/(\D)(\d{3})/g,fa=/(\d{3})(\D)/g,J="".normalize&&/[\u0300-\u036f]/g;function K(a){if(!this||this.constructor!==K)return new K(...arguments);for(let b=0;b<arguments.length;b++)this.assign(arguments[b])}
...
B(a.minlength,1,this.minlength);this.maxlength=B(a.maxlength,0,this.maxlength);if(this.cache=c=B(a.cache,!0,this.cache))this.I=null,this.O="number"===typeof c?c:2E5,this.D=new Map,this.G=new Map,this.J=this.A=128;this.K="";this.M=null;this.L="";this.N=null;if(this.matcher)for(const e of this.matcher.keys())this.K+=(this.K?"|":"")+e;if(this.stemmer)for(const e of this.stemmer.keys())this.L+=(this.L?"|":"")+e;return this};
...
l[w];let z;E(C)||(z=C,C=z.field,a=z.query||a,b=z.limit||b,n=z.offset||n,k=z.suggest||k,f=this.store&&(z.enrich||f));if(e)q=e[w];else if(u=z||c,y=this.index.get(C),m&&(u.enrich=!1),v){v[w]=y.search(a,b,u);u&&f&&(u.enrich=f);continue}else q=y.search(a,b,u),u&&f&&(u.enrich=f);A=q&&q.length;if(m&&A){u=[];y=0;for(let H=0,F,L;H<m.length;H+=2){F=this.tag.get(m[H]);if(!F)if(k)continue;else return d;if(L=(F=F&&F.get(m[H+1]))&&F.length)y++,u.push(F);else if(!k)return d}if(y){q=na(q,u);A=q.length;if(!A&&!k)return d;
...
new Map;this.B=[];this.R=[];for(let d=0,g,f;d<c.length;d++){g=c[d];f=g.field||g;if(!f)throw Error("The tag field from the document descriptor is undefined.");g.custom?this.B[d]=g.custom:(this.B[d]=T(f,this.H),g.filter&&("string"===typeof this.B[d]&&(this.B[d]=new String(this.B[d])),this.B[d].F=g.filter));this.R[d]=f;this.tag.set(f,new Map)}}}
function T(a,b){const c=a.split(":");let e=0;for(let d=0;d<c.length;d++)a=c[d],"]"===a[a.length-1]&&(a=a.substring(0,a.length-2))&&(b[e]=!0),a&&(c[e++]=a);e<c.length&&(c.length=e);return 1<e?c:c[0]}x=Q.prototype;x.append=function(a,b){return this.add(a,b,!0)};x.update=function(a,b){return this.remove(a).add(a,b)};
...
function Ga(a){let b=0;if(a.constructor===Array)for(let c=0,e;c<a.length;c++)(e=a[c])&&(b+=e.length);else for(const c of a){const e=c[0],d=Ga(c[1]);d?b+=d:a.delete(e)}return b}x.cleanup=function(){if(!this.fastupdate)return this;Ga(this.map);this.depth&&Ga(this.ctx);return this};x.searchCache=sa;
x.export=function(a,b,c,e=0){let d,g;switch(e){case 0:d="reg";g=la(this.reg);break;case 1:d="cfg";g={};break;case 2:d="map";g=N(this.map,this.reg.size);break;case 3:d="ctx";g=ja(this.ctx,this.reg.size);break;default:return}return P.call(this,a,b,d,g,c,e)};
x.import=function(a,b){if(b)switch(E(b)&&(b=JSON.parse(b)),a=a.split("."),"json"===a[a.length-1]&&a.pop(),a=1<a.length?a[1]:a[0],a){case "reg":this.fastupdate=!1;this.reg=ma(b,this.reg);break;case "map":this.map=O(b,this.map);break;case "ctx":this.ctx=ka(b,this.ctx)}};
