/**!
 * FlexSearch.js v0.8.209 (Bundle)
 * Author and Copyright: Thomas Wilkerling
 * Licence: Apache-2.0
 * Hosted by Nextapps GmbH
 * https://github.com/nextapps-de/flexsearch
 */
var B;function J(a,c,b){const e=typeof b,d=typeof a;if(e!=="undefined"){if(d!=="undefined"){if(b){if(d==="function"&&e===d)return function(k){return a(b(k))};c=a.constructor;if(c===b.constructor){if(c===Array)return b.concat(a);if(c===Map){var f=new Map(b);for(var g of a)f.set(g[0],g[1]);return f}if(c===Set){g=new Set(b);for(f of a.values())g.add(f);return g}}}return a}return b}return d==="undefined"?c:a}function K(a,c){return typeof a==="undefined"?c:a}function N(){return Object.create(null)}
...
V,Ha,ca;sa<c.length;sa++){let da;if(e)da=c,ca=e;else{var w=c[sa];ca=w.field;if(!ca)continue;da=w.result}Ha=b.get(ca);V=Ha.encoder;w=A.get(V);typeof w!=="string"&&(w=V.encode(a),A.set(V,w));for(let ia=0;ia<da.length;ia++){var n=da[ia].doc;if(!n)continue;n=aa(n,ca);if(!n)continue;var v=n.trim().split(/\s+/);if(!v.length)continue;n="";var r=[];let ja=[];var C=-1,z=-1,t=0;for(var y=0;y<v.length;y++){var E=v[y],D=V.encode(E);D=D.length>1?D.join(" "):D[0];let q;if(D&&E){var F=E.length,I=(V.split?E.replace(V.split,
""):E).length-D.length,L="",R=0;for(var W=0;W<w.length;W++){var M=w[W];if(M){var H=M.length;H+=I<0?0:I;R&&H<=R||(M=D.indexOf(M),M>-1&&(L=(M?E.substring(0,M):"")+g+E.substring(M,M+H)+k+(M+H<F?E.substring(M+H):""),R=H,q=!0))}}L&&(h&&(C<0&&(C=n.length+(n?1:0)),z=n.length+(n?1:0)+L.length,t+=F,ja.push(r.length),r.push({match:L})),n+=(n?" ":"")+L)}if(!q)E=v[y],n+=(n?" ":"")+E,h&&r.push({text:E});else if(h&&t>=h)break}t=ja.length*(f.length-2);if(u||p||h&&n.length-t>h)if(t=h+t-x*2,y=z-C,u>0&&(y+=u),p>0&&
(y+=p),y<=t)v=u?C-(u>0?u:0):C-((t-y)/2|0),r=p?z+(p>0?p:0):v+t,m||(v>0&&n.charAt(v)!==" "&&n.charAt(v-1)!==" "&&(v=n.indexOf(" ",v),v<0&&(v=0)),r<n.length&&n.charAt(r-1)!==" "&&n.charAt(r)!==" "&&(r=n.lastIndexOf(" ",r),r<z?r=z:++r)),n=(v?d:"")+n.substring(v,r)+(r<n.length?d:"");else{z=[];C={};t={};y={};E={};D={};L=I=F=0;for(W=R=1;;){var O=void 0;for(let q=0,G;q<ja.length;q++){G=ja[q];if(L)if(I!==L){if(y[q+1])continue;G+=L;if(C[G]){F-=x;t[q+1]=1;y[q+1]=1;continue}if(G>=r.length-1){if(G>=r.length){y[q+
1]=1;G>=v.length&&(t[q+1]=1);continue}F-=x}n=r[G].text;if(H=p&&D[q])if(H>0){if(n.length>H)if(y[q+1]=1,m)n=n.substring(0,H);else continue;(H-=n.length)||(H=-1);D[q]=H}else{y[q+1]=1;continue}if(F+n.length+1<=h)n=" "+n,z[q]+=n;else if(m)O=h-F-1,O>0&&(n=" "+n.substring(0,O),z[q]+=n),y[q+1]=1;else{y[q+1]=1;continue}}else{if(y[q])continue;G-=I;if(C[G]){F-=x;y[q]=1;t[q]=1;continue}if(G<=0){if(G<0){y[q]=1;t[q]=1;continue}F-=x}n=r[G].text;if(H=u&&E[q])if(H>0){if(n.length>H)if(y[q]=1,m)n=n.substring(n.length-
H);else continue;(H-=n.length)||(H=-1);E[q]=H}else{y[q]=1;continue}if(F+n.length+1<=h)n+=" ",z[q]=n+z[q];else if(m)O=n.length+1-(h-F),O>=0&&O<n.length&&(n=n.substring(O)+" ",z[q]=n+z[q]),y[q]=1;else{y[q]=1;continue}}else{n=r[G].match;u&&(E[q]=u);p&&(D[q]=p);q&&F++;let ta;G?!q&&x&&(F+=x):(t[q]=1,y[q]=1);G>=v.length-1?ta=1:G<r.length-1&&r[G+1].match?ta=1:x&&(F+=x);F-=f.length-2;if(!q||F+n.length<=h)z[q]=n;else{O=R=W=t[q]=0;break}ta&&(t[q+1]=1,y[q+1]=1)}F+=n.length;O=C[G]=1}if(O)I===L?L++:I++;else{I===
L?R=0:W=0;if(!R&&!W)break;R?(I++,L=I):L++}}n="";for(let q=0,G;q<z.length;q++)G=(t[q]?q?" ":"":(q&&!d?" ":"")+d)+z[q],n+=G;d&&!t[z.length]&&(n+=d)}l&&(n=n.replace(l," "));da[ia].highlight=n}if(e)break}return c};function Fa(a,c){const b=N(),e=[];for(let d=0,f;d<c.length;d++){f=c[d];for(let g=0;g<f.length;g++)b[f[g]]=1}for(let d=0,f;d<a.length;d++)f=a[d],b[f]&&(e.push(f),b[f]=0);return e};N();X.prototype.search=function(a,c,b,e){b||(!c&&Q(a)?(b=a,a=""):Q(c)&&(b=c,c=0));let d=[];var f=[];let g;let k,h,m,l;let A=0,x=!0,u;if(b){b.constructor===Array&&(b={index:b});a=b.query||a;var p=b.pluck;var w=b.merge;m=p||b.field||(m=b.index)&&(m.index?null:m);l=this.tag&&b.tag;k=b.suggest;x=!0;h=b.cache;u=x&&this.store&&b.highlight;g=!!u||x&&this.store&&b.enrich;c=b.limit||c;var n=b.offset||0;c||(c=x?100:0);if(l){l.constructor!==Array&&(l=[l]);var v=[];for(let z=0,t;z<l.length;z++)if(t=l[z],t.field&&
