 ...
 */
var B;function J(a,c,b){const e=typeof b,d=typeof a;if(e!=="undefined"){if(d!=="undefined"){if(b){if(d==="function"&&e===d)return function(h){return a(b(h))};c=a.constructor;if(c===b.constructor){if(c===Array)return b.concat(a);if(c===Map){var f=new Map(b);for(var g of a)f.set(g[0],g[1]);return f}if(c===Set){g=new Set(b);for(f of a.values())g.add(f);return g}}}return a}return b}return d==="undefined"?c:a}function K(a,c){return typeof a==="undefined"?c:a}function N(){return Object.create(null)}
...
H);else continue;(H-=n.length)||(H=-1);E[r]=H}else{y[r]=1;continue}if(F+n.length+1<=k)n+=" ",z[r]=n+z[r];else if(m)O=n.length+1-(k-F),O>=0&&O<n.length&&(n=n.substring(O)+" ",z[r]=n+z[r]),y[r]=1;else{y[r]=1;continue}}else{n=q[G].match;u&&(E[r]=u);p&&(D[r]=p);r&&F++;let ta;G?!r&&x&&(F+=x):(t[r]=1,y[r]=1);G>=v.length-1?ta=1:G<q.length-1&&q[G+1].match?ta=1:x&&(F+=x);F-=f.length-2;if(!r||F+n.length<=k)z[r]=n;else{O=R=W=t[r]=0;break}ta&&(t[r+1]=1,y[r+1]=1)}F+=n.length;O=C[G]=1}if(O)I===L?L++:I++;else{I===
...
typeof e[0].doc==="undefined"&&(e=Ia.call(this,e));if(p)return u?Ea(a,e,this.index,p,u):e;d[n]={field:f[n],result:e}}u&&(d=Ea(a,d,this.index,p,u));return w?Ja(d):d};function Ja(a){const c=[],b=N(),e=N();for(let d=0,f,g,h,k,m,l,A;d<a.length;d++){f=a[d];g=f.field;h=f.result;for(let x=0;x<h.length;x++)if(m=h[x],typeof m!=="object"?m={id:k=m}:k=m.id,(l=b[k])?l.push(g):(m.field=b[k]=[g],c.push(m)),A=m.highlight)l=e[k],l||(e[k]=l={},m.highlight=l),l[g]=A}return c}
