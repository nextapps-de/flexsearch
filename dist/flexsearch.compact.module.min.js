/**!
 * FlexSearch.js v0.8.207 (Bundle)
 * Author and Copyright: Thomas Wilkerling
 * Licence: Apache-2.0
 * Hosted by Nextapps GmbH
 * https://github.com/nextapps-de/flexsearch
 */
var B;function I(a,c,b){const e=typeof b,d=typeof a;if(e!=="undefined"){if(d!=="undefined"){if(b){if(d==="function"&&e===d)return function(k){return a(b(k))};c=a.constructor;if(c===b.constructor){if(c===Array)return b.concat(a);if(c===Map){var f=new Map(b);for(var g of a)f.set(g[0],g[1]);return f}if(c===Set){g=new Set(b);for(f of a.values())g.add(f);return g}}}return a}return b}return d==="undefined"?c:a}function K(a,c){return typeof a==="undefined"?c:a}function N(){return Object.create(null)}
...
[],d=N(),f,g,k=this.split||this.split===""?a.split(this.split):[a];for(let m=0,l,A;m<k.length;m++)if((l=A=k[m])&&!(l.length<this.minlength||l.length>this.maxlength)){if(c){if(d[l])continue;d[l]=1}else{if(f===l)continue;f=l}if(b)e.push(l);else if(!this.filter||(typeof this.filter==="function"?this.filter(l):!this.filter.has(l))){if(this.cache&&l.length<=this.H)if(this.D){var h=this.C.get(l);if(h||h===""){h&&e.push(h);continue}}else this.D=setTimeout(S,50,this);if(this.stemmer){this.J||(this.J=new RegExp("(?!^)("+
this.A+")$"));let x;for(;x!==l&&l.length>2;)x=l,l=l.replace(this.J,u=>this.stemmer.get(u))}if(l&&(this.mapper||this.dedupe&&l.length>1)){h="";for(let x=0,u="",p,w;x<l.length;x++)p=l.charAt(x),p===u&&this.dedupe||((w=this.mapper&&this.mapper.get(p))||w===""?w===u&&this.dedupe||!(u=w)||(h+=w):h+=u=p);l=h}this.matcher&&l.length>1&&(this.I||(this.I=new RegExp("("+this.h+")","g")),l=l.replace(this.I,x=>this.matcher.get(x)));if(l&&this.replacer)for(h=0;l&&h<this.replacer.length;h+=2)l=l.replace(this.replacer[h],
this.replacer[h+1]);this.cache&&A.length<=this.H&&(this.C.set(A,l),this.C.size>this.K&&(this.C.clear(),this.H=this.H/1.1|0));if(l){if(l!==A)if(c){if(d[l])continue;d[l]=1}else{if(g===l)continue;g=l}e.push(l)}}}this.finalize&&(e=this.finalize(e)||e);this.cache&&a.length<=this.G&&(this.B.set(a,e),this.B.size>this.K&&(this.B.clear(),this.G=this.G/1.1|0));return e};function S(a){a.D=null;a.B.clear();a.C.clear()};function ma(a,c,b){b||(c||typeof a!=="object"?typeof c==="object"&&(b=c,c=0):b=a);b&&(a=b.query||a,c=b.limit||c);let e=""+(c||0);b&&(e+=(b.offset||0)+!!b.context+!!b.suggest+(b.resolve!==!1)+(b.resolution||this.resolution)+(b.boost||0));a=(""+a).toLowerCase();this.cache||(this.cache=new T);let d=this.cache.get(a+e);if(!d){const f=b&&b.cache;f&&(b.cache=!1);d=this.search(a,c,b);f&&(b.cache=f);this.cache.set(a+e,d)}return d}function T(a){this.limit=a&&a!==!0?a:1E3;this.cache=new Map;this.h=""}
...
function U(a){this[a+"Async"]=function(){const c=arguments;var b=c[c.length-1];let e;typeof b==="function"&&(e=b,delete c[c.length-1]);xa?za||(za=Date.now()-ya>=this.priority*this.priority*3):(xa=setTimeout(Aa,0),ya=Date.now());if(za){const f=this;return new Promise(g=>{setTimeout(function(){g(f[a+"Async"].apply(f,c))},0)})}const d=this[a].apply(this,c);b=d.then?d:new Promise(f=>f(d));e&&b.then(e);return b}};X.prototype.add=function(a,c,b){Q(a)&&(c=a,a=aa(c,this.key));if(c&&(a||a===0)){if(!b&&this.reg.has(a))return this.update(a,c);for(let k=0,h;k<this.field.length;k++){h=this.B[k];var e=this.index.get(this.field[k]);if(typeof h==="function"){var d=h(c);d&&e.add(a,d,b,!0)}else if(d=h.F,!d||d(c))h.constructor===String?h=[""+h]:P(h)&&(h=[h]),Ba(c,h,this.C,0,e,a,h[0],b)}if(this.tag)for(e=0;e<this.A.length;e++){var f=this.A[e];d=this.tag.get(this.D[e]);let k=N();if(typeof f==="function"){if(f=f(c),!f)continue}else{var g=
f.F;if(g&&!g(c))continue;f.constructor===String&&(f=""+f);f=aa(c,f)}if(d&&f){P(f)&&(f=[f]);for(let h=0,m,l;h<f.length;h++)m=f[h],k[m]||(k[m]=1,(g=d.get(m))?l=g:d.set(m,l=[]),b&&l.includes(a)||(l.push(a),this.fastupdate&&((g=this.reg.get(a))?g.push(l):this.reg.set(a,[l]))))}}if(this.store&&(!b||!this.store.has(a))){let k;if(this.h){k=N();for(let h=0,m;h<this.h.length;h++){m=this.h[h];if((b=m.F)&&!b(c))continue;let l;if(typeof m==="function"){l=m(c);if(!l)continue;m=[m.M]}else if(P(m)||m.constructor===
String){k[m]=c[m];continue}Ca(c,k,m,0,m[0],l)}}this.store.set(a,k||c)}}return this};function Ca(a,c,b,e,d,f){a=a[d];if(e===b.length-1)c[d]=f||a;else if(a)if(a.constructor===Array)for(c=c[d]=Array(a.length),d=0;d<a.length;d++)Ca(a,c,b,e,d);else c=c[d]||(c[d]=N()),d=b[++e],Ca(a,c,b,e,d)}
function Ba(a,c,b,e,d,f,g,k){if(a=a[g])if(e===c.length-1){if(a.constructor===Array){if(b[e]){for(c=0;c<a.length;c++)d.add(f,a[c],!0,!0);return}a=a.join(" ")}d.add(f,a,k,!0)}else if(a.constructor===Array)for(g=0;g<a.length;g++)Ba(a,c,b,e,d,f,g,k);else g=c[++e],Ba(a,c,b,e,d,f,g,k)};function Da(a,c,b){if(!a.length)return a;if(a.length===1)return a=a[0],a=b||a.length>c?a.slice(b,b+c):a;let e=[];for(let d=0,f,g;d<a.length;d++)if((f=a[d])&&(g=f.length)){if(b){if(b>=g){b-=g;continue}f=f.slice(b,b+c);g=f.length;b=0}g>c&&(f=f.slice(0,c),g=c);if(!e.length&&g>=c)return f;e.push(f);c-=g;if(!c)break}return e=e.length>1?[].concat.apply([],e):e[0]};function Ea(a,c,b,e,d){let f,g,k;typeof d==="string"?(f=d,d=""):f=d.template;g=f.indexOf("$1");k=f.substring(g+2);g=f.substring(0,g);let h=d&&d.boundary,m=!d||d.clip!==!1,l=d&&d.merge&&k&&g&&new RegExp(k+" "+g,"g");d=d&&d.ellipsis;var A=0;if(typeof d==="object"){var x=d.template;A=x.length-2;d=d.pattern}typeof d!=="string"&&(d=d===!1?"":"...");A&&(d=x.replace("$1",d));x=d.length-A;let u,p;typeof h==="object"&&(u=h.before,u===0&&(u=-1),p=h.after,p===0&&(p=-1),h=h.total||9E5);A=new Map;for(let sa=0,
V,Ha,ca;sa<c.length;sa++){let da;if(e)da=c,ca=e;else{var w=c[sa];ca=w.field;if(!ca)continue;da=w.result}Ha=b.get(ca);V=Ha.encoder;w=A.get(V);typeof w!=="string"&&(w=V.encode(a),A.set(V,w));for(let ia=0;ia<da.length;ia++){var n=da[ia].doc;if(!n)continue;n=aa(n,ca);if(!n)continue;var v=n.trim().split(/\s+/);if(!v.length)continue;n="";var r=[];let ja=[];var C=-1,z=-1,t=0;for(var y=0;y<v.length;y++){var E=v[y],D=V.encode(E);D=D.length>1?D.join(" "):D[0];let q;if(D&&E){var F=E.length,J=(V.split?E.replace(V.split,
""):E).length-D.length,L="",R=0;for(var W=0;W<w.length;W++){var M=w[W];if(M){var H=M.length;H+=J;R&&H<=R||(M=D.indexOf(M),M>-1&&(L=(M?E.substring(0,M):"")+g+E.substring(M,M+H)+k+(M+H<F?E.substring(M+H):""),R=H,q=!0))}}L&&(h&&(C<0&&(C=n.length+(n?1:0)),z=n.length+(n?1:0)+L.length,t+=F,ja.push(r.length),r.push({match:L})),n+=(n?" ":"")+L)}if(!q)E=v[y],n+=(n?" ":"")+E,h&&r.push({text:E});else if(h&&t>=h)break}t=ja.length*(f.length-2);if(u||p||h&&n.length-t>h)if(t=h+t-x*2,y=z-C,u>0&&(y+=u),p>0&&(y+=p),
y<=t)v=u?C-(u>0?u:0):C-((t-y)/2|0),r=p?z+(p>0?p:0):v+t,m||(v>0&&n.charAt(v)!==" "&&n.charAt(v-1)!==" "&&(v=n.indexOf(" ",v),v<0&&(v=0)),r<n.length&&n.charAt(r-1)!==" "&&n.charAt(r)!==" "&&(r=n.lastIndexOf(" ",r),r<z?r=z:++r)),n=(v?d:"")+n.substring(v,r)+(r<n.length?d:"");else{z=[];C={};t={};y={};E={};D={};L=J=F=0;for(W=R=1;;){var O=void 0;for(let q=0,G;q<ja.length;q++){G=ja[q];if(L)if(J!==L){if(y[q+1])continue;G+=L;if(C[G]){F-=x;t[q+1]=1;y[q+1]=1;continue}if(G>=r.length-1){if(G>=r.length){y[q+1]=
1;G>=v.length&&(t[q+1]=1);continue}F-=x}n=r[G].text;if(H=p&&D[q])if(H>0){if(n.length>H)if(y[q+1]=1,m)n=n.substring(0,H);else continue;(H-=n.length)||(H=-1);D[q]=H}else{y[q+1]=1;continue}if(F+n.length+1<=h)n=" "+n,z[q]+=n;else if(m)O=h-F-1,O>0&&(n=" "+n.substring(0,O),z[q]+=n),y[q+1]=1;else{y[q+1]=1;continue}}else{if(y[q])continue;G-=J;if(C[G]){F-=x;y[q]=1;t[q]=1;continue}if(G<=0){if(G<0){y[q]=1;t[q]=1;continue}F-=x}n=r[G].text;if(H=u&&E[q])if(H>0){if(n.length>H)if(y[q]=1,m)n=n.substring(n.length-
H);else continue;(H-=n.length)||(H=-1);E[q]=H}else{y[q]=1;continue}if(F+n.length+1<=h)n+=" ",z[q]=n+z[q];else if(m)O=n.length+1-(h-F),O>=0&&O<n.length&&(n=n.substring(O)+" ",z[q]=n+z[q]),y[q]=1;else{y[q]=1;continue}}else{n=r[G].match;u&&(E[q]=u);p&&(D[q]=p);q&&F++;let ta;G?!q&&x&&(F+=x):(t[q]=1,y[q]=1);G>=v.length-1?ta=1:G<r.length-1&&r[G+1].match?ta=1:x&&(F+=x);F-=f.length-2;if(!q||F+n.length<=h)z[q]=n;else{O=R=W=t[q]=0;break}ta&&(t[q+1]=1,y[q+1]=1)}F+=n.length;O=C[G]=1}if(O)J===L?L++:J++;else{J===
L?R=0:W=0;if(!R&&!W)break;R?(J++,L=J):L++}}n="";for(let q=0,G;q<z.length;q++)G=(t[q]?q?" ":"":(q&&!d?" ":"")+d)+z[q],n+=G;d&&!t[z.length]&&(n+=d)}l&&(n=n.replace(l," "));da[ia].highlight=n}if(e)break}return c};function Fa(a,c){const b=N(),e=[];for(let d=0,f;d<c.length;d++){f=c[d];for(let g=0;g<f.length;g++)b[f[g]]=1}for(let d=0,f;d<a.length;d++)f=a[d],b[f]&&(e.push(f),b[f]=0);return e};N();X.prototype.search=function(a,c,b,e){b||(!c&&Q(a)?(b=a,a=""):Q(c)&&(b=c,c=0));let d=[];var f=[];let g;let k,h,m,l;let A=0,x=!0,u;if(b){b.constructor===Array&&(b={index:b});a=b.query||a;var p=b.pluck;var w=b.merge;m=p||b.field||(m=b.index)&&(m.index?null:m);l=this.tag&&b.tag;k=b.suggest;x=!0;h=b.cache;u=x&&this.store&&b.highlight;g=!!u||x&&this.store&&b.enrich;c=b.limit||c;var n=b.offset||0;c||(c=x?100:0);if(l){l.constructor!==Array&&(l=[l]);var v=[];for(let z=0,t;z<l.length;z++)if(t=l[z],t.field&&
...
Array&&(m=[m])}m||(m=this.field);v=!1;for(let z=0,t,y,E;z<m.length;z++){y=m[z];let D;P(y)||(D=y,y=D.field,a=D.query||a,c=K(D.limit,c),n=K(D.offset,n),k=K(D.suggest,k),u=x&&this.store&&K(D.highlight,u),g=!!u||x&&this.store&&K(D.enrich,g),h=K(D.cache,h));if(e)t=e[z];else{r=D||b||{};C=r.enrich;const F=this.index.get(y);l&&C&&(r.enrich=!1);t=h?F.searchCache(a,c,r):F.search(a,c,r);C&&(r.enrich=C);if(v){v[z]=t;continue}}E=(t=t.result||t)&&t.length;if(l&&E){r=[];C=0;for(let F=0,J,L;F<l.length;F+=2){J=this.tag.get(l[F]);
if(!J)if(k)continue;else return d;if(L=(J=J&&J.get(l[F+1]))&&J.length)C++,r.push(J);else if(!k)return d}if(C){t=Fa(t,r);E=t.length;if(!E&&!k)return t;C--}}if(E)f[A]=y,d.push(t),A++;else if(m.length===1)return d}if(v){const z=this;return Promise.all(v).then(function(t){b&&(b.resolve=x);t.length&&(t=z.search(a,c,b,t));return t})}if(!A)return d;if(p&&(!g||!this.store))return d=d[0];v=[];for(n=0;n<f.length;n++){e=d[n];g&&e.length&&typeof e[0].doc==="undefined"&&(e=Ia.call(this,e));if(p)return u?Ea(a,
e,this.index,p,u):e;d[n]={field:f[n],result:e}}u&&(d=Ea(a,d,this.index,p,u));return w?Ja(d):d};function Ja(a){const c=[],b=N(),e=N();for(let d=0,f,g,k,h,m,l,A;d<a.length;d++){f=a[d];g=f.field;k=f.result;for(let x=0;x<k.length;x++)if(m=k[x],typeof m!=="object"?m={id:h=m}:h=m.id,(l=b[h])?l.push(g):(m.field=b[h]=[g],c.push(m)),A=m.highlight)l=e[h],l||(e[h]=l={},m.highlight=l),l[g]=A}return c}
...
function Ta(a,c,b,e,d,f,g=0){const k=e&&e.constructor===Array;var h=k?e.shift():e;if(!h)return this.export(a,c,d,f+1);if((h=a((c?c+".":"")+(g+1)+"."+b,JSON.stringify(h)))&&h.then){const m=this;return h.then(function(){return Ta.call(m,a,c,b,k?e:null,d,f,g+1)})}return Ta.call(this,a,c,b,k?e:null,d,f,g+1)}
...
function Ua(a,c){let b="";for(const e of a.entries()){a=e[0];const d=e[1];let f="";for(let g=0,k;g<d.length;g++){k=d[g]||[""];let h="";for(let m=0;m<k.length;m++)h+=(h?",":"")+(c==="string"?'"'+k[m]+'"':k[m]);h="["+h+"]";f+=(f?",":"")+h}f='["'+a+'",['+f+"]]";b+=(b?",":"")+f}return b};Y.prototype.remove=function(a,c){const b=this.reg.size&&(this.fastupdate?this.reg.get(a):this.reg.has(a));if(b){if(this.fastupdate)for(let e=0,d,f;e<b.length;e++){if((d=b[e])&&(f=d.length))if(d[f-1]===a)d.pop();else{const g=d.indexOf(a);g>=0&&d.splice(g,1)}}else Va(this.map,a),this.depth&&Va(this.ctx,a);c||this.reg.delete(a)}this.cache&&this.cache.remove(a);return this};
function Va(a,c){let b=0;var e=typeof c==="undefined";if(a.constructor===Array)for(let d=0,f,g,k;d<a.length;d++){if((f=a[d])&&f.length){if(e)return 1;g=f.indexOf(c);if(g>=0){if(f.length>1)return f.splice(g,1),1;delete a[d];if(b)return 1;k=1}else{if(k)return 1;b++}}}else for(let d of a.entries())e=d[0],Va(d[1],c)?b++:a.delete(e);return b};const Wa={memory:{resolution:1},performance:{resolution:3,fastupdate:!0,context:{depth:1,resolution:1}},match:{tokenize:"forward"},score:{resolution:9,context:{depth:2,resolution:3}}};Y.prototype.add=function(a,c,b,e){if(c&&(a||a===0)){if(!e&&!b&&this.reg.has(a))return this.update(a,c);e=this.depth;c=this.encoder.encode(c,!e);const m=c.length;if(m){const l=N(),A=N(),x=this.resolution;for(let u=0;u<m;u++){let p=c[this.rtl?m-1-u:u];var d=p.length;if(d&&(e||!A[p])){var f=this.score?this.score(c,p,u,null,0):Xa(x,m,u),g="";switch(this.tokenize){case "tolerant":Z(this,A,p,f,a,b);if(d>2){for(let w=1,n,v,r,C;w<d-1;w++)n=p.charAt(w),v=p.charAt(w+1),r=p.substring(0,w)+v,C=p.substring(w+
2),g=r+n+C,Z(this,A,g,f,a,b),g=r+C,Z(this,A,g,f,a,b);Z(this,A,p.substring(0,p.length-1),f,a,b)}break;case "full":if(d>2){for(let w=0,n;w<d;w++)for(f=d;f>w;f--){g=p.substring(w,f);n=this.rtl?d-1-w:w;var k=this.score?this.score(c,p,u,g,n):Xa(x,m,u,d,n);Z(this,A,g,k,a,b)}break}case "bidirectional":case "reverse":if(d>1){for(k=d-1;k>0;k--){g=p[this.rtl?d-1-k:k]+g;var h=this.score?this.score(c,p,u,g,k):Xa(x,m,u,d,k);Z(this,A,g,h,a,b)}g=""}case "forward":if(d>1){for(k=0;k<d;k++)g+=p[this.rtl?d-1-k:k],Z(this,
A,g,f,a,b);break}default:if(Z(this,A,p,f,a,b),e&&m>1&&u<m-1)for(d=this.L,g=p,f=Math.min(e+1,this.rtl?u+1:m-u),k=1;k<f;k++){p=c[this.rtl?m-1-u-k:u+k];h=this.bidirectional&&p>g;const w=this.score?this.score(c,g,u,p,k-1):Xa(d+(m/2>d?0:1),m,u,f-1,k-1);Z(this,l,h?g:p,w,a,b,h?p:g)}}}}this.fastupdate||this.reg.add(a)}}return this};
function Z(a,c,b,e,d,f,g){let k,h;if(!(k=c[b])||g&&!k[g]){g?(c=k||(c[b]=N()),c[g]=1,h=a.ctx,(k=h.get(g))?h=k:h.set(g,h=new Map)):(h=a.map,c[b]=1);(k=h.get(b))?h=k:h.set(b,h=k=[]);if(f)for(let m=0,l;m<k.length;m++)if((l=k[m])&&l.includes(d)){if(m<=e)return;l.splice(l.indexOf(d),1);a.fastupdate&&(c=a.reg.get(d))&&c.splice(c.indexOf(l),1);break}h=h[e]||(h[e]=[]);h.push(d);a.fastupdate&&((e=a.reg.get(d))?e.push(h):a.reg.set(d,[h]))}}
...
(d=Ya(this,b[1],b[0]))&&d.length?Da.call(this,d,c,g):[];k=N();var m=0;if(f){var l=b[0];m=1}h||h===0||(h=l?this.L:this.resolution);for(let p,w;m<a;m++){if((w=b[m])&&!k[w]){k[w]=1;p=Ya(this,w,l);a:{f=p;var A=e,x=g,u=h;let n=[];if(f&&f.length){if(f.length<=u){A.push(f);p=void 0;break a}for(let v=0,r;v<u;v++)if(r=f[v])n[v]=r;if(n.length){A.push(n);p=void 0;break a}}p=x?void 0:n}if(p){e=p;break}l&&(g&&p&&e.length||(l=w))}g&&l&&m===a-1&&!e.length&&(h=this.resolution,l="",m=-1,k=N())}a:{b=e;e=b.length;l=
b;if(e>1)b:{e=g;l=b.length;g=[];a=N();for(let p=0,w,n,v,r;p<h;p++)for(m=0;m<l;m++)if(v=b[m],p<v.length&&(w=v[p]))for(f=0;f<w.length;f++)if(n=w[f],(k=a[n])?a[n]++:(k=0,a[n]=1),r=g[k]||(g[k]=[]),r.push(n),c&&k===l-1&&r.length-d===c){l=d?r.slice(d):r;break b}if(b=g.length)if(e){if(g.length>1)c:for(b=[],h=N(),e=g.length,k=e-1;k>=0;k--){if(a=(e=g[k])&&e.length)for(m=0;m<a;m++)if(l=e[m],!h[l])if(h[l]=1,d)d--;else if(b.push(l),b.length===c)break c}else b=(g=g[0])&&c&&g.length>c||d?g.slice(d,c+d):g;g=b}else{if(b<
