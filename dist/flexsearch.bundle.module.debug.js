/**!
 * FlexSearch.js v0.8.105 (Bundle/Module/Debug)
  ...
  for (let z = 0, w, D, H; z < h.length; z++) {
    ...
    if (p && H) {
      ...
      if (this.db && e) {
        ...
        for (let G = 0, K, hb; G < p.length; G += 2) {
          K = this.tag.get(p[G]);
          if (!K) {
          ...
          if (hb = (K = K && K.get(p[G + 1])) && K.length) {
            y++, x.push(K);
          } else if (!q) {
            return r ? d : new W(d);
          }
...
ma(N.prototype);
const ib = "undefined" !== typeof window && (window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB), jb = ["map", "ctx", "tag", "reg", "cfg"];
function kb(a, b = {}) {
  if (!this) {
    return new kb(a, b);
  }
...
t.open = function() {
  ...
  return this.db || new Promise(function(b, c) {
    const e = ib.open(a.id + (a.field ? ":" + a.field : ""), 1);
...
t.destroy = function() {
  const a = ib.deleteDatabase(this.id + (this.field ? ":" + this.field : ""));
  return Z(a);
};
t.clear = function() {
  const a = this.db.transaction(jb, "readwrite");
  for (let b = 0; b < jb.length; b++) {
    a.objectStore(jb[b]).clear();
  }
  return Z(a);
};
...
;export default {Index:N, Charset:Ya, Encoder:L, Document:U, Worker:O, Resolver:W, IndexedDB:kb, Language:{}};

export const Index=N;export const  Charset=Ya;export const  Encoder=L;export const  Document=U;export const  Worker=O;export const  Resolver=W;export const  IndexedDB=kb;export const  Language={};