...
V.prototype.search = function(a, c, b, e) {
    ...
    if (m) {
      ...
      for (let x = 0, r; x < m.length; x++) {
        ...
        if (r.field && r.tag) {
          ...
          v = Object.keys(r);
          for (let E = 0, F, A; E < v.length; E++) {
            if (F = v[E], A = r[F], A.constructor === Array) {
      ...
      if (!a) {
        ...
        if (u.length) {
          for (f = 0; f < u.length; f += 2) {
            y = Ha.call(this, u[f], u[f + 1], c, n, g), d.push({field:u[f], tag:u[f + 1], result:y});
          }
        }
  ...
  for (let x = 0, r, E, F; x < l.length; x++) {
    ...
    if (e) {
      ...
      if (v = A || b, z = this.index.get(E), m && (v.enrich = !1), u) {
        u[x] = z.search(a, c, v);
        v && g && (v.enrich = g);
        continue;
      } else {
      ...
      for (let x = 0, r, E; x < z.length; x++) {
        ...
        for (let A = 0, G; A < k.length; A++) {
          if (G = k[A], "strict" === h) {
            ...
            const H = r.indexOf(G);
            if (-1 < H) {
              v += (v ? " " : "") + E.substring(0, H) + f.replace("$1", E.substring(H, G.length)) + E.substring(H + G.length);
              F = !0;
              break;
            }
          }
...
const Oa = new Map([["ae", "a"], ["oe", "o"], ["sh", "s"], ["kh", "k"], ["th", "t"], ["pf", "f"]]), Pa = [/([^aeo])h(.)/g, "$1$2", /([aeo])h([^aeo]|$)/g, "$1$2", /([^0-9])\1+/g, "$1"];
...
const Va = {memory:{resolution:1}, performance:{resolution:6, fastupdate:!0, context:{depth:1, resolution:3}}, match:{tokenize:"forward"}, score:{resolution:9, context:{depth:2, resolution:9}}};
        ...
        if (d && (n || !m[q])) {
          ...
          switch(this.tokenize) {
            case "full":
              if (2 < d) {
                for (f = 0; f < d; f++) {
                  for (var k = d; k > f; k--) {
                    g = q.substring(f, k);
                    var h = this.score ? this.score(c, q, p, g, f) : Wa(t, e, p, d, f);
                    Z(this, m, g, h, a, b);
                  }
                }
                break;
            ...
            case "reverse":
              if (1 < d) {
                for (k = d - 1; 0 < k; k--) {
                  g = q[k] + g, h = this.score ? this.score(c, q, p, g, k) : Wa(t, e, p, d, k), Z(this, m, g, h, a, b);
                }
                g = "";
              }
            case "forward":
              ...
              if (Z(this, m, q, f, a, b), n && 1 < e && p < e - 1) {
                for (d = C(), g = this.S, f = q, k = Math.min(n + 1, e - p), d[f] = 1, h = 1; h < k; h++) {
                  if ((q = c[this.rtl ? e - 1 - p - h : p + h]) && !d[q]) {
                    d[q] = 1;
                    const y = this.score ? this.score(c, f, p, q, h) : Wa(g + (e / 2 > g ? 0 : 1), e, p, k - 1, h - 1), u = this.bidirectional && q > f;
...
ja(O.prototype);
const ab = {Index:O, Charset:Ua, Encoder:K, Document:V, Worker:null, Resolver:null, IndexedDB:null, Language:{}}, bb = "undefined" !== typeof self ? self : "undefined" !== typeof global ? global : "undefined" !== typeof window ? window : {};
let cb;
