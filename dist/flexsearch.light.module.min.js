...
d=x(),h,e,k=this.split||""===this.split?a.split(this.split):[a];for(let n=0,g,w;n<k.length;n++)if((g=w=k[n])&&!(g.length<this.minlength||g.length>this.maxlength)){if(c){if(d[g])continue;d[g]=1}else{if(h===g)continue;h=g}if(b)f.push(g);else if(!this.filter||("function"===typeof this.filter?this.filter(g):!this.filter.has(g))){if(this.cache&&g.length<=this.o)if(this.l){var l=this.j.get(g);if(l||""===l){l&&f.push(l);continue}}else this.l=setTimeout(G,50,this);if(this.stemmer){this.u||(this.u=new RegExp("(?!^)("+
this.h+")$"));let v;for(;v!==g&&2<g.length;)v=g,g=g.replace(this.u,p=>this.stemmer.get(p))}if(g&&(this.mapper||this.dedupe&&1<g.length)){l="";for(let v=0,p="",m,q;v<g.length;v++)m=g.charAt(v),m===p&&this.dedupe||((q=this.mapper&&this.mapper.get(m))||""===q?q===p&&this.dedupe||!(p=q)||(l+=q):l+=p=m);g=l}this.matcher&&1<g.length&&(this.s||(this.s=new RegExp("("+this.g+")","g")),g=g.replace(this.s,v=>this.matcher.get(v)));if(g&&this.replacer)for(l=0;g&&l<this.replacer.length;l+=2)g=g.replace(this.replacer[l],
this.replacer[l+1]);this.cache&&w.length<=this.o&&(this.j.set(w,g),this.j.size>this.A&&(this.j.clear(),this.o=this.o/1.1|0));if(g){if(g!==w)if(c){if(d[g])continue;d[g]=1}else{if(e===g)continue;e=g}f.push(g)}}}this.finalize&&(f=this.finalize(f)||f);this.cache&&a.length<=this.m&&(this.i.set(a,f),this.i.size>this.A&&(this.i.clear(),this.m=this.m/1.1|0));return f};function G(a){a.l=null;a.i.clear();a.j.clear()};function H(a,c,b){if(!a.length)return a;if(1===a.length)return a=a[0],a=b||a.length>c?a.slice(b,b+c):a;let f=[];for(let d=0,h,e;d<a.length;d++)if((h=a[d])&&(e=h.length)){if(b){if(b>=e){b-=e;continue}h=h.slice(b,b+c);e=h.length;b=0}e>c&&(h=h.slice(0,c),e=c);if(!f.length&&e>=c)return h;f.push(h);c-=e;if(!c)break}return f=1<f.length?[].concat.apply([],f):f[0]};x();I.prototype.remove=function(a,c){const b=this.reg.size&&(this.fastupdate?this.reg.get(a):this.reg.has(a));if(b){if(this.fastupdate)for(let f=0,d,h;f<b.length;f++){if((d=b[f])&&(h=d.length))if(d[h-1]===a)d.pop();else{const e=d.indexOf(a);0<=e&&d.splice(e,1)}}else J(this.map,a),this.depth&&J(this.ctx,a);c||this.reg.delete(a)}return this};
function J(a,c){let b=0;var f="undefined"===typeof c;if(a.constructor===Array)for(let d=0,h,e,k;d<a.length;d++){if((h=a[d])&&h.length){if(f)return 1;e=h.indexOf(c);if(0<=e){if(1<h.length)return h.splice(e,1),1;delete a[d];if(b)return 1;k=1}else{if(k)return 1;b++}}}else for(let d of a.entries())f=d[0],J(d[1],c)?b++:a.delete(f);return b};const K={memory:{resolution:1},performance:{resolution:3,fastupdate:!0,context:{depth:1,resolution:1}},match:{tokenize:"forward"},score:{resolution:9,context:{depth:2,resolution:3}}};I.prototype.add=function(a,c,b,f){if(c&&(a||0===a)){if(!f&&!b&&this.reg.has(a))return this.update(a,c);f=this.depth;c=this.encoder.encode(c,!f);const n=c.length;if(n){const g=x(),w=x(),v=this.resolution;for(let p=0;p<n;p++){let m=c[this.rtl?n-1-p:p];var d=m.length;if(d&&(f||!w[m])){var h=this.score?this.score(c,m,p,null,0):L(v,n,p),e="";switch(this.tokenize){case "full":if(2<d){for(let q=0,t;q<d;q++)for(h=d;h>q;h--){e=m.substring(q,h);t=this.rtl?d-1-q:q;var k=this.score?this.score(c,m,p,e,t):L(v,
n,p,d,t);M(this,w,e,k,a,b)}break}case "bidirectional":case "reverse":if(1<d){for(k=d-1;0<k;k--){e=m[this.rtl?d-1-k:k]+e;var l=this.score?this.score(c,m,p,e,k):L(v,n,p,d,k);M(this,w,e,l,a,b)}e=""}case "forward":if(1<d){for(k=0;k<d;k++)e+=m[this.rtl?d-1-k:k],M(this,w,e,h,a,b);break}default:if(M(this,w,m,h,a,b),f&&1<n&&p<n-1)for(d=x(),e=this.v,h=m,k=Math.min(f+1,this.rtl?p+1:n-p),d[h]=1,l=1;l<k;l++)if((m=c[this.rtl?n-1-p-l:p+l])&&!d[m]){d[m]=1;const q=this.score?this.score(c,h,p,m,l-1):L(e+(n/2>e?0:
1),n,p,k-1,l-1),t=this.bidirectional&&m>h;M(this,g,t?h:m,q,a,b,t?m:h)}}}}this.fastupdate||this.reg.add(a)}}return this};function M(a,c,b,f,d,h,e){let k=e?a.ctx:a.map,l;if(!c[b]||e&&!(l=c[b])[e])e?(c=l||(c[b]=x()),c[e]=1,(l=k.get(e))?k=l:k.set(e,k=new Map)):c[b]=1,(l=k.get(b))?k=l:k.set(b,k=[]),k=k[f]||(k[f]=[]),h&&k.includes(d)||(k.push(d),a.fastupdate&&((c=a.reg.get(d))?c.push(k):a.reg.set(d,[k])))}function L(a,c,b,f,d){return b&&1<a?c+(f||0)<=a?b+(d||0):(a-1)/(c+(f||0))*(b+(d||0))+1|0:0};I.prototype.search=function(a,c,b){b||(c||"object"!==typeof a?"object"===typeof c&&(b=c,c=0):(b=a,a=""));var f=[],d=0;if(b){a=b.query||a;c=b.limit||c;d=b.offset||0;var h=b.context;var e=b.suggest;var k=!0;var l=b.resolution}"undefined"===typeof k&&(k=!0);h=this.depth&&!1!==h;a=this.encoder.encode(a,!h);b=a.length;c=c||(k?100:0);if(1===b)return e=d,(d=N(this,a[0],""))&&d.length?H.call(this,d,c,e):[];if(2===b&&h&&!e)return e=d,(d=N(this,a[1],a[0]))&&d.length?H.call(this,d,c,e):[];k=x();var n=0;if(h){var g=
a[0];n=1}l||0===l||(l=g?this.v:this.resolution);for(let m,q;n<b;n++){if((q=a[n])&&!k[q]){k[q]=1;m=N(this,q,g);a:{h=m;var w=f,v=e,p=l;let t=[];if(h&&h.length){if(h.length<=p){w.push(h);m=void 0;break a}for(let y=0,z;y<p;y++)if(z=h[y])t[y]=z;if(t.length){w.push(t);m=void 0;break a}}m=v?void 0:t}if(m){f=m;break}g&&(e&&m&&f.length||(g=q))}e&&g&&n===b-1&&!f.length&&(l=this.resolution,g="",n=-1,k=x())}a:{a=f;f=a.length;g=a;if(1<f)b:{f=e;g=a.length;e=[];b=x();for(let m=0,q,t,y,z;m<l;m++)for(n=0;n<g;n++)if(y=
a[n],m<y.length&&(q=y[m]))for(h=0;h<q.length;h++)if(t=q[h],(k=b[t])?b[t]++:(k=0,b[t]=1),z=e[k]||(e[k]=[]),z.push(t),c&&k===g-1&&z.length-d===c){g=d?z.slice(d):z;break b}if(a=e.length)if(f){if(1<e.length)c:for(a=[],l=x(),f=e.length,k=f-1;0<=k;k--){if(b=(f=e[k])&&f.length)for(n=0;n<b;n++)if(g=f[n],!l[g])if(l[g]=1,d)d--;else if(a.push(g),a.length===c)break c}else a=(e=e[0])&&c&&e.length>c||d?e.slice(d,c+d):e;e=a}else{if(a<g){g=[];break b}e=e[a-1];if(c||d)if(e.length>c||d)e=e.slice(d,c+d)}g=e}else if(1===
