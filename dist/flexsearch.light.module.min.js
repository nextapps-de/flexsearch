...
this.split||""===this.split?a.split(this.split):a;for(let g=0,e,h;g<f.length;g++)if((e=h=f[g])&&!(e.length<this.minlength||e.length>this.maxlength))if(d)b.push(e);else if(!this.filter||("function"===typeof this.filter?this.filter(e):!this.filter.has(e))){if(this.cache&&e.length<=this.o)if(this.l){var c=this.j.get(e);if(c||""===c){c&&b.push(c);continue}}else this.l=setTimeout(G,50,this);this.stemmer&&2<e.length&&(this.u||(this.u=new RegExp("(?!^)("+this.h+")$")),e=e.replace(this.u,k=>this.stemmer.get(k)));
if(e&&(this.mapper||this.dedupe&&1<e.length)){c="";for(let k=0,n="",m,v;k<e.length;k++)m=e.charAt(k),m===n&&this.dedupe||((v=this.mapper&&this.mapper.get(m))||""===v?v===n&&this.dedupe||!(n=v)||(c+=v):c+=n=m);e=c}this.matcher&&1<e.length&&(this.s||(this.s=new RegExp("("+this.g+")","g")),e=e.replace(this.s,k=>this.matcher.get(k)));if(e&&this.replacer)for(c=0;e&&c<this.replacer.length;c+=2)e=e.replace(this.replacer[c],this.replacer[c+1]);this.cache&&h.length<=this.o&&(this.j.set(h,e),this.j.size>this.A&&
