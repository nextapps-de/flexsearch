  ...
  for (let z = 0, w, D, H; z < l.length; z++) {
    ...
    if (p && H) {
      ...
      if (this.db && e) {
        ...
        for (let G = 0, L, rb; G < p.length; G += 2) {
          L = this.tag.get(p[G]);
          if (!L) {
          ...
          if (rb = (L = L && L.get(p[G + 1])) && L.length) {
            y++, x.push(L);
          } else if (!v) {
            return q ? d : new X(d);
          }
...
t.clear = function() {
  ...
  for (let c = 0; c < pb.length; c++) {
    a.objectStore(pb[c]).clear();
  }
  return Z(a);
};
...
;const tb = {Index:O, Charset:db, Encoder:J, Document:V, Worker:Q, Resolver:X, IndexedDB:qb, Language:{}}, ub = self;
let vb;
