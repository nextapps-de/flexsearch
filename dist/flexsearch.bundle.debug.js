  ...
  if (c && (a || 0 === a)) {
    ...
    for (let h = 0, k; h < this.field.length; h++) {
      ...
      if ("function" === typeof k) {
        ...
        if (d = k.J, !d || d(c)) {
          k.constructor === String ? k = ["" + k] : N(k) && (k = [k]), Oa(c, k, this.K, 0, e, a, k[0], b);
        }
      }
    }
    if (this.tag) {
...
function db(a, c, b, e, d) {
  ...
  n = new Map();
  for (let Pa = 0, ea, gb, oa; Pa < c.length; Pa++) {
    let pa;
    if (e) {
      pa = c, oa = e;
    } else {
          ...
          for (O = L = 1;;) {
            ...
            for (let x = 0, J; x < ya.length; x++) {
              ...
              if (G) {
                ...
                J >= y.length - 1 ? Qa = 1 : J < v.length - 1 && v[J + 1].match ? Qa = 1 : u && (D += u);
                D -= f.length - 2;
                if (!x || D + q.length <= k) {
                  B[x] = q;
                } else {
...
;Na.prototype.search = function(a, c, b, e) {
        ...
        if (v.length) {
          for (r = 0; r < v.length; r += 2) {
            if (this.db) {
              ...
              e = eb.call(this, v[r], v[r + 1], c, y, q);
            }
  ...
  for (y = 0; y < f.length; y++) {
    ...
    if (g) {
      return p ? t ? db(a, r, this.index, g, t) : r : new Y(r, this);
    }
