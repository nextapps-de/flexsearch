 ...
 */
(function(self){'use strict';var r;function u(a,c,b){const f=typeof b,d=typeof a;if("undefined"!==f){if("undefined"!==d){if(b){if("function"===d&&f===d)return function(h){return a(b(h))};c=a.constructor;if(c===b.constructor){if(c===Array)return b.concat(a);if(c===Map){var g=new Map(b);for(var e of a)g.set(e[0],e[1]);return g}if(c===Set){e=new Set(b);for(g of a.values())e.add(g);return e}}}return a}return b}return"undefined"===d?c:a}function w(){return Object.create(null)};const y=/[^\p{L}\p{N}]+/u,z=/(\d{3})/g,A=/(\D)(\d{3})/g,B=/(\d{3})(\D)/g,D="".normalize&&/[\u0300-\u036f]/g;function F(a={}){if(!this||this.constructor!==F)return new F(...arguments);if(arguments.length)for(a=0;a<arguments.length;a++)this.assign(arguments[a]);else this.assign(a)}r=F.prototype;
...
r.addMatcher=function(a,c){if("object"===typeof a)return this.addReplacer(a,c);if(2>a.length&&(this.dedupe||this.mapper))return this.addMapper(a,c);this.matcher||(this.matcher=new Map);this.matcher.set(a,c);this.g+=(this.g?"|":"")+a;this.s=null;this.cache&&G(this);return this};r.addReplacer=function(a,c){if("string"===typeof a)return this.addMatcher(a,c);this.replacer||(this.replacer=[]);this.replacer.push(a,c);this.cache&&G(this);return this};
...
if(e&&(this.mapper||this.dedupe&&1<e.length)){d="";for(let k=0,n="",m,v;k<e.length;k++)m=e.charAt(k),m===n&&this.dedupe||((v=this.mapper&&this.mapper.get(m))||""===v?v===n&&this.dedupe||!(n=v)||(d+=v):d+=n=m);e=d}this.matcher&&1<e.length&&(this.s||(this.s=new RegExp("("+this.g+")","g")),e=e.replace(this.s,k=>this.matcher.get(k)));if(e&&this.replacer)for(d=0;e&&d<this.replacer.length;d+=2)e=e.replace(this.replacer[d],this.replacer[d+1]);this.cache&&h.length<=this.o&&(this.j.set(h,e),this.j.size>this.A&&
...
0===k||(k=m?this.v:this.resolution);for(let l,q;n<b;n++){if((q=a[n])&&!h[q]){h[q]=1;l=O(this,q,m);a:{g=l;var v=f,C=e,p=k;let t=[];if(g&&g.length){if(g.length<=p){v.push(g);l=void 0;break a}for(let x=0,E;x<p;x++)if(E=g[x])t[x]=E;if(t.length){v.push(t);l=void 0;break a}}l=C?void 0:t}if(l){f=l;break}m&&(e&&l&&f.length||(m=q))}e&&m&&n===b-1&&!f.length&&(k=this.resolution,m="",n=-1,h=w())}a:{a=f;f=a.length;m=a;if(1<f)b:{f=e;m=a.length;e=[];b=w();for(let l=0,q,t,x,E;l<k;l++)for(n=0;n<m;n++)if(x=a[n],l<
x.length&&(q=x[l]))for(g=0;g<q.length;g++)t=q[g],(h=b[t])?b[t]++:(h=0,b[t]=1),E=e[h]||(e[h]=[]),E.push(t);if(a=e.length)if(f){if(1<e.length)c:for(a=[],k=w(),f=e.length,h=f-1;0<=h;h--){if(b=(f=e[h])&&f.length)for(n=0;n<b;n++)if(m=f[n],!k[m])if(k[m]=1,d)d--;else if(a.push(m),a.length===c)break c}else a=(e=e[0]).length>c||d?e.slice(d,c+d):e;e=a}else{if(a<m){m=[];break b}e=e[a-1];if(c||d)if(e.length>c||d)e=e.slice(d,c+d)}m=e}else if(1===f){c=H.call(null,a[0],c,d);break a}c=m}return c};
...
function Q(a){let c=0;if(a.constructor===Array)for(let b=0,f;b<a.length;b++)(f=a[b])&&(c+=f.length);else for(const b of a){const f=b[0],d=Q(b[1]);d?c+=d:a.delete(f)}return c}r.cleanup=function(){if(!this.fastupdate)return this;Q(this.map);this.depth&&Q(this.ctx);return this};
r.searchCache=function(a,c,b){a=("object"===typeof a?""+a.query:a).toLowerCase();this.cache||(this.cache=new I);let f=this.cache.get(a);if(!f){f=this.search(a,c,b);if(f.then){const d=this;f.then(function(g){d.cache.set(a,g);return g})}this.cache.set(a,f)}return f};const R={Index:L,Charset:null,Encoder:F,Document:null,Worker:null,Resolver:null,IndexedDB:null,Language:{}},S=self;let T;(T=S.define)&&T.amd?T([],function(){return R}):"object"===typeof S.exports?S.exports=R:S.FlexSearch=R;}(this||self));
