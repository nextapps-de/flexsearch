...
this.maxlength=u(a.maxlength,0,this.maxlength);if(this.cache=b=u(a.cache,!0,this.cache))this.l=null,this.v="number"===typeof b?b:2E5,this.h=new Map,this.j=new Map,this.o=this.m=128;this.g="";this.s=null;this.i="";this.u=null;if(this.matcher)for(const d of this.matcher.keys())this.g+=(this.g?"|":"")+d;if(this.stemmer)for(const d of this.stemmer.keys())this.i+=(this.i?"|":"")+d;return this};
...
""===this.split?a.split(this.split):a;for(let g=0,f,h;g<d.length;g++){if(!(f=h=d[g]))continue;if(f.length<this.minlength)continue;if(c){b.push(f);continue}if(this.filter&&this.filter.has(f))continue;if(this.cache&&f.length<=this.o)if(this.l){var e=this.j.get(f);if(e||""===e){e&&b.push(e);continue}}else this.l=setTimeout(G,50,this);let k;this.stemmer&&2<f.length&&(this.u||(this.u=new RegExp("(?!^)("+this.i+")$")),f=f.replace(this.u,l=>this.stemmer.get(l)),k=1);f&&k&&(f.length<this.minlength||this.filter&&
this.filter.has(f))&&(f="");if(f&&(this.mapper||this.dedupe&&1<f.length)){e="";for(let l=0,t="",q,n;l<f.length;l++)q=f.charAt(l),q===t&&this.dedupe||((n=this.mapper&&this.mapper.get(q))||""===n?n===t&&this.dedupe||!(t=n)||(e+=n):e+=t=q);f=e}this.matcher&&1<f.length&&(this.s||(this.s=new RegExp("("+this.g+")","g")),f=f.replace(this.s,l=>this.matcher.get(l)));if(f&&this.replacer)for(e=0;f&&e<this.replacer.length;e+=2)f=f.replace(this.replacer[e],this.replacer[e+1]);this.cache&&h.length<=this.o&&(this.j.set(h,
