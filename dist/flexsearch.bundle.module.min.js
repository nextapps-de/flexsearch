/**!
 * FlexSearch.js v0.8.163 (Bundle/Module)
...
this.A+")$"));let q;for(;q!==m&&2<m.length;)q=m,m=m.replace(this.N,n=>this.stemmer.get(n))}if(m&&(this.mapper||this.dedupe&&1<m.length)){h="";for(let q=0,n="",r,u;q<m.length;q++)r=m.charAt(q),r===n&&this.dedupe||((u=this.mapper&&this.mapper.get(r))||""===u?u===n&&this.dedupe||!(n=u)||(h+=u):h+=n=r);m=h}this.matcher&&1<m.length&&(this.M||(this.M=new RegExp("("+this.h+")","g")),m=m.replace(this.M,q=>this.matcher.get(q)));if(m&&this.replacer)for(h=0;m&&h<this.replacer.length;h+=2)m=m.replace(this.replacer[h],
...
function $a(a,b,c,e,d){const f=[],g=G();let k=0;for(let h=0,l;h<a.length;h++)if(l=a[h]){k<l.length&&(k=l.length);for(let m=0,t;m<l.length;m++)if(t=l[m])for(let q=0,n;q<t.length;q++)n=t[q],g[n]=g[n]?2:1}for(let h=0,l,m=0;h<k;h++)for(let t=0,q;t<a.length;t++)if(q=a[t])if(l=q[h])for(let n=0,r;n<l.length;n++)if(r=l[n],1===g[r])if(c)c--;else if(e){if(f.push(r),f.length===b)return f}else{const u=h+(t?d:0);f[u]||(f[u]=[]);f[u].push(r);if(++m===b)return f}return f};Y.prototype.not=function(){const {O:a,P:b,limit:c,offset:e,enrich:d,resolve:f,suggest:g}=Wa(this,"not",arguments);return ab.call(this,a,b,c,e,d,f,g)};function ab(a,b,c,e,d,f,g){if(b.length){const k=this;return Promise.all(b).then(function(h){a=[];for(let l=0,m;l<h.length;l++)(m=h[l]).length&&(a[l]=m);return ab.call(k,a,[],c,e,d,f,g)})}if(a.length&&this.result.length)this.result=bb.call(this,a,c,e,f);else if(f)return this.resolve(c,e,d);return f?d?X.call(this.index,this.result):this.result:this}
function bb(a,b,c,e){const d=[];a=new Set(a.flat().flat());for(let f=0,g,k=0;f<this.result.length;f++)if(g=this.result[f])for(let h=0,l;h<g.length;h++)if(l=g[h],!a.has(l))if(c)c--;else if(e){if(d.push(l),d.length===b)return d}else if(d[f]||(d[f]=[]),d[f].push(l),++k===b)return d;return d};function cb(a,b,c,e,d){let f,g,k;"string"===typeof d?(f=d,d=""):f=d.template;g=f.indexOf("$1");k=f.substring(g+2);g=f.substring(0,g);let h=d&&d.boundary,l=!d||!1!==d.clip,m=d&&d.merge&&k&&g&&new RegExp(k+" "+g,"g");d=d&&d.ellipsis;var t=0;if("object"===typeof d){var q=d.template;t=q.length-2;d=d.pattern}"string"!==typeof d&&(d=!1===d?"":"...");t&&(d=q.replace("$1",d));q=d.length-t;let n,r;"object"===typeof h&&(n=h.before,0===n&&(n=-1),r=h.after,0===r&&(r=-1),h=h.total||9E5);t=new Map;for(let Ka=0,
aa,eb,na;Ka<b.length;Ka++){let oa;if(e)oa=b,na=e;else{var u=b[Ka];na=u.field;if(!na)continue;oa=u.result}eb=c.get(na);aa=eb.encoder;u=t.get(aa);"string"!==typeof u&&(u=aa.encode(a),t.set(aa,u));for(let va=0;va<oa.length;va++){var p=oa[va].doc;if(!p)continue;p=ea(p,na);if(!p)continue;var y=p.trim().split(/\s+/);if(!y.length)continue;p="";var x=[];let wa=[];var J=-1,B=-1,z=0;for(var A=0;A<y.length;A++){var D=y[A],C=aa.encode(D);C=1<C.length?C.join(" "):C[0];let w;if(C&&D){var E=D.length,H=(aa.split?
D.replace(aa.split,""):D).length-C.length,M="",V=0;for(var ba=0;ba<u.length;ba++){var Q=u[ba];if(Q){var K=Q.length;K+=H;V&&K<=V||(Q=C.indexOf(Q),-1<Q&&(M=(Q?D.substring(0,Q):"")+g+D.substring(Q,Q+K)+k+(Q+K<E?D.substring(Q+K):""),V=K,w=!0))}}M&&(h&&(0>J&&(J=p.length+(p?1:0)),B=p.length+(p?1:0)+M.length,z+=E,wa.push(x.length),x.push({match:M})),p+=(p?" ":"")+M)}if(!w)D=y[A],p+=(p?" ":"")+D,h&&x.push({text:D});else if(h&&z>=h)break}z=wa.length*(f.length-2);if(n||r||h&&p.length-z>h)if(z=h+z-2*q,A=B-J,
0<n&&(A+=n),0<r&&(A+=r),A<=z)y=n?J-(0<n?n:0):J-((z-A)/2|0),x=r?B+(0<r?r:0):y+z,l||(0<y&&" "!==p.charAt(y)&&" "!==p.charAt(y-1)&&(y=p.indexOf(" ",y),0>y&&(y=0)),x<p.length&&" "!==p.charAt(x-1)&&" "!==p.charAt(x)&&(x=p.lastIndexOf(" ",x),x<B?x=B:++x)),p=(y?d:"")+p.substring(y,x)+(x<p.length?d:"");else{B=[];J={};z={};A={};D={};C={};M=H=E=0;for(ba=V=1;;){var S=void 0;for(let w=0,I;w<wa.length;w++){I=wa[w];if(M)if(H!==M){if(A[w+1])continue;I+=M;if(J[I]){E-=q;z[w+1]=1;A[w+1]=1;continue}if(I>=x.length-1){if(I>=
x.length){A[w+1]=1;I>=y.length&&(z[w+1]=1);continue}E-=q}p=x[I].text;if(K=r&&C[w])if(0<K){if(p.length>K)if(A[w+1]=1,l)p=p.substring(0,K);else continue;(K-=p.length)||(K=-1);C[w]=K}else{A[w+1]=1;continue}if(E+p.length+1<=h)p=" "+p,B[w]+=p;else if(l)S=h-E-1,0<S&&(p=" "+p.substring(0,S),B[w]+=p),A[w+1]=1;else{A[w+1]=1;continue}}else{if(A[w])continue;I-=H;if(J[I]){E-=q;A[w]=1;z[w]=1;continue}if(0>=I){if(0>I){A[w]=1;z[w]=1;continue}E-=q}p=x[I].text;if(K=n&&D[w])if(0<K){if(p.length>K)if(A[w]=1,l)p=p.substring(p.length-
K);else continue;(K-=p.length)||(K=-1);D[w]=K}else{A[w]=1;continue}if(E+p.length+1<=h)p+=" ",B[w]=p+B[w];else if(l)S=p.length+1-(h-E),0<=S&&S<p.length&&(p=p.substring(S)+" ",B[w]=p+B[w]),A[w]=1;else{A[w]=1;continue}}else{p=x[I].match;n&&(D[w]=n);r&&(C[w]=r);w&&E++;let La;I?!w&&q&&(E+=q):(z[w]=1,A[w]=1);I>=y.length-1?La=1:I<x.length-1&&x[I+1].match?La=1:q&&(E+=q);E-=f.length-2;if(!w||E+p.length<=h)B[w]=p;else{S=V=ba=z[w]=0;break}La&&(z[w+1]=1,A[w+1]=1)}E+=p.length;S=J[I]=1}if(S)H===M?M++:H++;else{H===
M?V=0:ba=0;if(!V&&!ba)break;V?(H++,M=H):M++}}p="";for(let w=0,I;w<B.length;w++)I=(w&&z[w]?" ":(w&&!d?" ":"")+d)+B[w],p+=I;d&&!z[B.length]&&(p+=d)}m&&(p=p.replace(m," "));oa[va].highlight=p}if(e)break}return b};function Y(a,b){if(!this||this.constructor!==Y)return new Y(a,b);if(a&&a.index)return a.resolve=!1,this.index=a.index,this.h=a.boost||0,this.result=this.index.search(a).result,this;this.index=b||null;this.result=a||[];this.h=0}Y.prototype.limit=function(a){if(this.result.length){const b=[];for(let c=0,e;c<this.result.length;c++)if(e=this.result[c])if(e.length<=a){if(b[c]=e,a-=e.length,!a)break}else{b[c]=e.slice(0,a);break}this.result=b}return this};
Y.prototype.offset=function(a){if(this.result.length){const b=[];for(let c=0,e;c<this.result.length;c++)if(e=this.result[c])e.length<=a?a-=e.length:(b[c]=e.slice(a),a=0);this.result=b}return this};Y.prototype.boost=function(a){this.h+=a;return this};Y.prototype.resolve=function(a,b,c){const e=this.index;let d=this.result;this.result=this.index=null;d.length&&("object"===typeof a&&(c=a.enrich,b=a.offset,a=a.limit),d=Va.call(e,d,a||100,b,c));return d};G();W.prototype.search=function(a,b,c,e){c||(!b&&ca(a)?(c=a,a=""):ca(b)&&(c=b,b=0));if(c&&c.cache){c.cache=!1;var d=this.searchCache(a,b,c);c.cache=!0;return d}let f=[];var g=[];let k,h;let l,m;let t=0;var q=!0;let n;if(c){c.constructor===Array&&(c={index:c});a=c.query||a;k=c.pluck;h=c.merge;l=k||c.field||(l=c.index)&&(l.index?null:l);m=this.tag&&c.tag;var r=c.suggest;q=!1!==c.resolve;q||k||!(l=l||this.field)||(L(l)?k=l:(l.constructor===Array&&1===l.length&&(l=l[0]),k=l.field||l.index));n=q&&this.store&&
c.highlight;d=!!n||q&&this.store&&c.enrich;b=c.limit||b;var u=c.offset||0;b||(b=100);if(m&&(!this.db||!e)){m.constructor!==Array&&(m=[m]);var p=[];for(let B=0,z;B<m.length;B++)if(z=m[B],z.field&&z.tag){var y=z.tag;if(y.constructor===Array)for(var x=0;x<y.length;x++)p.push(z.field,y[x]);else p.push(z.field,y)}else{y=Object.keys(z);for(let A=0,D,C;A<y.length;A++)if(D=y[A],C=z[D],C.constructor===Array)for(x=0;x<C.length;x++)p.push(D,C[x]);else p.push(D,C)}m=p;if(!a){q=[];if(p.length)for(g=0;g<p.length;g+=
2){if(this.db){r=this.index.get(p[g]);if(!r)continue;q.push(r=r.db.tag(p[g+1],b,u,d))}else r=db.call(this,p[g],p[g+1],b,u,d);f.push({field:p[g],tag:p[g+1],result:r})}return q.length?Promise.all(q).then(function(B){for(let z=0;z<B.length;z++)f[z].result=B[z];return f}):f}}l&&l.constructor!==Array&&(l=[l])}l||(l=this.field);let J;p=(this.worker||this.db)&&!e&&[];for(let B=0,z,A,D;B<l.length;B++){A=l[B];if(this.db&&this.tag&&!this.F[B])continue;let C;L(A)||(C=A,A=C.field,a=C.query||a,b=fb(C.limit,b),
u=fb(C.offset,u),r=fb(C.suggest,r),n=q&&this.store&&fb(C.highlight,n),d=!!n||q&&this.store&&fb(C.enrich,d));if(e)z=e[B];else if(y=C||c,x=this.index.get(A),m&&(this.db&&(y.tag=m,J=x.db.support_tag_search,y.field=l),J||(y.enrich=!1)),p){p[B]=x.search(a,b,y);y&&d&&(y.enrich=d);continue}else z=x.search(a,b,y),y&&d&&(y.enrich=d);D=z&&(q?z.length:z.result.length);if(m&&D){y=[];x=0;if(this.db&&e){if(!J)for(let E=l.length;E<e.length;E++){let H=e[E];if(H&&H.length)x++,y.push(H);else if(!r)return q?f:new Y(f,
this)}}else for(let E=0,H,M;E<m.length;E+=2){H=this.tag.get(m[E]);if(!H)if(r)continue;else return q?f:new Y(f,this);if(M=(H=H&&H.get(m[E+1]))&&H.length)x++,y.push(H);else if(!r)return q?f:new Y(f,this)}if(x){z=Ua(z,y,q);D=z.length;if(!D&&!r)return q?z:new Y(z,this);x--}}if(D)g[t]=A,f.push(z),t++;else if(1===l.length)return q?f:new Y(f,this)}if(p){if(this.db&&m&&m.length&&!J)for(d=0;d<m.length;d+=2){g=this.index.get(m[d]);if(!g)if(r)continue;else return q?f:new Y(f,this);p.push(g.db.tag(m[d+1],b,u,
!1))}const B=this;return Promise.all(p).then(function(z){return z.length?B.search(a,b,c,z):z})}if(!t)return q?f:new Y(f,this);if(k&&(!d||!this.store))return f=f[0],q||(f.index=this),f;p=[];for(u=0;u<g.length;u++){r=f[u];d&&r.length&&"undefined"===typeof r[0].doc&&(this.db?p.push(r=this.index.get(this.field[0]).db.enrich(r)):r=X.call(this,r));if(k)return q?n?cb(a,r,this.index,k,n):r:new Y(r,this);f[u]={field:g[u],result:r}}if(d&&this.db&&p.length){const B=this;return Promise.all(p).then(function(z){for(let A=
0;A<z.length;A++)f[A].result=z[A];return h?gb(f):n?cb(a,f,B.index,k,n):f})}return h?gb(f):n?cb(a,f,this.index,k,n):f};function fb(a,b){return"undefined"===typeof a?b:a}function gb(a){const b=[],c=G();for(let e=0,d,f;e<a.length;e++){d=a[e];f=d.result;for(let g=0,k,h,l;g<f.length;g++)h=f[g],"object"!==typeof h&&(h={id:h}),k=h.id,(l=c[k])?l.push(d.field):(h.field=c[k]=[d.field],b.push(h))}return b}
function db(a,b,c,e,d){a=this.tag.get(a);if(!a)return[];if((b=(a=a&&a.get(b))&&a.length-e)&&0<b){if(b>c||e)a=a.slice(e,e+c);d&&(a=X.call(this,a));return a}}function X(a){if(!this||!this.store)return a;const b=Array(a.length);for(let c=0,e;c<a.length;c++)e=a[c],b[c]={id:e,doc:this.store.get(e)};return b};function W(a){if(!this||this.constructor!==W)return new W(a);const b=a.document||a.doc||a;let c,e;this.F=[];this.field=[];this.J=[];this.key=(c=b.key||b.id)&&hb(c,this.J)||"id";(e=a.keystore||0)&&(this.keystore=e);this.fastupdate=!!a.fastupdate;this.reg=!this.fastupdate||a.worker||a.db?e?new U(e):new Set:e?new T(e):new Map;this.C=(c=b.store||null)&&c&&!0!==c&&[];this.store=c&&(e?new T(e):new Map);this.cache=(c=a.cache||null)&&new ib(c);a.cache=!1;this.worker=a.worker||!1;this.priority=a.priority||
...
function sb(a,b){let c=0;var e="undefined"===typeof b;if(a.constructor===Array)for(let d=0,f,g;d<a.length;d++){if((f=a[d])&&f.length)if(e)c++;else if(g=f.indexOf(b),0<=g){1<f.length?(f.splice(g,1),c++):delete a[d];break}else c++}else for(let d of a.entries()){e=d[0];const f=sb(d[1],b);f?c+=f:a.delete(e)}return c};const ub={memory:{resolution:1},performance:{resolution:3,fastupdate:!0,context:{depth:1,resolution:1}},match:{tokenize:"forward"},score:{resolution:9,context:{depth:2,resolution:3}}};O.prototype.add=function(a,b,c,e){if(b&&(a||0===a)){if(!e&&!c&&this.reg.has(a))return this.update(a,b);e=this.depth;b=this.encoder.encode(b,!e);const l=b.length;if(l){const m=G(),t=G(),q=this.resolution;for(let n=0;n<l;n++){let r=b[this.rtl?l-1-n:n];var d=r.length;if(d&&(e||!t[r])){var f=this.score?this.score(b,r,n,null,0):vb(q,l,n),g="";switch(this.tokenize){case "full":if(2<d){for(let u=0,p;u<d;u++)for(f=d;f>u;f--){g=r.substring(u,f);p=this.rtl?d-1-u:u;var k=this.score?this.score(b,r,n,g,p):vb(q,
l,n,d,p);wb(this,t,g,k,a,c)}break}case "bidirectional":case "reverse":if(1<d){for(k=d-1;0<k;k--){g=r[this.rtl?d-1-k:k]+g;var h=this.score?this.score(b,r,n,g,k):vb(q,l,n,d,k);wb(this,t,g,h,a,c)}g=""}case "forward":if(1<d){for(k=0;k<d;k++)g+=r[this.rtl?d-1-k:k],wb(this,t,g,f,a,c);break}default:if(wb(this,t,r,f,a,c),e&&1<l&&n<l-1)for(d=G(),g=this.U,f=r,k=Math.min(e+1,this.rtl?n+1:l-n),d[f]=1,h=1;h<k;h++)if((r=b[this.rtl?l-1-n-h:n+h])&&!d[r]){d[r]=1;const u=this.score?this.score(b,f,n,r,h-1):vb(g+(l/
2>g?0:1),l,n,k-1,h-1),p=this.bidirectional&&r>f;wb(this,m,p?f:r,u,a,c,p?r:f)}}}}this.fastupdate||this.reg.add(a)}else b=""}this.db&&(b||this.commit_task.push({del:a}),this.T&&tb(this));return this};
...
function vb(a,b,c,e,d){return c&&1<a?b+(e||0)<=a?c+(d||0):(a-1)/(b+(e||0))*(c+(d||0))+1|0:0};O.prototype.search=function(a,b,c){c||(b||"object"!==typeof a?"object"===typeof b&&(c=b,b=0):(c=a,a=""));if(c&&c.cache)return c.cache=!1,a=this.searchCache(a,b,c),c.cache=!0,a;let e=[],d,f,g,k=0,h,l,m,t,q;c&&(a=c.query||a,b=c.limit||b,k=c.offset||0,f=c.context,g=c.suggest,q=(h=c.resolve)&&c.enrich,m=c.boost,t=c.resolution,l=this.db&&c.tag);"undefined"===typeof h&&(h=this.resolve);f=this.depth&&!1!==f;let n=this.encoder.encode(a,!f);d=n.length;b=b||(h?100:0);if(1===d)return xb.call(this,n[0],"",b,
k,h,q,l);if(2===d&&f&&!g)return xb.call(this,n[1],n[0],b,k,h,q,l);let r=G(),u=0,p;f&&(p=n[0],u=1);t||0===t||(t=p?this.U:this.resolution);if(this.db){if(this.db.search&&(c=this.db.search(this,n,b,k,g,h,q,l),!1!==c))return c;const y=this;return async function(){for(let x,J;u<d;u++){if((J=n[u])&&!r[J]){r[J]=1;x=await yb(y,J,p,0,0,!1,!1);if(x=zb(x,e,g,t)){e=x;break}p&&(g&&x&&e.length||(p=J))}g&&p&&u===d-1&&!e.length&&(t=y.resolution,p="",u=-1,r=G())}return Ab(e,t,b,k,g,m,h)}()}for(let y,x;u<d;u++){if((x=
n[u])&&!r[x]){r[x]=1;y=yb(this,x,p,0,0,!1,!1);if(y=zb(y,e,g,t)){e=y;break}p&&(g&&y&&e.length||(p=x))}g&&p&&u===d-1&&!e.length&&(t=this.resolution,p="",u=-1,r=G())}return Ab(e,t,b,k,g,m,h)};function Ab(a,b,c,e,d,f,g){let k=a.length,h=a;if(1<k)h=Sa(a,b,c,e,d,f,g);else if(1===k)return g?Va.call(null,a[0],c,e):new Y(a[0],this);return g?h:new Y(h,this)}
function xb(a,b,c,e,d,f,g){a=yb(this,a,b,c,e,d,f,g);return this.db?a.then(function(k){return d?k||[]:new Y(k,this)}):a&&a.length?d?Va.call(this,a,c,e):new Y(a,this):d?[]:new Y([],this)}function zb(a,b,c,e){let d=[];if(a&&a.length){if(a.length<=e){b.push(a);return}for(let f=0,g;f<e;f++)if(g=a[f])d[f]=g;if(d.length){b.push(d);return}}if(!c)return d}
...
0;t<u.length;t++)r.push(u[t]);else m[n]=u;t=1}}else m=l,t=1;t&&e.put(m,f+":"+h)})}}}),a.store?await this.transaction("reg","readwrite",function(e){for(const d of a.store){const f=d[0],g=d[1];e.put("object"===typeof g?JSON.stringify(g):1,f)}}):a.bypass||await this.transaction("reg","readwrite",function(e){for(const d of a.reg.keys())e.put(1,d)}),a.tag&&await this.transaction("tag","readwrite",function(e){for(const d of a.tag){const f=d[0],g=d[1];g.length&&(e.get(f).onsuccess=function(){let k=this.result;
