...
if(!d)if(r)continue;else return e;t.push(d.db.tag(n[q+1],b,l,!1))}const y=this;return Promise.all(t).then(function(w){return w.length?y.search(a,b,c,w):w})}if(!m)return e;if(g&&(!q||!this.store))return e[0];t=[];for(let y=0,w;y<f.length;y++){w=e[y];q&&w.length&&!w[0].doc&&(this.db?t.push(w=this.index.get(this.field[0]).db.enrich(w)):w.length&&(w=Ea.call(this,w)));if(g)return w;e[y]={field:f[y],result:w}}if(q&&this.db&&t.length){const y=this;return Promise.all(t).then(function(w){for(let A=0;A<w.length;A++)e[A].result=
w[A];return h?Fa(e,b):p?Ga(e,a,y.index,y.field,y.D,p):e})}return h?Fa(e,b):p?Ga(e,a,this.index,this.field,this.D,p):e};
