...
[],d=G(),f,g,k=this.split||""===this.split?a.split(this.split):[a];for(let l=0,m,q;l<k.length;l++)if((m=q=k[l])&&!(m.length<this.minlength||m.length>this.maxlength)){if(b){if(d[m])continue;d[m]=1}else{if(f===m)continue;f=m}if(c)e.push(m);else if(!this.filter||("function"===typeof this.filter?this.filter(m):!this.filter.has(m))){if(this.cache&&m.length<=this.L)if(this.H){var h=this.G.get(m);if(h||""===h){h&&e.push(h);continue}}else this.H=setTimeout(N,50,this);if(this.stemmer){this.N||(this.N=new RegExp("(?!^)("+
this.A+")$"));let p;for(;p!==m&&2<m.length;)p=m,m=m.replace(this.N,t=>this.stemmer.get(t))}if(m&&(this.mapper||this.dedupe&&1<m.length)){h="";for(let p=0,t="",r,u;p<m.length;p++)r=m.charAt(p),r===t&&this.dedupe||((u=this.mapper&&this.mapper.get(r))||""===u?u===t&&this.dedupe||!(t=u)||(h+=u):h+=t=r);m=h}this.matcher&&1<m.length&&(this.M||(this.M=new RegExp("("+this.h+")","g")),m=m.replace(this.M,p=>this.matcher.get(p)));if(m&&this.replacer)for(h=0;m&&h<this.replacer.length;h+=2)m=m.replace(this.replacer[h],
this.replacer[h+1]);this.cache&&q.length<=this.L&&(this.G.set(q,m),this.G.size>this.S&&(this.G.clear(),this.L=this.L/1.1|0));if(m){if(m!==q)if(b){if(d[m])continue;d[m]=1}else{if(g===m)continue;g=m}e.push(m)}}}this.finalize&&(e=this.finalize(e)||e);this.cache&&a.length<=this.K&&(this.B.set(a,e),this.B.size>this.S&&(this.B.clear(),this.K=this.K/1.1|0));return e};function N(a){a.H=null;a.B.clear();a.G.clear()};let pa,qa;async function ra(a){a=a.data;var b=a.task;const c=a.id;let e=a.args;switch(b){case "init":qa=a.options||{};(b=a.factory)?(Function("return "+b)()(self),pa=new self.FlexSearch.Index(qa),delete self.FlexSearch):pa=new O(qa);postMessage({id:c});break;default:let d;"export"===b&&(e[1]?(e[0]=qa.export,e[2]=0,e[3]=1):e=null);"import"===b?e[0]&&(a=await qa.import.call(pa,e[0]),pa.import(e[0],a)):(d=e&&pa[b].apply(pa,e))&&d.then&&(d=await d);postMessage("search"===b?{id:c,msg:d}:{id:c})}};function sa(a){ta.call(a,"add");ta.call(a,"append");ta.call(a,"search");ta.call(a,"update");ta.call(a,"remove");ta.call(a,"searchCache")}let ua,xa,ya;function za(){ua=ya=0}
...
f.I;if(g&&!g(b))continue;f.constructor===String&&(f=""+f);f=ea(b,f)}if(d&&f){L(f)&&(f=[f]);for(let h=0,l,m;h<f.length;h++)if(l=f[h],!k[l]&&(k[l]=1,(g=d.get(l))?m=g:d.set(l,m=[]),!c||!m.includes(a))){if(m.length===2**31-1){g=new R(m);if(this.fastupdate)for(let q of this.reg.values())q.includes(m)&&(q[q.indexOf(m)]=g);d.set(l,m=g)}m.push(a);this.fastupdate&&((g=this.reg.get(a))?g.push(m):this.reg.set(a,[m]))}}}if(this.store&&(!c||!this.store.has(a))){let k;if(this.C){k=G();for(let h=0,l;h<this.C.length;h++){l=
...
function $a(a,b,c,e,d){const f=[],g=G();let k=0;for(let h=0,l;h<a.length;h++)if(l=a[h]){k<l.length&&(k=l.length);for(let m=0,q;m<l.length;m++)if(q=l[m])for(let p=0,t;p<q.length;p++)t=q[p],g[t]=g[t]?2:1}for(let h=0,l,m=0;h<k;h++)for(let q=0,p;q<a.length;q++)if(p=a[q])if(l=p[h])for(let t=0,r;t<l.length;t++)if(r=l[t],1===g[r])if(c)c--;else if(e){if(f.push(r),f.length===b)return f}else{const u=h+(q?d:0);f[u]||(f[u]=[]);f[u].push(r);if(++m===b)return f}return f};Y.prototype.not=function(){const {O:a,P:b,limit:c,offset:e,enrich:d,resolve:f,suggest:g}=Wa(this,"not",arguments);return ab.call(this,a,b,c,e,d,f,g)};function ab(a,b,c,e,d,f,g){if(b.length){const k=this;return Promise.all(b).then(function(h){a=[];for(let l=0,m;l<h.length;l++)(m=h[l]).length&&(a[l]=m);return ab.call(k,a,[],c,e,d,f,g)})}if(a.length&&this.result.length)this.result=bb.call(this,a,c,e,f);else if(f)return this.resolve(c,e,d);return f?d?X.call(this.index,this.result):this.result:this}
...
Y.prototype.offset=function(a){if(this.result.length){const b=[];for(let c=0,e;c<this.result.length;c++)if(e=this.result[c])e.length<=a?a-=e.length:(b[c]=e.slice(a),a=0);this.result=b}return this};Y.prototype.boost=function(a){this.h+=a;return this};Y.prototype.resolve=function(a,b,c){const e=this.result,d=this.index;this.result=this.index=null;return e.length?("object"===typeof a&&(c=a.enrich,b=a.offset,a=a.limit),Va.call(d,e,a||100,b,c)):e};G();function cb(a,b,c,e,d){let f,g,k;"string"===typeof d?(f=d,d=""):f=d.template;g=f.indexOf("$1");k=f.substring(g+2);g=f.substring(0,g);let h=d&&d.boundary,l=!d||!1!==d.clip,m=d&&d.merge&&k&&g&&new RegExp(k+" "+g,"g");d=d&&d.ellipsis;var q=0;if("object"===typeof d){var p=d.template;q=p.length-2;d=d.pattern}"string"!==typeof d&&(d=!1===d?"":"...");q&&(d=p.replace("$1",d));p=d.length-q;let t,r;"object"===typeof h&&(t=h.before,0===t&&(t=-1),r=h.after,0===r&&(r=-1),h=h.total||9E5);q=new Map;for(let Ka=0,
aa,eb,na;Ka<b.length;Ka++){let oa;if(e)oa=b,na=e;else{var u=b[Ka];na=u.field;if(!na)continue;oa=u.result}eb=c.get(na);aa=eb.encoder;u=q.get(aa);"string"!==typeof u&&(u=aa.encode(a),q.set(aa,u));for(let va=0;va<oa.length;va++){var n=oa[va].doc;if(!n)continue;n=ea(n,na);if(!n)continue;var y=n.trim().split(/\s+/);if(!y.length)continue;n="";var x=[];let wa=[];var J=-1,B=-1,z=0;for(var A=0;A<y.length;A++){var D=y[A],C=aa.encode(D);C=1<C.length?C.join(" "):C[0];let w;if(C&&D){var E=D.length,H=(aa.split?
D.replace(aa.split,""):D).length-C.length,M="",V=0;for(var ba=0;ba<u.length;ba++){var Q=u[ba];if(Q){var K=Q.length;K+=H;V&&K<=V||(Q=C.indexOf(Q),-1<Q&&(M=(Q?D.substring(0,Q):"")+g+D.substring(Q,Q+K)+k+(Q+K<E?D.substring(Q+K):""),V=K,w=!0))}}M&&(h&&(0>J&&(J=n.length+(n?1:0)),B=n.length+(n?1:0)+M.length,z+=E,wa.push(x.length),x.push({match:M})),n+=(n?" ":"")+M)}if(!w)D=y[A],n+=(n?" ":"")+D,h&&x.push({text:D});else if(h&&z>=h)break}z=wa.length*(f.length-2);if(t||r||h&&n.length-z>h)if(z=h+z-2*p,A=B-J,
0<t&&(A+=t),0<r&&(A+=r),A<=z)y=t?J-(0<t?t:0):J-((z-A)/2|0),x=r?B+(0<r?r:0):y+z,l||(0<y&&" "!==n.charAt(y)&&" "!==n.charAt(y-1)&&(y=n.indexOf(" ",y),0>y&&(y=0)),x<n.length&&" "!==n.charAt(x-1)&&" "!==n.charAt(x)&&(x=n.lastIndexOf(" ",x),x<B?x=B:++x)),n=(y?d:"")+n.substring(y,x)+(x<n.length?d:"");else{B=[];J={};z={};A={};D={};C={};M=H=E=0;for(ba=V=1;;){var S=void 0;for(let w=0,I;w<wa.length;w++){I=wa[w];if(M)if(H!==M){if(A[w+1])continue;I+=M;if(J[I]){E-=p;z[w+1]=1;A[w+1]=1;continue}if(I>=x.length-1){if(I>=
x.length){A[w+1]=1;I>=y.length&&(z[w+1]=1);continue}E-=p}n=x[I].text;if(K=r&&C[w])if(0<K){if(n.length>K)if(A[w+1]=1,l)n=n.substring(0,K);else continue;(K-=n.length)||(K=-1);C[w]=K}else{A[w+1]=1;continue}if(E+n.length+1<=h)n=" "+n,B[w]+=n;else if(l)S=h-E-1,0<S&&(n=" "+n.substring(0,S),B[w]+=n),A[w+1]=1;else{A[w+1]=1;continue}}else{if(A[w])continue;I-=H;if(J[I]){E-=p;A[w]=1;z[w]=1;continue}if(0>=I){if(0>I){A[w]=1;z[w]=1;continue}E-=p}n=x[I].text;if(K=t&&D[w])if(0<K){if(n.length>K)if(A[w]=1,l)n=n.substring(n.length-
K);else continue;(K-=n.length)||(K=-1);D[w]=K}else{A[w]=1;continue}if(E+n.length+1<=h)n+=" ",B[w]=n+B[w];else if(l)S=n.length+1-(h-E),0<=S&&S<n.length&&(n=n.substring(S)+" ",B[w]=n+B[w]),A[w]=1;else{A[w]=1;continue}}else{n=x[I].match;t&&(D[w]=t);r&&(C[w]=r);w&&E++;let La;I?!w&&p&&(E+=p):(z[w]=1,A[w]=1);I>=y.length-1?La=1:I<x.length-1&&x[I+1].match?La=1:p&&(E+=p);E-=f.length-2;if(!w||E+n.length<=h)B[w]=n;else{S=V=ba=z[w]=0;break}La&&(z[w+1]=1,A[w+1]=1)}E+=n.length;S=J[I]=1}if(S)H===M?M++:H++;else{H===
M?V=0:ba=0;if(!V&&!ba)break;V?(H++,M=H):M++}}n="";for(let w=0,I;w<B.length;w++)I=(w&&z[w]?" ":(w&&!d?" ":"")+d)+B[w],n+=I;d&&!z[B.length]&&(n+=d)}m&&(n=n.replace(m," "));oa[va].highlight=n}if(e)break}return b};W.prototype.search=function(a,b,c,e){c||(!b&&ca(a)?(c=a,a=""):ca(b)&&(c=b,b=0));if(c&&c.cache){c.cache=!1;var d=this.searchCache(a,b,c);c.cache=!0;return d}let f=[];var g=[];let k,h;let l,m;let q=0;var p=!0;let t;if(c){c.constructor===Array&&(c={index:c});a=c.query||a;k=c.pluck;h=c.merge;l=k||c.field||(l=c.index)&&(l.index?null:l);m=this.tag&&c.tag;var r=c.suggest;p=!1!==c.resolve;p||k||!(l=l||this.field)||(L(l)?k=l:(l.constructor===Array&&1===l.length&&(l=l[0]),k=l.field||l.index));t=p&&this.store&&
c.highlight;d=!!t||p&&this.store&&c.enrich;b=c.limit||b;var u=c.offset||0;b||(b=100);if(m&&(!this.db||!e)){m.constructor!==Array&&(m=[m]);var n=[];for(let B=0,z;B<m.length;B++)if(z=m[B],z.field&&z.tag){var y=z.tag;if(y.constructor===Array)for(var x=0;x<y.length;x++)n.push(z.field,y[x]);else n.push(z.field,y)}else{y=Object.keys(z);for(let A=0,D,C;A<y.length;A++)if(D=y[A],C=z[D],C.constructor===Array)for(x=0;x<C.length;x++)n.push(D,C[x]);else n.push(D,C)}m=n;if(!a){p=[];if(n.length)for(g=0;g<n.length;g+=
2){if(this.db){r=this.index.get(n[g]);if(!r)continue;p.push(r=r.db.tag(n[g+1],b,u,d))}else r=db.call(this,n[g],n[g+1],b,u,d);f.push({field:n[g],tag:n[g+1],result:r})}return p.length?Promise.all(p).then(function(B){for(let z=0;z<B.length;z++)f[z].result=B[z];return f}):f}}l&&l.constructor!==Array&&(l=[l])}l||(l=this.field);let J;n=(this.worker||this.db)&&!e&&[];for(let B=0,z,A,D;B<l.length;B++){A=l[B];if(this.db&&this.tag&&!this.F[B])continue;let C;L(A)||(C=A,A=C.field,a=C.query||a,b=fb(C.limit,b),
u=fb(C.offset,u),r=fb(C.suggest,r),t=p&&this.store&&fb(C.highlight,t),d=!!t||p&&this.store&&fb(C.enrich,d));if(e)z=e[B];else if(y=C||c,x=this.index.get(A),m&&(this.db&&(y.tag=m,J=x.db.support_tag_search,y.field=l),J||(y.enrich=!1)),n){n[B]=x.search(a,b,y);y&&d&&(y.enrich=d);continue}else z=x.search(a,b,y),y&&d&&(y.enrich=d);D=z&&(p?z.length:z.result.length);if(m&&D){y=[];x=0;if(this.db&&e){if(!J)for(let E=l.length;E<e.length;E++){let H=e[E];if(H&&H.length)x++,y.push(H);else if(!r)return p?f:new Y(f)}}else for(let E=
0,H,M;E<m.length;E+=2){H=this.tag.get(m[E]);if(!H)if(r)continue;else return p?f:new Y(f);if(M=(H=H&&H.get(m[E+1]))&&H.length)x++,y.push(H);else if(!r)return p?f:new Y(f)}if(x){z=Ua(z,y,p);D=z.length;if(!D&&!r)return p?z:new Y(z);x--}}if(D)g[q]=A,f.push(z),q++;else if(1===l.length)return p?f:new Y(f)}if(n){if(this.db&&m&&m.length&&!J)for(d=0;d<m.length;d+=2){g=this.index.get(m[d]);if(!g)if(r)continue;else return p?f:new Y(f);n.push(g.db.tag(m[d+1],b,u,!1))}const B=this;return Promise.all(n).then(function(z){return z.length?
B.search(a,b,c,z):z})}if(!q)return p?f:new Y(f);if(k&&(!d||!this.store))return f[0];n=[];for(u=0;u<g.length;u++){r=f[u];d&&r.length&&"undefined"===typeof r[0].doc&&(this.db?n.push(r=this.index.get(this.field[0]).db.enrich(r)):r=X.call(this,r));if(k)return p?t?cb(a,r,this.index,k,t):r:new Y(r);f[u]={field:g[u],result:r}}if(d&&this.db&&n.length){const B=this;return Promise.all(n).then(function(z){for(let A=0;A<z.length;A++)f[A].result=z[A];return h?gb(f):t?cb(a,f,B.index,k,t):f})}return h?gb(f):t?cb(a,
f,this.index,k,t):f};function fb(a,b){return"undefined"===typeof a?b:a}function gb(a){const b=[],c=G();for(let e=0,d,f;e<a.length;e++){d=a[e];f=d.result;for(let g=0,k,h,l;g<f.length;g++)h=f[g],"object"!==typeof h&&(h={id:h}),k=h.id,(l=c[k])?l.push(d.field):(h.field=c[k]=[d.field],b.push(h))}return b}function db(a,b,c,e,d){a=this.tag.get(a);if(!a)return[];if((b=(a=a&&a.get(b))&&a.length-e)&&0<b){if(b>c||e)a=a.slice(e,e+c);d&&(a=X.call(this,a));return a}}
...
d.push(f);if(d.length){const f=this;return Promise.all(d).then(function(g){const k=new Map;let h=0;for(const m of f.index.entries()){const q=m[0];var l=m[1];if(l.then){l=d[h].encoder||{};let p=k.get(l);p||(p=l.encode?l:new ma(l),k.set(l,p));l=g[h];l.encoder=p;f.index.set(q,l);h++}}return f})}}else a.db&&(this.fastupdate=!1,this.mount(a.db))}v=W.prototype;
...
function sb(a,b){let c=0;var e="undefined"===typeof b;if(a.constructor===Array)for(let d=0,f,g;d<a.length;d++){if((f=a[d])&&f.length)if(e)c++;else if(g=f.indexOf(b),0<=g){1<f.length?(f.splice(g,1),c++):delete a[d];break}else c++}else for(let d of a.entries()){e=d[0];const f=sb(d[1],b);f?c+=f:a.delete(e)}return c};const ub={memory:{resolution:1},performance:{resolution:3,fastupdate:!0,context:{depth:1,resolution:1}},match:{tokenize:"forward"},score:{resolution:9,context:{depth:2,resolution:3}}};O.prototype.add=function(a,b,c,e){if(b&&(a||0===a)){if(!e&&!c&&this.reg.has(a))return this.update(a,b);e=this.depth;b=this.encoder.encode(b,!e);const l=b.length;if(l){const m=G(),q=G(),p=this.resolution;for(let t=0;t<l;t++){let r=b[this.rtl?l-1-t:t];var d=r.length;if(d&&(e||!q[r])){var f=this.score?this.score(b,r,t,null,0):vb(p,l,t),g="";switch(this.tokenize){case "full":if(2<d){for(let u=0,n;u<d;u++)for(f=d;f>u;f--){g=r.substring(u,f);n=this.rtl?d-1-u:u;var k=this.score?this.score(b,r,t,g,n):vb(p,
l,t,d,n);wb(this,q,g,k,a,c)}break}case "bidirectional":case "reverse":if(1<d){for(k=d-1;0<k;k--){g=r[this.rtl?d-1-k:k]+g;var h=this.score?this.score(b,r,t,g,k):vb(p,l,t,d,k);wb(this,q,g,h,a,c)}g=""}case "forward":if(1<d){for(k=0;k<d;k++)g+=r[this.rtl?d-1-k:k],wb(this,q,g,f,a,c);break}default:if(wb(this,q,r,f,a,c),e&&1<l&&t<l-1)for(d=G(),g=this.U,f=r,k=Math.min(e+1,this.rtl?t+1:l-t),d[f]=1,h=1;h<k;h++)if((r=b[this.rtl?l-1-t-h:t+h])&&!d[r]){d[r]=1;const u=this.score?this.score(b,f,t,r,h-1):vb(g+(l/
2>g?0:1),l,t,k-1,h-1),n=this.bidirectional&&r>f;wb(this,m,n?f:r,u,a,c,n?r:f)}}}}this.fastupdate||this.reg.add(a)}else b=""}this.db&&(b||this.commit_task.push({del:a}),this.T&&tb(this));return this};
...
function vb(a,b,c,e,d){return c&&1<a?b+(e||0)<=a?c+(d||0):(a-1)/(b+(e||0))*(c+(d||0))+1|0:0};O.prototype.search=function(a,b,c){c||(b||"object"!==typeof a?"object"===typeof b&&(c=b,b=0):(c=a,a=""));if(c&&c.cache)return c.cache=!1,a=this.searchCache(a,b,c),c.cache=!0,a;let e=[],d,f,g,k=0,h,l,m,q,p;c&&(a=c.query||a,b=c.limit||b,k=c.offset||0,f=c.context,g=c.suggest,p=(h=c.resolve)&&c.enrich,m=c.boost,q=c.resolution,l=this.db&&c.tag);"undefined"===typeof h&&(h=this.resolve);f=this.depth&&!1!==f;let t=this.encoder.encode(a,!f);d=t.length;b=b||(h?100:0);if(1===d)return xb.call(this,t[0],"",b,
k,h,p,l);if(2===d&&f&&!g)return xb.call(this,t[1],t[0],b,k,h,p,l);let r=G(),u=0,n;f&&(n=t[0],u=1);q||0===q||(q=n?this.U:this.resolution);if(this.db){if(this.db.search&&(c=this.db.search(this,t,b,k,g,h,p,l),!1!==c))return c;const y=this;return async function(){for(let x,J;u<d;u++){if((J=t[u])&&!r[J]){r[J]=1;x=await yb(y,J,n,0,0,!1,!1);if(x=zb(x,e,g,q)){e=x;break}n&&(g&&x&&e.length||(n=J))}g&&n&&u===d-1&&!e.length&&(q=y.resolution,n="",u=-1,r=G())}return Ab(e,q,b,k,g,m,h)}()}for(let y,x;u<d;u++){if((x=
t[u])&&!r[x]){r[x]=1;y=yb(this,x,n,0,0,!1,!1);if(y=zb(y,e,g,q)){e=y;break}n&&(g&&y&&e.length||(n=x))}g&&n&&u===d-1&&!e.length&&(q=this.resolution,n="",u=-1,r=G())}return Ab(e,q,b,k,g,m,h)};function Ab(a,b,c,e,d,f,g){let k=a.length,h=a;if(1<k)h=Sa(a,b,c,e,d,f,g);else if(1===k)return g?Va.call(null,a[0],c,e):new Y(a[0]);return g?h:new Y(h)}
...
for(let m=0,q,p;m<l;m++)if((p=g[m])&&p.length){if((q=k[m])&&q.length)for(h=0;h<p.length;h++)q.push(p[h]);else k[m]=p;h=1}}else k=g,h=1;h&&e.put(k,f)})}}),await this.transaction("ctx","readwrite",function(e){for(const d of a.ctx){const f=d[0],g=d[1];for(const k of g){const h=k[0],l=k[1];l.length&&(b?e.put(l,f+":"+h):e.get(f+":"+h).onsuccess=function(){let m=this.result;var q;if(m&&m.length){const p=Math.max(m.length,l.length);for(let t=0,r,u;t<p;t++)if((u=l[t])&&u.length){if((r=m[t])&&r.length)for(q=
0;q<u.length;q++)r.push(u[q]);else m[t]=u;q=1}}else m=l,q=1;q&&e.put(m,f+":"+h)})}}}),a.store?await this.transaction("reg","readwrite",function(e){for(const d of a.store){const f=d[0],g=d[1];e.put("object"===typeof g?JSON.stringify(g):1,f)}}):a.bypass||await this.transaction("reg","readwrite",function(e){for(const d of a.reg.keys())e.put(1,d)}),a.tag&&await this.transaction("tag","readwrite",function(e){for(const d of a.tag){const f=d[0],g=d[1];g.length&&(e.get(f).onsuccess=function(){let k=this.result;
