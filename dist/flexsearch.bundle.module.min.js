...
function E(a={}){if(!(this instanceof E))return new E(...arguments);for(a=0;a<arguments.length;a++)this.assign(arguments[a])}
...
this.V);ka&&(this.G=new Map(ka));this.rtl=a.rtl||!1;this.C=y(a.C,!0,this.C);this.filter=y((c=a.filter)&&new Set(c),null,this.filter);this.J=y((c=a.J)&&new Map(c),null,this.J);this.G=y((c=a.G)&&new Map(c),null,this.G);this.N=y((c=a.N)&&new Map(c),null,this.N);this.K=y(a.K,null,this.K);this.Z=y(a.Z,1,this.Z);this.ha=y(a.ha,0,this.ha);if(this.cache=c=y(a.cache,!0,this.cache))this.X=null,this.ea="number"===typeof c?c:2E5,this.P=new Map,this.U=new Map,this.B=this.h=128;this.D="";this.ba=null;this.Y="";
this.ca=null;if(this.J)for(const d of this.J.keys())this.D+=(this.D?"|":"")+d;if(this.N)for(const d of this.N.keys())this.Y+=(this.Y?"|":"")+d;return this};
E.prototype.encode=function(a){if(this.cache&&a.length<=this.h)if(this.X){if(this.P.has(a))return this.P.get(a)}else this.X=setTimeout(la,0,this);this.normalize&&(a="function"===typeof this.normalize?this.normalize(a):ja?a.normalize("NFKD").replace(ja,"").toLowerCase():a.toLowerCase());this.$&&(a=this.$(a));this.numeric&&3<a.length&&(a=a.replace(ha,"$1 $2").replace(ia,"$1 $2").replace(fa,"$1 "));const b=!(this.C||this.G||this.filter||this.J||this.N||this.K);let c=[],d=this.split||""===this.split?
a.split(this.split):a;for(let f=0,g,h;f<d.length;f++){if(!(g=h=d[f]))continue;if(g.length<this.Z)continue;if(b){c.push(g);continue}if(this.filter&&this.filter.has(g))continue;if(this.cache&&g.length<=this.B)if(this.X){var e=this.U.get(g);if(e||""===e){e&&c.push(e);continue}}else this.X=setTimeout(la,0,this);let k;this.N&&2<g.length&&(this.ca||(this.ca=new RegExp("(?!^)("+this.Y+")$")),g=g.replace(this.ca,l=>this.N.get(l)),k=1);this.J&&1<g.length&&(this.ba||(this.ba=new RegExp("("+this.D+")","g")),
...
k.limit||0;f=k.offset||0;g=k.resolve;continue}d[h]=l;l instanceof Promise&&a.push(l)}if(a.length)return Promise.all(a).then(function(){d=[b.result].concat(d);b.result=xa(d,e,f,g,b.O);return g?b.result:b});d=[this.result].concat(d);this.result=xa(d,e,f,g,b.O);return g?this.result:this}return this};
...
0;g=l.enrich;h=l.resolve;continue}d[k]=m;m instanceof Promise&&c.push(m)}if(c.length)return Promise.all(c).then(function(){a.result.length&&(d=[a.result].concat(d));a.result=ya(d,e,f,g,!h,a.O);return h?a.result:a});this.result.length&&(d=[this.result].concat(d));this.result=ya(d,e,f,g,!h,a.O);return h?this.result:this};
...
h[p]=1}}return f}function Ba(a,b){const c=z(),d=z(),e=[];for(let f=0;f<a.length;f++)c[a[f]]=1;for(let f=0,g;f<b.length;f++){g=b[f];for(let h=0,k;h<g.length;h++)k=g[h],c[k]&&!d[k]&&(d[k]=1,e.push(k))}return e};let T=1;
...
d.length;M++){let G=d[M];if(G&&G.length)x++,r.push(G);else if(!t)return e}}else for(let M=0,G,Wa;M<n.length;M+=2){G=this.tag.get(n[M]);if(!G)if(t)continue;else return e;if(Wa=(G=G&&G.get(n[M+1]))&&G.length)x++,r.push(G);else if(!t)return e}if(x){v=Ba(v,r);J=v.length;if(!J&&!t)return e;x--}}if(J)f[m]=I,e.push(v),m++;else if(1===k.length)return e}if(p){if(this.db&&n&&n.length&&!A)for(q=0;q<n.length;q+=2){d=this.index.get(n[q]);if(!d)if(t)continue;else return e;p.push(d.db.tag(n[q+1],b,l,!1))}const w=
