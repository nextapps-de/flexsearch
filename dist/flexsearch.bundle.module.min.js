...
["\u0477","\u0475"],["\u04c2","\u0436"],["\u04d1","\u0430"],["\u04d3","\u0430"],["\u04d7","\u0435"],["\u04db","\u04d9"],["\u04dd","\u0436"],["\u04df","\u0437"],["\u04e3","\u0438"],["\u04e5","\u0438"],["\u04e7","\u043e"],["\u04eb","\u04e9"],["\u04ed","\u044d"],["\u04ef","\u0443"],["\u04f1","\u0443"],["\u04f3","\u0443"],["\u04f5","\u0447"]];const ea=/[^\p{L}\p{N}]+/u,fa=/(\d{3})/g,ha=/(\D)(\d{3})/g,ia=/(\d{3})(\D)/g,ja="".normalize&&/[\u0300-\u036f]/g;function K(a){if(!this||this.constructor!==K)return new K(...arguments);for(let b=0;b<arguments.length;b++)this.assign(arguments[b])}
...
t.push(d.db.tag(n[q+1],b,l,!1))}const y=this;return Promise.all(t).then(function(w){return w.length?y.search(a,b,c,w):w})}if(!m)return e;if(g&&(!q||!this.store))return e[0];t=[];for(let y=0,w;y<f.length;y++){w=e[y];q&&w.length&&!w[0].doc&&(this.db?t.push(w=this.index.get(this.field[0]).db.enrich(w)):w.length&&(w=Ea.call(this,w)));if(g)return w;e[y]={field:f[y],result:w}}if(q&&this.db&&t.length){const y=this;return Promise.all(t).then(function(w){for(let A=0;A<w.length;A++)e[A].result=w[A];return h?
Fa(e,b):p?Ga(e,a,y.index,y.field,y.D,p):e})}return h?Fa(e,b):p?Ga(e,a,this.index,this.field,this.D,p):e};
...
v.set=function(a,b){this.store.set(a,b);return this};v.searchCache=Ja;
...
this.not(l.not);else continue;d[k]=m;m.then&&c.push(m)}if(c.length)return Promise.all(c).then(function(){a.result.length&&(d=[a.result].concat(d));a.result=Za(d,e,f,g,!h,a.F);return h?a.result:a});d.length&&(this.result.length&&(d=[this.result].concat(d)),this.result=Za(d,e,f,g,!h,a.F));return h?this.result:this};
...
else continue;d[h]=l;l.then&&c.push(l)}if(c.length)return Promise.all(c).then(function(){a.result=$a.call(a,d,e,f,g);return g?a.result:a});d.length&&(this.result=$a.call(this,d,e,f,g));return g?this.result:this};
...
L.prototype.search=function(a,b,c){c||(!b&&G(a)?(c=a,a=""):G(b)&&(c=b,b=0));let d=[],e;let f,g=0,h,k,l,m;if(c){a=c.query||a;b=c.limit||b;g=c.offset||0;var p=c.context;f=c.suggest;(h=ab&&!1!==c.resolve)||(ab=0);k=h&&c.enrich;m=c.boost;l=this.db&&c.tag}else h=this.resolve||ab;a=this.encoder.encode(a);e=a.length;b||!h||(b=100);if(1===e)return bb.call(this,a[0],"",b,g,h,k,l);p=this.depth&&!1!==p;if(2===e&&p&&!f)return bb.call(this,a[0],a[1],b,g,h,k,l);let n=c=0;if(1<e){const t=B(),u=[];for(let x=0,D;x<
e;x++)if((D=a[x])&&!t[D]){if(f||this.db||Y(this,D))u.push(D),t[D]=1;else return h?d:new X(d);const y=D.length;c=Math.max(c,y);n=n?Math.min(n,y):y}a=u;e=a.length}if(!e)return h?d:new X(d);let q=0,r;if(1===e)return bb.call(this,a[0],"",b,g,h,k,l);if(2===e&&p&&!f)return bb.call(this,a[0],a[1],b,g,h,k,l);1<e&&(p?(r=a[0],q=1):9<c&&3<c/n&&a.sort(aa));if(this.db){if(this.db.search&&(p=this.db.search(this,a,b,g,f,h,k,l),!1!==p))return p;const t=this;return async function(){for(let u,x;q<e;q++){x=a[q];r?(u=
...
v.commit=function(a,b){this.commit_timer&&(clearTimeout(this.commit_timer),this.commit_timer=null);return this.db.commit(this,a,b)};v.destroy=function(){this.commit_timer&&(clearTimeout(this.commit_timer),this.commit_timer=null);return this.db.destroy()};function Va(a){a.commit_timer||(a.commit_timer=setTimeout(function(){a.commit_timer=null;a.db.commit(a,void 0,void 0)},0))}
...
function fb(a){let b=0;if(a.constructor===Array)for(let c=0,d;c<a.length;c++)(d=a[c])&&(b+=d.length);else for(const c of a){const d=c[0],e=fb(c[1]);e?b+=e:a.delete(d)}return b}v.cleanup=function(){if(!this.fastupdate)return this;fb(this.map);this.depth&&fb(this.ctx);return this};v.searchCache=Ja;
v.export=function(a,b,c,d=0){let e,f;switch(d){case 0:e="reg";f=sa(this.reg);break;case 1:e="cfg";f={};break;case 2:e="map";f=oa(this.map,this.reg.size);break;case 3:e="ctx";f=qa(this.ctx,this.reg.size);break;default:return}return ua.call(this,a,b,e,f,c,d)};
v.import=function(a,b){if(b)switch(F(b)&&(b=JSON.parse(b)),a=a.split("."),"json"===a[a.length-1]&&a.pop(),a=1<a.length?a[1]:a[0],a){case "reg":this.fastupdate=!1;this.reg=ta(b,this.reg);break;case "map":this.map=pa(b,this.map);break;case "ctx":this.ctx=ra(b,this.ctx)}};
...
function ib(a,b={}){if(!this)return new ib(a,b);"object"===typeof a&&(b=a=a.name);a||console.info("Default storage space was used, because a name was not passed.");this.id="flexsearch"+(a?":"+a.toLowerCase().replace(/[^a-z0-9_\-]/g,""):"");this.field=b.field?b.field.toLowerCase().replace(/[^a-z0-9_\-]/g,""):"";this.support_tag_search=!1;this.db=null;this.h={}}v=ib.prototype;v.mount=function(a){if(!a.encoder)return a.mount(this);a.db=this;return this.open()};
...
v.close=function(){this.db.close();this.db=null};v.destroy=function(){return gb.deleteDatabase(this.id+(this.field?":"+this.field:""))};v.clear=function(){const a=this.db.transaction(hb,"readwrite");for(let b=0;b<hb.length;b++)a.objectStore(hb[b]).clear();return Z(a)};
...
v.transaction=function(a,b,c){let d=this.h[a+":"+b];if(d)return c.call(this,d);let e=this.db.transaction(a,b);this.h[a+":"+b]=d=e.objectStore(a);return new Promise((f,g)=>{e.onerror=h=>{this.h[a+":"+b]=null;e.abort();e=d=null;g(h)};e.oncomplete=h=>{e=d=this.h[a+":"+b]=null;f(h||!0)};return c.call(this,d)})};
v.commit=async function(a,b,c){if(b)await this.clear(),a.commit_task=[];else{let d=a.commit_task;a.commit_task=[];for(let e=0,f;e<d.length;e++)if(f=d[e],f.clear){await this.clear();b=!0;break}else d[e]=f.W;b||(c||(d=d.concat(ba(a.reg))),d.length&&await this.remove(d))}a.reg.size&&(await this.transaction("map","readwrite",function(d){for(const e of a.map){const f=e[0],g=e[1];g.length&&(b?d.put(g,f):d.get(f).onsuccess=function(){let h=this.result;var k;if(h&&h.length){const l=Math.max(h.length,g.length);
...
v.remove=function(a){"object"!==typeof a&&(a=[a]);return Promise.all([this.transaction("map","readwrite",function(b){b.openCursor().onsuccess=function(){const c=this.result;c&&jb(c,a)}}),this.transaction("ctx","readwrite",function(b){b.openCursor().onsuccess=function(){const c=this.result;c&&jb(c,a)}}),this.transaction("tag","readwrite",function(b){b.openCursor().onsuccess=function(){const c=this.result;c&&jb(c,a,!0)}}),this.transaction("reg","readwrite",function(b){for(let c=0;c<a.length;c++)b.delete(a[c])})])};
function Z(a){return new Promise((b,c)=>{a.onsuccess=function(){b(this.result)};a.oncomplete=function(){b(this.result)};a.onerror=c;a=null})};export default {Index:L,Charset:Sa,Encoder:K,Document:T,Worker:N,Resolver:X,IndexedDB:ib,Language:{}};
