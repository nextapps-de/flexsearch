/**!
 * FlexSearch.js v0.8.162 (Bundle/Module)
...
[],d=G(),f,g,k=this.split||""===this.split?a.split(this.split):[a];for(let l=0,m,t;l<k.length;l++)if((m=t=k[l])&&!(m.length<this.minlength||m.length>this.maxlength)){if(b){if(d[m])continue;d[m]=1}else{if(f===m)continue;f=m}if(c)e.push(m);else if(!this.filter||("function"===typeof this.filter?this.filter(m):!this.filter.has(m))){if(this.cache&&m.length<=this.L)if(this.H){var h=this.G.get(m);if(h||""===h){h&&e.push(h);continue}}else this.H=setTimeout(N,50,this);if(this.stemmer){this.N||(this.N=new RegExp("(?!^)("+
this.A+")$"));let q;for(;q!==m&&2<m.length;)q=m,m=m.replace(this.N,p=>this.stemmer.get(p))}if(m&&(this.mapper||this.dedupe&&1<m.length)){h="";for(let q=0,p="",r,u;q<m.length;q++)r=m.charAt(q),r===p&&this.dedupe||((u=this.mapper&&this.mapper.get(r))||""===u?u===p&&this.dedupe||!(p=u)||(h+=u):h+=p=r);m=h}this.matcher&&1<m.length&&(this.M||(this.M=new RegExp("("+this.h+")","g")),m=m.replace(this.M,q=>this.matcher.get(q)));if(m&&this.replacer)for(h=0;m&&h<this.replacer.length;h+=2)m=m.replace(this.replacer[h],
this.replacer[h+1]);this.cache&&t.length<=this.L&&(this.G.set(t,m),this.G.size>this.S&&(this.G.clear(),this.L=this.L/1.1|0));if(m){if(m!==t)if(b){if(d[m])continue;d[m]=1}else{if(g===m)continue;g=m}e.push(m)}}}this.finalize&&(e=this.finalize(e)||e);this.cache&&a.length<=this.K&&(this.B.set(a,e),this.B.size>this.S&&(this.B.clear(),this.K=this.K/1.1|0));return e};function N(a){a.H=null;a.B.clear();a.G.clear()};let pa,qa;async function ra(a){a=a.data;var b=a.task;const c=a.id;let e=a.args;switch(b){case "init":qa=a.options||{};(b=a.factory)?(Function("return "+b)()(self),pa=new self.FlexSearch.Index(qa),delete self.FlexSearch):pa=new O(qa);postMessage({id:c});break;default:let d;"export"===b&&(e[1]?(e[0]=qa.export,e[2]=0,e[3]=1):e=null);"import"===b?e[0]&&(a=await qa.import.call(pa,e[0]),pa.import(e[0],a)):(d=e&&pa[b].apply(pa,e))&&d.then&&(d=await d);postMessage("search"===b?{id:c,msg:d}:{id:c})}};function sa(a){ta.call(a,"add");ta.call(a,"append");ta.call(a,"search");ta.call(a,"update");ta.call(a,"remove");ta.call(a,"searchCache")}let ua,xa,ya;function za(){ua=ya=0}
...
function Ja(a,b,c,e,d,f,g=0){const k=e&&e.constructor===Array;var h=k?e.shift():e;if(!h)return this.export(a,b,d,f+1);if((h=a((b?b+".":"")+(g+1)+"."+c,JSON.stringify(h)))&&h.then){const l=this;return h.then(function(){return Ja.call(l,a,b,c,k?e:null,d,f,g+1)})}return Ja.call(this,a,b,c,k?e:null,d,f,g+1)}
...
f.I;if(g&&!g(b))continue;f.constructor===String&&(f=""+f);f=ea(b,f)}if(d&&f){L(f)&&(f=[f]);for(let h=0,l,m;h<f.length;h++)if(l=f[h],!k[l]&&(k[l]=1,(g=d.get(l))?m=g:d.set(l,m=[]),!c||!m.includes(a))){if(m.length===2**31-1){g=new R(m);if(this.fastupdate)for(let t of this.reg.values())t.includes(m)&&(t[t.indexOf(m)]=g);d.set(l,m=g)}m.push(a);this.fastupdate&&((g=this.reg.get(a))?g.push(m):this.reg.set(a,[m]))}}}if(this.store&&(!c||!this.store.has(a))){let k;if(this.C){k=G();for(let h=0,l;h<this.C.length;h++){l=
...
function Qa(a,b,c,e,d,f,g,k){if(a=a[g])if(e===b.length-1){if(a.constructor===Array){if(c[e]){for(b=0;b<a.length;b++)d.add(f,a[b],!0,!0);return}a=a.join(" ")}d.add(f,a,k,!0)}else if(a.constructor===Array)for(g=0;g<a.length;g++)Qa(a,b,c,e,d,f,g,k);else g=b[++e],Qa(a,b,c,e,d,f,g,k);else d.db&&d.remove(f)};function Sa(a,b,c,e,d,f,g){const k=a.length;let h=[],l,m;l=G();for(let t=0,q,p,r,u;t<b;t++)for(let n=0;n<k;n++)if(r=a[n],t<r.length&&(q=r[t]))for(let y=0;y<q.length;y++){p=q[y];(m=l[p])?l[p]++:(m=0,l[p]=1);u=h[m]||(h[m]=[]);if(!g){let x=t+(n||!d?0:f||0);u=u[x]||(u[x]=[])}u.push(p);if(g&&c&&m===k-1&&u.length-e===c)return e?u.slice(e):u}if(a=h.length)if(d)h=1<h.length?Ta(h,c,e,g,f):(h=h[0]).length>c||e?h.slice(e,c+e):h;else{if(a<k)return[];h=h[a-1];if(c||e)if(g){if(h.length>c||e)h=h.slice(e,c+e)}else{d=
[];for(let t=0,q;t<h.length;t++)if(q=h[t],q.length>e)e-=q.length;else{if(q.length>c||e)q=q.slice(e,c+e),c-=q.length,e&&(e-=q.length);d.push(q);if(!c)break}h=1<d.length?[].concat.apply([],d):d[0]}}return h}
function Ta(a,b,c,e,d){const f=[],g=G();let k;var h=a.length;let l;if(e)for(d=h-1;0<=d;d--){if(l=(e=a[d])&&e.length)for(h=0;h<l;h++)if(k=e[h],!g[k])if(g[k]=1,c)c--;else if(f.push(k),f.length===b)return f}else for(let m=h-1,t,q=0;0<=m;m--){t=a[m];for(let p=0;p<t.length;p++)if(l=(e=t[p])&&e.length)for(let r=0;r<l;r++)if(k=e[r],!g[k])if(g[k]=1,c)c--;else{let u=(p+(m<h-1?d||0:0))/(m+1)|0;(f[u]||(f[u]=[])).push(k);if(++q===b)return f}}return f}
function Ua(a,b,c){const e=G(),d=[];for(let f=0,g;f<b.length;f++){g=b[f];for(let k=0;k<g.length;k++)e[g[k]]=1}if(c)for(let f=0,g;f<a.length;f++)g=a[f],e[g]&&(d.push(g),e[g]=0);else for(let f=0,g,k;f<a.result.length;f++)for(g=a.result[f],b=0;b<g.length;b++)k=g[b],e[k]&&((d[f]||(d[f]=[])).push(k),e[k]=0);return d};function Va(a,b,c,e){if(!a.length)return a;if(1===a.length)return a=a[0],a=c||a.length>b?b?a.slice(c,c+b):a.slice(c):a,e?X.call(this,a):a;let d=[];for(let f=0,g,k;f<a.length;f++)if((g=a[f])&&(k=g.length)){if(c){if(c>=k){c-=k;continue}c<k&&(g=b?g.slice(c,c+b):g.slice(c),k=g.length,c=0)}k>b&&(g=g.slice(0,b),k=b);if(!d.length&&k>=b)return e?X.call(this,g):g;d.push(g);b-=k;if(!b)break}d=1<d.length?[].concat.apply([],d):d[0];return e?X.call(this,d):d};function Wa(a,b,c){var e=c[0];if(e.then)return Promise.all(c).then(function(q){return a[b].apply(a,q)});if(e[0]&&e[0].index)return a[b].apply(a,e);e=[];let d=[],f=0,g=0,k,h,l,m,t;for(let q=0,p;q<c.length;q++)if(p=c[q]){let r;if(p.constructor===Y)r=p.result;else if(p.constructor===Array)r=p;else if(f=p.limit||0,g=p.offset||0,l=p.suggest,h=p.resolve,k=(m=p.highlight&&h)||p.enrich&&h,p.index)p.resolve=!1,r=p.index.search(p).result,p.resolve=h,m&&(t=p.search);else if(p.and)r=a.and(p.and);else if(p.or)r=
a.or(p.or);else if(p.xor)r=a.xor(p.xor);else if(p.not)r=a.not(p.not);else continue;if(r.then)d.push(r);else if(r.length)e[q]=r;else if(!l&&("and"===b||"xor"===b)){e=[];break}}return{O:e,P:d,limit:f,offset:g,enrich:k,resolve:h,suggest:l,highlight:m,W:t}};Y.prototype.or=function(){const {O:a,P:b,limit:c,offset:e,enrich:d,resolve:f}=Wa(this,"or",arguments);return Xa.call(this,a,b,c,e,d,f)};function Xa(a,b,c,e,d,f){if(b.length){const g=this;return Promise.all(b).then(function(k){a=[];for(let h=0,l;h<k.length;h++)(l=k[h]).length&&(a[h]=l);return Xa.call(g,a,[],c,e,d,f)})}a.length&&(this.result.length&&a.push(this.result),2>a.length?this.result=a[0]:(this.result=Ta(a,c,e,!1,this.h),e=0));return f?this.resolve(c,e,d):this};Y.prototype.and=function(){let a=this.result.length,b,c,e,d;if(!a){const f=arguments[0];f&&(a=!!f.suggest,d=f.resolve,b=f.limit,c=f.offset,e=f.enrich&&d)}if(a){const {O:f,P:g,limit:k,offset:h,enrich:l,resolve:m,suggest:t}=Wa(this,"and",arguments);return Ya.call(this,f,g,k,h,l,m,t)}return d?this.resolve(b,c,e):this};
...
function $a(a,b,c,e,d){const f=[],g=G();let k=0;for(let h=0,l;h<a.length;h++)if(l=a[h]){k<l.length&&(k=l.length);for(let m=0,t;m<l.length;m++)if(t=l[m])for(let q=0,p;q<t.length;q++)p=t[q],g[p]=g[p]?2:1}for(let h=0,l,m=0;h<k;h++)for(let t=0,q;t<a.length;t++)if(q=a[t])if(l=q[h])for(let p=0,r;p<l.length;p++)if(r=l[p],1===g[r])if(c)c--;else if(e){if(f.push(r),f.length===b)return f}else{const u=h+(t?d:0);f[u]||(f[u]=[]);f[u].push(r);if(++m===b)return f}return f};Y.prototype.not=function(){const {O:a,P:b,limit:c,offset:e,enrich:d,resolve:f,suggest:g}=Wa(this,"not",arguments);return ab.call(this,a,b,c,e,d,f,g)};function ab(a,b,c,e,d,f,g){if(b.length){const k=this;return Promise.all(b).then(function(h){a=[];for(let l=0,m;l<h.length;l++)(m=h[l]).length&&(a[l]=m);return ab.call(k,a,[],c,e,d,f,g)})}if(a.length&&this.result.length)this.result=bb.call(this,a,c,e,f);else if(f)return this.resolve(c,e,d);return f?d?X.call(this.index,this.result):this.result:this}
function bb(a,b,c,e){const d=[];a=new Set(a.flat().flat());for(let f=0,g,k=0;f<this.result.length;f++)if(g=this.result[f])for(let h=0,l;h<g.length;h++)if(l=g[h],!a.has(l))if(c)c--;else if(e){if(d.push(l),d.length===b)return d}else if(d[f]||(d[f]=[]),d[f].push(l),++k===b)return d;return d};function cb(a,b,c,e,d){let f,g,k;"string"===typeof d?(f=d,d=""):f=d.template;g=f.indexOf("$1");k=f.substring(g+2);g=f.substring(0,g);let h=d&&d.boundary,l=!d||!1!==d.clip,m=d&&d.merge&&k&&g&&new RegExp(k+" "+g,"g");d=d&&d.ellipsis;var t=0;if("object"===typeof d){var q=d.template;t=q.length-2;d=d.pattern}"string"!==typeof d&&(d=!1===d?"":"...");t&&(d=q.replace("$1",d));q=d.length-t;let p,r;"object"===typeof h&&(p=h.before,0===p&&(p=-1),r=h.after,0===r&&(r=-1),h=h.total||9E5);t=new Map;for(let Ka=0,
aa,eb,na;Ka<b.length;Ka++){let oa;if(e)oa=b,na=e;else{var u=b[Ka];na=u.field;if(!na)continue;oa=u.result}eb=c.get(na);aa=eb.encoder;u=t.get(aa);"string"!==typeof u&&(u=aa.encode(a),t.set(aa,u));for(let va=0;va<oa.length;va++){var n=oa[va].doc;if(!n)continue;n=ea(n,na);if(!n)continue;var y=n.trim().split(/\s+/);if(!y.length)continue;n="";var x=[];let wa=[];var J=-1,B=-1,z=0;for(var A=0;A<y.length;A++){var D=y[A],C=aa.encode(D);C=1<C.length?C.join(" "):C[0];let w;if(C&&D){var E=D.length,H=(aa.split?
D.replace(aa.split,""):D).length-C.length,M="",V=0;for(var ba=0;ba<u.length;ba++){var Q=u[ba];if(Q){var K=Q.length;K+=H;V&&K<=V||(Q=C.indexOf(Q),-1<Q&&(M=(Q?D.substring(0,Q):"")+g+D.substring(Q,Q+K)+k+(Q+K<E?D.substring(Q+K):""),V=K,w=!0))}}M&&(h&&(0>J&&(J=n.length+(n?1:0)),B=n.length+(n?1:0)+M.length,z+=E,wa.push(x.length),x.push({match:M})),n+=(n?" ":"")+M)}if(!w)D=y[A],n+=(n?" ":"")+D,h&&x.push({text:D});else if(h&&z>=h)break}z=wa.length*(f.length-2);if(p||r||h&&n.length-z>h)if(z=h+z-2*q,A=B-J,
0<p&&(A+=p),0<r&&(A+=r),A<=z)y=p?J-(0<p?p:0):J-((z-A)/2|0),x=r?B+(0<r?r:0):y+z,l||(0<y&&" "!==n.charAt(y)&&" "!==n.charAt(y-1)&&(y=n.indexOf(" ",y),0>y&&(y=0)),x<n.length&&" "!==n.charAt(x-1)&&" "!==n.charAt(x)&&(x=n.lastIndexOf(" ",x),x<B?x=B:++x)),n=(y?d:"")+n.substring(y,x)+(x<n.length?d:"");else{B=[];J={};z={};A={};D={};C={};M=H=E=0;for(ba=V=1;;){var S=void 0;for(let w=0,I;w<wa.length;w++){I=wa[w];if(M)if(H!==M){if(A[w+1])continue;I+=M;if(J[I]){E-=q;z[w+1]=1;A[w+1]=1;continue}if(I>=x.length-1){if(I>=
x.length){A[w+1]=1;I>=y.length&&(z[w+1]=1);continue}E-=q}n=x[I].text;if(K=r&&C[w])if(0<K){if(n.length>K)if(A[w+1]=1,l)n=n.substring(0,K);else continue;(K-=n.length)||(K=-1);C[w]=K}else{A[w+1]=1;continue}if(E+n.length+1<=h)n=" "+n,B[w]+=n;else if(l)S=h-E-1,0<S&&(n=" "+n.substring(0,S),B[w]+=n),A[w+1]=1;else{A[w+1]=1;continue}}else{if(A[w])continue;I-=H;if(J[I]){E-=q;A[w]=1;z[w]=1;continue}if(0>=I){if(0>I){A[w]=1;z[w]=1;continue}E-=q}n=x[I].text;if(K=p&&D[w])if(0<K){if(n.length>K)if(A[w]=1,l)n=n.substring(n.length-
K);else continue;(K-=n.length)||(K=-1);D[w]=K}else{A[w]=1;continue}if(E+n.length+1<=h)n+=" ",B[w]=n+B[w];else if(l)S=n.length+1-(h-E),0<=S&&S<n.length&&(n=n.substring(S)+" ",B[w]=n+B[w]),A[w]=1;else{A[w]=1;continue}}else{n=x[I].match;p&&(D[w]=p);r&&(C[w]=r);w&&E++;let La;I?!w&&q&&(E+=q):(z[w]=1,A[w]=1);I>=y.length-1?La=1:I<x.length-1&&x[I+1].match?La=1:q&&(E+=q);E-=f.length-2;if(!w||E+n.length<=h)B[w]=n;else{S=V=ba=z[w]=0;break}La&&(z[w+1]=1,A[w+1]=1)}E+=n.length;S=J[I]=1}if(S)H===M?M++:H++;else{H===
M?V=0:ba=0;if(!V&&!ba)break;V?(H++,M=H):M++}}n="";for(let w=0,I;w<B.length;w++)I=(w&&z[w]?" ":(w&&!d?" ":"")+d)+B[w],n+=I;d&&!z[B.length]&&(n+=d)}m&&(n=n.replace(m," "));oa[va].highlight=n}if(e)break}return b};function Y(a){if(!this||this.constructor!==Y)return new Y(a);if(a&&a.index)return a.resolve=!1,this.index=a.index,this.h=a.boost||0,this.result=a.index.search(a).result,this;this.index=null;this.result=a||[];this.h=0}Y.prototype.limit=function(a){if(this.result.length){const b=[];for(let c=0,e;c<this.result.length;c++)if(e=this.result[c])if(e.length<=a){if(b[c]=e,a-=e.length,!a)break}else{b[c]=e.slice(0,a);break}this.result=b}return this};
Y.prototype.offset=function(a){if(this.result.length){const b=[];for(let c=0,e;c<this.result.length;c++)if(e=this.result[c])e.length<=a?a-=e.length:(b[c]=e.slice(a),a=0);this.result=b}return this};Y.prototype.boost=function(a){this.h+=a;return this};Y.prototype.resolve=function(a,b,c){const e=this.index;let d=this.result;this.result=this.index=null;d.length&&("object"===typeof a&&(c=a.enrich,b=a.offset,a=a.limit),d=Va.call(e,d,a||100,b,c));return d};G();W.prototype.search=function(a,b,c,e){c||(!b&&ca(a)?(c=a,a=""):ca(b)&&(c=b,b=0));if(c&&c.cache){c.cache=!1;var d=this.searchCache(a,b,c);c.cache=!0;return d}let f=[];var g=[];let k,h;let l,m;let t=0;var q=!0;let p;if(c){c.constructor===Array&&(c={index:c});a=c.query||a;k=c.pluck;h=c.merge;l=k||c.field||(l=c.index)&&(l.index?null:l);m=this.tag&&c.tag;var r=c.suggest;q=!1!==c.resolve;q||k||!(l=l||this.field)||(L(l)?k=l:(l.constructor===Array&&1===l.length&&(l=l[0]),k=l.field||l.index));p=q&&this.store&&
c.highlight;d=!!p||q&&this.store&&c.enrich;b=c.limit||b;var u=c.offset||0;b||(b=100);if(m&&(!this.db||!e)){m.constructor!==Array&&(m=[m]);var n=[];for(let B=0,z;B<m.length;B++)if(z=m[B],z.field&&z.tag){var y=z.tag;if(y.constructor===Array)for(var x=0;x<y.length;x++)n.push(z.field,y[x]);else n.push(z.field,y)}else{y=Object.keys(z);for(let A=0,D,C;A<y.length;A++)if(D=y[A],C=z[D],C.constructor===Array)for(x=0;x<C.length;x++)n.push(D,C[x]);else n.push(D,C)}m=n;if(!a){q=[];if(n.length)for(g=0;g<n.length;g+=
2){if(this.db){r=this.index.get(n[g]);if(!r)continue;q.push(r=r.db.tag(n[g+1],b,u,d))}else r=db.call(this,n[g],n[g+1],b,u,d);f.push({field:n[g],tag:n[g+1],result:r})}return q.length?Promise.all(q).then(function(B){for(let z=0;z<B.length;z++)f[z].result=B[z];return f}):f}}l&&l.constructor!==Array&&(l=[l])}l||(l=this.field);let J;n=(this.worker||this.db)&&!e&&[];for(let B=0,z,A,D;B<l.length;B++){A=l[B];if(this.db&&this.tag&&!this.F[B])continue;let C;L(A)||(C=A,A=C.field,a=C.query||a,b=fb(C.limit,b),
u=fb(C.offset,u),r=fb(C.suggest,r),p=q&&this.store&&fb(C.highlight,p),d=!!p||q&&this.store&&fb(C.enrich,d));if(e)z=e[B];else if(y=C||c,x=this.index.get(A),m&&(this.db&&(y.tag=m,J=x.db.support_tag_search,y.field=l),J||(y.enrich=!1)),n){n[B]=x.search(a,b,y);y&&d&&(y.enrich=d);continue}else z=x.search(a,b,y),y&&d&&(y.enrich=d);D=z&&(q?z.length:z.result.length);if(m&&D){y=[];x=0;if(this.db&&e){if(!J)for(let E=l.length;E<e.length;E++){let H=e[E];if(H&&H.length)x++,y.push(H);else if(!r)return q?f:new Y(f)}}else for(let E=
0,H,M;E<m.length;E+=2){H=this.tag.get(m[E]);if(!H)if(r)continue;else return q?f:new Y(f);if(M=(H=H&&H.get(m[E+1]))&&H.length)x++,y.push(H);else if(!r)return q?f:new Y(f)}if(x){z=Ua(z,y,q);D=z.length;if(!D&&!r)return q?z:new Y(z);x--}}if(D)g[t]=A,f.push(z),t++;else if(1===l.length)return q?f:new Y(f)}if(n){if(this.db&&m&&m.length&&!J)for(d=0;d<m.length;d+=2){g=this.index.get(m[d]);if(!g)if(r)continue;else return q?f:new Y(f);n.push(g.db.tag(m[d+1],b,u,!1))}const B=this;return Promise.all(n).then(function(z){return z.length?
B.search(a,b,c,z):z})}if(!t)return q?f:new Y(f);if(k&&(!d||!this.store))return f[0];n=[];for(u=0;u<g.length;u++){r=f[u];d&&r.length&&"undefined"===typeof r[0].doc&&(this.db?n.push(r=this.index.get(this.field[0]).db.enrich(r)):r=X.call(this,r));if(k)return q?p?cb(a,r,this.index,k,p):r:new Y(r);f[u]={field:g[u],result:r}}if(d&&this.db&&n.length){const B=this;return Promise.all(n).then(function(z){for(let A=0;A<z.length;A++)f[A].result=z[A];return h?gb(f):p?cb(a,f,B.index,k,p):f})}return h?gb(f):p?cb(a,
f,this.index,k,p):f};function fb(a,b){return"undefined"===typeof a?b:a}function gb(a){const b=[],c=G();for(let e=0,d,f;e<a.length;e++){d=a[e];f=d.result;for(let g=0,k,h,l;g<f.length;g++)h=f[g],"object"!==typeof h&&(h={id:h}),k=h.id,(l=c[k])?l.push(d.field):(h.field=c[k]=[d.field],b.push(h))}return b}function db(a,b,c,e,d){a=this.tag.get(a);if(!a)return[];if((b=(a=a&&a.get(b))&&a.length-e)&&0<b){if(b>c||e)a=a.slice(e,e+c);d&&(a=X.call(this,a));return a}}
function X(a){if(!this||!this.store)return a;const b=Array(a.length);for(let c=0,e;c<a.length;c++)e=a[c],b[c]={id:e,doc:this.store.get(e)};return b};function W(a){if(!this||this.constructor!==W)return new W(a);const b=a.document||a.doc||a;let c,e;this.F=[];this.field=[];this.J=[];this.key=(c=b.key||b.id)&&hb(c,this.J)||"id";(e=a.keystore||0)&&(this.keystore=e);this.fastupdate=!!a.fastupdate;this.reg=!this.fastupdate||a.worker||a.db?e?new U(e):new Set:e?new T(e):new Map;this.C=(c=b.store||null)&&c&&!0!==c&&[];this.store=c&&(e?new T(e):new Map);this.cache=(c=a.cache||null)&&new ib(c);a.cache=!1;this.worker=a.worker||!1;this.priority=a.priority||
4;this.index=jb.call(this,a,b);this.tag=null;if(c=b.tag)if("string"===typeof c&&(c=[c]),c.length){this.tag=new Map;this.D=[];this.R=[];for(let d=0,f,g;d<c.length;d++){f=c[d];g=f.field||f;if(!g)throw Error("The tag field from the document descriptor is undefined.");f.custom?this.D[d]=f.custom:(this.D[d]=hb(g,this.J),f.filter&&("string"===typeof this.D[d]&&(this.D[d]=new String(this.D[d])),this.D[d].I=f.filter));this.R[d]=g;this.tag.set(g,new Map)}}if(this.worker){this.fastupdate=!1;const d=[];for(const f of this.index.values())f.then&&
d.push(f);if(d.length){const f=this;return Promise.all(d).then(function(g){const k=new Map;let h=0;for(const m of f.index.entries()){const t=m[0];var l=m[1];if(l.then){l=d[h].encoder||{};let q=k.get(l);q||(q=l.encode?l:new ma(l),k.set(l,q));l=g[h];l.encoder=q;f.index.set(t,l);h++}}return f})}}else a.db&&(this.fastupdate=!1,this.mount(a.db))}v=W.prototype;
...
function jb(a,b){const c=new Map;let e=b.index||b.field||b;L(e)&&(e=[e]);for(let d=0,f,g;d<e.length;d++){f=e[d];L(f)||(g=f,f=f.field);g=ca(g)?Object.assign({},a,g):a;if(this.worker){const k=new Ba(g);k.encoder=g.encoder;c.set(f,k)}this.worker||c.set(f,new O(g,this.reg));g.custom?this.F[d]=g.custom:(this.F[d]=hb(f,this.J),g.filter&&("string"===typeof this.F[d]&&(this.F[d]=new String(this.F[d])),this.F[d].I=g.filter));this.field[d]=f}if(this.C){a=b.store;L(a)&&(a=[a]);for(let d=0,f,g;d<a.length;d++)f=
a[d],g=f.field||f,f.custom?(this.C[d]=f.custom,f.custom.V=g):(this.C[d]=hb(g,this.J),f.filter&&("string"===typeof this.C[d]&&(this.C[d]=new String(this.C[d])),this.C[d].I=f.filter))}return c}function hb(a,b){const c=a.split(":");let e=0;for(let d=0;d<c.length;d++)a=c[d],"]"===a[a.length-1]&&(a=a.substring(0,a.length-2))&&(b[e]=!0),a&&(c[e++]=a);e<c.length&&(c.length=e);return 1<e?c:c[0]}v.append=function(a,b){return this.add(a,b,!0)};v.update=function(a,b){return this.remove(a).add(a,b)};
...
v.get=function(a){return this.db?this.index.get(this.field[0]).db.enrich(a).then(function(b){return b[0]&&b[0].doc||null}):this.store.get(a)||null};v.set=function(a,b){"object"===typeof a&&(b=a,a=ea(b,this.key));this.store.set(a,b);return this};v.searchCache=kb;
v.export=function(a,b,c=0,e=0){if(c<this.field.length){const g=this.field[c];if((b=this.index.get(g).export(a,g,c,e=1))&&b.then){const k=this;return b.then(function(){return k.export(a,g,c+1)})}return this.export(a,g,c+1)}let d,f;switch(e){case 0:d="reg";f=Ha(this.reg);b=null;break;case 1:d="tag";f=this.tag&&Fa(this.tag,this.reg.size);b=null;break;case 2:d="doc";f=this.store&&Da(this.store);b=null;break;default:return}return Ja.call(this,a,b,d,f,c,e)};
...
ib.prototype.get=function(a){const b=this.cache.get(a);b&&this.h!==a&&(this.cache.delete(a),this.cache.set(this.h=a,b));return b};ib.prototype.remove=function(a){for(const b of this.cache){const c=b[0];b[1].includes(a)&&this.cache.delete(c)}};ib.prototype.clear=function(){this.cache.clear();this.h=""};const lb={normalize:!1,numeric:!1,dedupe:!1};const mb={};const nb=new Map([["b","p"],["v","f"],["w","f"],["z","s"],["x","s"],["d","t"],["n","m"],["c","k"],["g","k"],["j","k"],["q","k"],["i","e"],["y","e"],["u","o"]]);const ob=new Map([["ae","a"],["oe","o"],["sh","s"],["kh","k"],["th","t"],["ph","f"],["pf","f"]]),pb=[/([^aeo])h(.)/g,"$1$2",/([aeo])h([^aeo]|$)/g,"$1$2",/(.)\1+/g,"$1"];const qb={a:"",e:"",i:"",o:"",u:"",y:"",b:1,f:1,p:1,v:1,c:2,g:2,j:2,k:2,q:2,s:2,x:2,z:2,"\u00df":2,d:3,t:3,l:4,m:5,n:5,r:6};var rb={Exact:lb,Default:mb,Normalize:mb,LatinBalance:{mapper:nb},LatinAdvanced:{mapper:nb,matcher:ob,replacer:pb},LatinExtra:{mapper:nb,replacer:pb.concat([/(?!^)[aeo]/g,""]),matcher:ob},LatinSoundex:{dedupe:!1,include:{letter:!0},finalize:function(a){for(let c=0;c<a.length;c++){var b=a[c];let e=b.charAt(0),d=qb[e];for(let f=1,g;f<b.length&&(g=b.charAt(f),"h"===g||"w"===g||!(g=qb[g])||g===d||(e+=g,d=g,4!==e.length));f++);a[c]=e}}},CJK:{split:""},LatinExact:lb,LatinDefault:mb,LatinSimple:mb};O.prototype.remove=function(a,b){const c=this.reg.size&&(this.fastupdate?this.reg.get(a):this.reg.has(a));if(c){if(this.fastupdate)for(let e=0,d;e<c.length;e++){if(d=c[e])if(2>d.length)d.pop();else{const f=d.indexOf(a);f===c.length-1?d.pop():d.splice(f,1)}}else sb(this.map,a),this.depth&&sb(this.ctx,a);b||this.reg.delete(a)}this.db&&(this.commit_task.push({del:a}),this.T&&tb(this));this.cache&&this.cache.remove(a);return this};
function sb(a,b){let c=0;var e="undefined"===typeof b;if(a.constructor===Array)for(let d=0,f,g;d<a.length;d++){if((f=a[d])&&f.length)if(e)c++;else if(g=f.indexOf(b),0<=g){1<f.length?(f.splice(g,1),c++):delete a[d];break}else c++}else for(let d of a.entries()){e=d[0];const f=sb(d[1],b);f?c+=f:a.delete(e)}return c};const ub={memory:{resolution:1},performance:{resolution:3,fastupdate:!0,context:{depth:1,resolution:1}},match:{tokenize:"forward"},score:{resolution:9,context:{depth:2,resolution:3}}};O.prototype.add=function(a,b,c,e){if(b&&(a||0===a)){if(!e&&!c&&this.reg.has(a))return this.update(a,b);e=this.depth;b=this.encoder.encode(b,!e);const l=b.length;if(l){const m=G(),t=G(),q=this.resolution;for(let p=0;p<l;p++){let r=b[this.rtl?l-1-p:p];var d=r.length;if(d&&(e||!t[r])){var f=this.score?this.score(b,r,p,null,0):vb(q,l,p),g="";switch(this.tokenize){case "full":if(2<d){for(let u=0,n;u<d;u++)for(f=d;f>u;f--){g=r.substring(u,f);n=this.rtl?d-1-u:u;var k=this.score?this.score(b,r,p,g,n):vb(q,
l,p,d,n);wb(this,t,g,k,a,c)}break}case "bidirectional":case "reverse":if(1<d){for(k=d-1;0<k;k--){g=r[this.rtl?d-1-k:k]+g;var h=this.score?this.score(b,r,p,g,k):vb(q,l,p,d,k);wb(this,t,g,h,a,c)}g=""}case "forward":if(1<d){for(k=0;k<d;k++)g+=r[this.rtl?d-1-k:k],wb(this,t,g,f,a,c);break}default:if(wb(this,t,r,f,a,c),e&&1<l&&p<l-1)for(d=G(),g=this.U,f=r,k=Math.min(e+1,this.rtl?p+1:l-p),d[f]=1,h=1;h<k;h++)if((r=b[this.rtl?l-1-p-h:p+h])&&!d[r]){d[r]=1;const u=this.score?this.score(b,f,p,r,h-1):vb(g+(l/
2>g?0:1),l,p,k-1,h-1),n=this.bidirectional&&r>f;wb(this,m,n?f:r,u,a,c,n?r:f)}}}}this.fastupdate||this.reg.add(a)}else b=""}this.db&&(b||this.commit_task.push({del:a}),this.T&&tb(this));return this};
...
function vb(a,b,c,e,d){return c&&1<a?b+(e||0)<=a?c+(d||0):(a-1)/(b+(e||0))*(c+(d||0))+1|0:0};O.prototype.search=function(a,b,c){c||(b||"object"!==typeof a?"object"===typeof b&&(c=b,b=0):(c=a,a=""));if(c&&c.cache)return c.cache=!1,a=this.searchCache(a,b,c),c.cache=!0,a;let e=[],d,f,g,k=0,h,l,m,t,q;c&&(a=c.query||a,b=c.limit||b,k=c.offset||0,f=c.context,g=c.suggest,q=(h=c.resolve)&&c.enrich,m=c.boost,t=c.resolution,l=this.db&&c.tag);"undefined"===typeof h&&(h=this.resolve);f=this.depth&&!1!==f;let p=this.encoder.encode(a,!f);d=p.length;b=b||(h?100:0);if(1===d)return xb.call(this,p[0],"",b,
k,h,q,l);if(2===d&&f&&!g)return xb.call(this,p[1],p[0],b,k,h,q,l);let r=G(),u=0,n;f&&(n=p[0],u=1);t||0===t||(t=n?this.U:this.resolution);if(this.db){if(this.db.search&&(c=this.db.search(this,p,b,k,g,h,q,l),!1!==c))return c;const y=this;return async function(){for(let x,J;u<d;u++){if((J=p[u])&&!r[J]){r[J]=1;x=await yb(y,J,n,0,0,!1,!1);if(x=zb(x,e,g,t)){e=x;break}n&&(g&&x&&e.length||(n=J))}g&&n&&u===d-1&&!e.length&&(t=y.resolution,n="",u=-1,r=G())}return Ab(e,t,b,k,g,m,h)}()}for(let y,x;u<d;u++){if((x=
p[u])&&!r[x]){r[x]=1;y=yb(this,x,n,0,0,!1,!1);if(y=zb(y,e,g,t)){e=y;break}n&&(g&&y&&e.length||(n=x))}g&&n&&u===d-1&&!e.length&&(t=this.resolution,n="",u=-1,r=G())}return Ab(e,t,b,k,g,m,h)};function Ab(a,b,c,e,d,f,g){let k=a.length,h=a;if(1<k)h=Sa(a,b,c,e,d,f,g);else if(1===k)return g?Va.call(null,a[0],c,e):new Y(a[0]);return g?h:new Y(h)}
function xb(a,b,c,e,d,f,g){a=yb(this,a,b,c,e,d,f,g);return this.db?a.then(function(k){return d?k||[]:new Y(k)}):a&&a.length?d?Va.call(this,a,c,e):new Y(a):d?[]:new Y}function zb(a,b,c,e){let d=[];if(a&&a.length){if(a.length<=e){b.push(a);return}for(let f=0,g;f<e;f++)if(g=a[f])d[f]=g;if(d.length){b.push(d);return}}if(!c)return d}function yb(a,b,c,e,d,f,g,k){let h;c&&(h=a.bidirectional&&b>c)&&(h=c,c=b,b=h);if(a.db)return a.db.get(b,c,e,d,f,g,k);a=c?(a=a.ctx.get(c))&&a.get(b):a.map.get(b);return a};function O(a,b){if(!this||this.constructor!==O)return new O(a);if(a){var c=L(a)?a:a.preset;c&&(a=Object.assign({},ub[c],a))}else a={};c=a.context;const e=!0===c?{depth:1}:c||{},d=L(a.encoder)?rb[a.encoder]:a.encode||a.encoder||{};this.encoder=d.encode?d:"object"===typeof d?new ma(d):{encode:d};this.resolution=a.resolution||9;this.tokenize=c=(c=a.tokenize)&&"default"!==c&&"exact"!==c&&c||"strict";this.depth="strict"===c&&e.depth||0;this.bidirectional=!1!==e.bidirectional;this.fastupdate=!!a.fastupdate;
...
v.open=function(){if(this.db)return this.db;let a=this;navigator.storage&&navigator.storage.persist();Db[a.id]||(Db[a.id]=[]);Db[a.id].push(a.field);const b=Bb.open(a.id,1);b.onupgradeneeded=function(){const c=a.db=this.result;for(let e=0,d;e<Cb.length;e++){d=Cb[e];for(let f=0,g;f<Db[a.id].length;f++)g=Db[a.id][f],c.objectStoreNames.contains(d+("reg"!==d?g?":"+g:"":""))||c.createObjectStore(d+("reg"!==d?g?":"+g:"":""))}};return a.db=Z(b,function(c){a.db=c;a.db.onversionchange=function(){a.close()}})};
v.close=function(){this.db&&this.db.close();this.db=null};v.destroy=function(){const a=Bb.deleteDatabase(this.id);return Z(a)};v.clear=function(){const a=[];for(let c=0,e;c<Cb.length;c++){e=Cb[c];for(let d=0,f;d<Db[this.id].length;d++)f=Db[this.id][d],a.push(e+("reg"!==e?f?":"+f:"":""))}const b=this.db.transaction(a,"readwrite");for(let c=0;c<a.length;c++)b.objectStore(a[c]).clear();return Z(b)};
v.get=function(a,b,c=0,e=0,d=!0,f=!1){a=this.db.transaction((b?"ctx":"map")+(this.field?":"+this.field:""),"readonly").objectStore((b?"ctx":"map")+(this.field?":"+this.field:"")).get(b?b+":"+a:a);const g=this;return Z(a).then(function(k){let h=[];if(!k||!k.length)return h;if(d){if(!c&&!e&&1===k.length)return k[0];for(let l=0,m;l<k.length;l++)if((m=k[l])&&m.length){if(e>=m.length){e-=m.length;continue}const t=c?e+Math.min(m.length-e,c):m.length;for(let q=e;q<t;q++)h.push(m[q]);e=0;if(h.length===c)break}return f?
...
for(let m=0,t,q;m<l;m++)if((q=g[m])&&q.length){if((t=k[m])&&t.length)for(h=0;h<q.length;h++)t.push(q[h]);else k[m]=q;h=1}}else k=g,h=1;h&&e.put(k,f)})}}),await this.transaction("ctx","readwrite",function(e){for(const d of a.ctx){const f=d[0],g=d[1];for(const k of g){const h=k[0],l=k[1];l.length&&(b?e.put(l,f+":"+h):e.get(f+":"+h).onsuccess=function(){let m=this.result;var t;if(m&&m.length){const q=Math.max(m.length,l.length);for(let p=0,r,u;p<q;p++)if((u=l[p])&&u.length){if((r=m[p])&&r.length)for(t=
0;t<u.length;t++)r.push(u[t]);else m[p]=u;t=1}}else m=l,t=1;t&&e.put(m,f+":"+h)})}}}),a.store?await this.transaction("reg","readwrite",function(e){for(const d of a.store){const f=d[0],g=d[1];e.put("object"===typeof g?JSON.stringify(g):1,f)}}):a.bypass||await this.transaction("reg","readwrite",function(e){for(const d of a.reg.keys())e.put(1,d)}),a.tag&&await this.transaction("tag","readwrite",function(e){for(const d of a.tag){const f=d[0],g=d[1];g.length&&(e.get(f).onsuccess=function(){let k=this.result;
k=k&&k.length?k.concat(g):g;e.put(k,f)})}}),a.map.clear(),a.ctx.clear(),a.tag&&a.tag.clear(),a.store&&a.store.clear(),a.document||a.reg.clear())};function Fb(a,b,c){const e=a.value;let d,f=0;for(let g=0,k;g<e.length;g++){if(k=c?e:e[g]){for(let h=0,l,m;h<b.length;h++)if(m=b[h],l=k.indexOf(m),0<=l)if(d=1,1<k.length)k.splice(l,1);else{e[g]=[];break}f+=k.length}if(c)break}f?d&&a.update(e):a.delete();a.continue()}
...
function Z(a,b){return new Promise((c,e)=>{a.onsuccess=a.oncomplete=function(){b&&b(this.result);b=null;c(this.result)};a.onerror=a.onblocked=e;a=null})};export default {Index:O,Charset:rb,Encoder:ma,Document:W,Worker:Ba,Resolver:Y,IndexedDB:Eb,Language:{}};
export const Index=O;export const Charset=rb;export const Encoder=ma;export const Document=W;export const Worker=Ba;export const Resolver=Y;export const IndexedDB=Eb;export const Language={};