/**!
 * FlexSearch.js v0.8.102 (Bundle/Module)
...
else{if(n.length>c||d)n=n.slice(d,c+d),c-=n.length,d&&(d-=n.length);e.push(n);if(!c)break}h=1<e.length?[].concat.apply([],e):e[0]}}return h}
...
0;for(const k of e.index.entries()){const h=k[0];k[1].then&&e.index.set(h,f[g++])}return e})}}else a.db&&this.mount(a.db)}u=T.prototype;
...
this.not(l.not);else continue;d[h]=m;m.then&&c.push(m)}if(c.length)return Promise.all(c).then(function(){a.result.length&&(d=[a.result].concat(d));a.result=$a(d,e,f,g,!k,a.F);return k?a.result:a});d.length&&(this.result.length&&(d=[this.result].concat(d)),this.result=$a(d,e,f,g,!k,a.F));return k?this.result:this};
...
if(++m===b)return d}return d};X.prototype.not=function(){const a=this;let b=arguments;var c=b[0];if(c.then)return c.then(function(){return a.not.apply(a,b)});if(c[0]&&c[0].index)return this.not.apply(this,c);let d=[];c=[];let e=0,f=0,g;for(let k=0,h;k<b.length;k++)if(h=b[k]){e=h.limit||0;f=h.offset||0;g=h.resolve;let l;if(h.constructor===X)l=h.result;else if(h.constructor===Array)l=h;else if(h.index)h.resolve=!1,l=h.index.search(h).result;else if(h.or)l=this.or(h.or);else if(h.and)l=this.and(h.and);else if(h.xor)l=this.xor(h.xor);
else continue;d[k]=l;l.then&&c.push(l)}if(c.length)return Promise.all(c).then(function(){a.result=ab.call(a,d,e,f,g);return g?a.result:a});d.length&&(this.result=ab.call(this,d,e,f,g));return g?this.result:this};
...
function gb(a){let b=0;if(a.constructor===Array)for(let c=0,d;c<a.length;c++)(d=a[c])&&(b+=d.length);else for(const c of a){const d=c[0],e=gb(c[1]);e?b+=e:a.delete(d)}return b}u.cleanup=function(){if(!this.fastupdate)return this;gb(this.map);this.depth&&gb(this.ctx);return this};u.searchCache=Ka;
...
u.close=function(){this.db.close();this.db=null};u.destroy=function(){return hb.deleteDatabase(this.id+(this.field?":"+this.field:""))};u.clear=function(){const a=this.db.transaction(ib,"readwrite");for(let b=0;b<ib.length;b++)a.objectStore(ib[b]).clear();return Z(a)};
u.get=function(a,b,c=0,d=0,e=!0,f=!1){a=this.db.transaction(b?"ctx":"map","readonly").objectStore(b?"ctx":"map").get(b?b+":"+a:a);const g=this;return Z(a).then(function(k){let h=[];if(!k||!k.length)return h;if(e){if(!c&&!d&&1===k.length)return k[0];for(let l=0,m;l<k.length;l++)if((m=k[l])&&m.length){if(d>=m.length){d-=m.length;continue}const q=c?d+Math.min(m.length-d,c):m.length;for(let n=d;n<q;n++)h.push(m[n]);d=0;if(h.length===c)break}return f?g.enrich(h):h}return k})};
...
u.transaction=function(a,b,c){let d=this.h[a+":"+b];if(d)return c.call(this,d);let e=this.db.transaction(a,b);this.h[a+":"+b]=d=e.objectStore(a);return new Promise((f,g)=>{e.onerror=k=>{this.h[a+":"+b]=null;e.abort();e=d=null;g(k)};e.oncomplete=k=>{e=d=this.h[a+":"+b]=null;f(k||!0)};return c.call(this,d)})};
...
for(let m=0,q,n;m<l;m++)if((n=g[m])&&n.length){if((q=k[m])&&q.length)for(h=0;h<n.length;h++)q.push(n[h]);else k[m]=n;h=1}}else k=g,h=1;h&&d.put(k,f)})}}),await this.transaction("ctx","readwrite",function(d){for(const e of a.ctx){const f=e[0],g=e[1];for(const k of g){const h=k[0],l=k[1];l.length&&(b?d.put(l,f+":"+h):d.get(f+":"+h).onsuccess=function(){let m=this.result;var q;if(m&&m.length){const n=Math.max(m.length,l.length);for(let p=0,r,t;p<n;p++)if((t=l[p])&&t.length){if((r=m[p])&&r.length)for(q=
0;q<t.length;q++)r.push(t[q]);else m[p]=t;q=1}}else m=l,q=1;q&&d.put(m,f+":"+h)})}}}),a.store?await this.transaction("reg","readwrite",function(d){for(const e of a.store){const f=e[0],g=e[1];d.put("object"===typeof g?JSON.stringify(g):1,f)}}):a.bypass||await this.transaction("reg","readwrite",function(d){for(const e of a.reg.keys())d.put(1,e)}),a.tag&&await this.transaction("tag","readwrite",function(d){for(const e of a.tag){const f=e[0],g=e[1];g.length&&(d.get(f).onsuccess=function(){let k=this.result;
k=k&&k.length?k.concat(g):g;d.put(k,f)})}}),a.map.clear(),a.ctx.clear(),a.tag&&a.tag.clear(),a.store&&a.store.clear(),a.document||a.reg.clear())};
