/**!
 * FlexSearch.js v0.8.0 (Bundle/Module)
...
this.cache&&h.length<=this.A&&(this.J.set(h,g),this.J.size>this.S&&(this.J.clear(),this.A=this.A/1.1|0));g&&c.push(g)}this.finalize&&(c=this.finalize(c)||c);this.cache&&a.length<=this.h&&(this.H.set(a,c),this.H.size>this.S&&(this.H.clear(),this.h=this.h/1.1|0));return c};function ka(a){a.L=null;a.H.clear();a.J.clear()};async function la(a){a=a.data;var b=self._index;const c=a.args;var d=a.task;switch(d){case "init":d=a.options||{};(b=d.config)&&(d=(await import(b))["default"]);(b=a.factory)?(Function("return "+b)()(self),self._index=new self.FlexSearch.Index(d),delete self.FlexSearch):self._index=new K(d);postMessage({id:a.id});break;default:a=a.id,b=b[d].apply(b,c),postMessage("search"===d?{id:a,msg:b}:{id:a})}};let M=0;
function N(a={}){function b(g){function h(k){k=k.data||k;const l=k.id,m=l&&e.h[l];m&&(m(k.msg),delete e.h[l])}this.worker=g;this.h=A();if(this.worker){d?this.worker.on("message",h):this.worker.onmessage=h;if(a.config)return new Promise(function(k){e.h[++M]=function(){k(e)};e.worker.postMessage({id:M,task:"init",factory:c,options:a})});this.worker.postMessage({task:"init",factory:c,options:a});return this}}if(!this)return new N(a);let c="undefined"!==typeof self?self._factory:"undefined"!==typeof window?
...
function wa(a,b){const c=A(),d=[];for(let e=0,f;e<b.length;e++){f=b[e];for(let g=0;g<f.length;g++)c[f[g]]=1}for(let e=0,f;e<a.length;e++)f=a[e],1===c[f]&&(d.push(f),c[f]=2);return d};T.prototype.search=function(a,b,c,d){c||(!b&&D(a)?(c=a,a=""):D(b)&&(c=b,b=0));let e=[],f=[],g;let h;let k;let l,m=0,n;if(c){c.constructor===Array&&(c={index:c});a=c.query||a;g=c.pluck;h=c.merge;k=g||c.field||c.index;var p=this.tag&&c.tag;var q=this.store&&c.enrich;var t=c.suggest;n=c.V;b=c.limit||b;l=c.offset||0;b||(b=100);if(p&&(!this.db||!d)){p.constructor!==Array&&(p=[p]);var r=[];for(let y=0,w;y<p.length;y++)if(w=p[y],w.field&&w.tag){var v=w.tag;if(v.constructor===Array)for(var x=0;x<v.length;x++)r.push(w.field,
v[x]);else r.push(w.field,v)}else{v=Object.keys(w);for(let B=0,J,E;B<v.length;B++)if(J=v[B],E=w[J],E.constructor===Array)for(x=0;x<E.length;x++)r.push(J,E[x]);else r.push(J,E)}p=r;if(!a){t=[];if(r.length)for(p=0;p<r.length;p+=2){if(this.db){d=this.index.get(r[p]);if(!d)continue;t.push(d=d.db.tag(r[p+1],b,l,q))}else d=xa.call(this,r[p],r[p+1],b,l,q);e.push({field:r[p],tag:r[p+1],result:d})}return t.length?Promise.all(t).then(function(y){for(let w=0;w<y.length;w++)e[w].result=y[w];return e}):e}}C(k)&&
(k=[k])}k||(k=this.field);r=!d&&(this.worker||this.db)&&[];let F;for(let y=0,w,B,J;y<k.length;y++){B=k[y];if(this.db&&this.tag&&!this.D[y])continue;let E;C(B)||(E=B,B=E.field,a=E.query||a,b=E.limit||b,l=E.offset||l,t=E.suggest||t,q=this.store&&(E.enrich||q));if(d)w=d[y];else if(v=E||c,x=this.index.get(B),p&&(this.db&&(v.tag=p,F=x.db.support_tag_search,v.field=k),F||(v.enrich=!1)),r){r[y]=x.search(a,b,v);v&&q&&(v.enrich=q);continue}else w=x.search(a,b,v),v&&q&&(v.enrich=q);J=w&&w.length;if(p&&J){v=
[];x=0;if(this.db&&d){if(!F)for(let L=k.length;L<d.length;L++){let G=d[L];if(G&&G.length)x++,v.push(G);else if(!t)return e}}else for(let L=0,G,Za;L<p.length;L+=2){G=this.tag.get(p[L]);if(!G)if(t)continue;else return e;if(Za=(G=G&&G.get(p[L+1]))&&G.length)x++,v.push(G);else if(!t)return e}if(x){w=wa(w,v);J=w.length;if(!J&&!t)return e;x--}}if(J)f[m]=B,e.push(w),m++;else if(1===k.length)return e}if(r){if(this.db&&p&&p.length&&!F)for(q=0;q<p.length;q+=2){d=this.index.get(p[q]);if(!d)if(t)continue;else return e;
...
K.prototype.search=function(a,b,c){c||(!b&&D(a)?(c=a,a=""):D(b)&&(c=b,b=0));let d=[],e;let f,g=0,h,k,l,m;if(c){a=c.query||a;b=c.limit||b;g=c.offset||0;var n=c.context;f=c.suggest;(h=Va&&!1!==c.resolve)||(Va=0);k=h&&c.enrich;m=c.boost;l=this.db&&c.tag}else h=this.resolve||Va;a=this.encoder.encode(a);e=a.length;b||!h||(b=100);if(1===e)return Wa.call(this,a[0],"",b,g,h,k,l);n=this.depth&&!1!==n;if(2===e&&n&&!f)return Wa.call(this,a[0],a[1],b,g,h,k,l);let p=c=0;if(1<e){const r=A(),v=[];for(let x=0,F;x<
e;x++)if((F=a[x])&&!r[F]){if(f||this.db||Y(this,F))v.push(F),r[F]=1;else return h?d:new X(d);const y=F.length;c=Math.max(c,y);p=p?Math.min(p,y):y}a=v;e=a.length}if(!e)return h?d:new X(d);let q=0,t;if(1===e)return Wa.call(this,a[0],"",b,g,h,k,l);if(2===e&&n&&!f)return Wa.call(this,a[0],a[1],b,g,h,k,l);1<e&&(n?(t=a[0],q=1):9<c&&3<c/p&&a.sort(aa));if(this.db){if(this.db.search&&(n=this.db.search(this,a,b,g,f,h,k,l),!1!==n))return n;const r=this;return async function(){for(let v,x;q<e;q++){x=a[q];t?(v=
await Y(r,x,t,0,0,!1,!1),v=Xa(v,d,f,r.R),f&&!1===v&&d.length||(t=x)):(v=await Y(r,x,"",0,0,!1,!1),v=Xa(v,d,f,r.resolution));if(v)return v;if(f&&q===e-1){let F=d.length;if(!F){if(t){t="";q=-1;continue}return d}if(1===F)return h?W(d[0],b,g):new X(d[0])}}return h?ua(d,r.resolution,b,g,f,m,h):new X(d[0])}()}for(let r,v;q<e;q++){v=a[q];t?(r=Y(this,v,t,0,0,!1,!1),r=Xa(r,d,f,this.R),f&&!1===r&&d.length||(t=v)):(r=Y(this,v,"",0,0,!1,!1),r=Xa(r,d,f,this.resolution));if(r)return r;if(f&&q===e-1){n=d.length;
if(!n){if(t){t="";q=-1;continue}return d}if(1===n)return h?W(d[0],b,g):new X(d[0])}}d=ua(d,this.resolution,b,g,f,m,h);return h?d:new X(d)};function Wa(a,b,c,d,e,f,g){a=Y(this,a,b,c,d,e,f,g);return this.db?a.then(function(h){return e?h:h&&h.length?e?W(h,c,d):new X(h):e?[]:new X([])}):a&&a.length?e?W(a,c,d):new X(a):e?[]:new X([])}function Xa(a,b,c,d){let e=[];if(a){d=Math.min(a.length,d);for(let f=0,g;f<d;f++)(g=a[f])&&g&&(e[f]=g);if(e.length){b.push(e);return}}return!c&&e}
...
function $a(a){let b=0;if(a.constructor===Array)for(let c=0,d;c<a.length;c++)(d=a[c])&&(b+=d.length);else for(const c of a){const d=c[0],e=$a(c[1]);e?b+=e:a.delete(d)}return b}u.cleanup=function(){if(!this.fastupdate)return this;$a(this.map);this.depth&&$a(this.ctx);return this};u.searchCache=Da;
...
u.close=function(){this.db.close();this.db=null};u.destroy=function(){return ab.deleteDatabase(this.id+(this.field?":"+this.field:""))};u.clear=function(){const a=this.db.transaction(bb,"readwrite");for(let b=0;b<bb.length;b++)a.objectStore(bb[b]).clear();return Z(a)};
...
function Z(a){return new Promise((b,c)=>{a.onsuccess=function(){b(this.result)};a.oncomplete=function(){b(this.result)};a.onerror=c;a=null})};export default {Index:K,Charset:Ma,Encoder:I,Document:T,Worker:N,Resolver:X,IndexedDB:cb,Language:{}};
export const Index=K;export const Charset=Ma;export const Encoder=I;export const Document=T;export const Worker=N;export const Resolver=X;export const IndexedDB=cb;export const Language={};